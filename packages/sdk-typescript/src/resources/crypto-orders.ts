...
export interface CreateCryptoCheckoutOptions {
  /** An idempotency key. The SDK passes it as the Idempotency-Key header. */
  idempotencyKey?: string;
}

/**
 * The list options. Sourced from @driftstack/api-types so the
...
export class CryptoOrdersResource {
  ...
  /** Mint a new crypto order. Send an `idempotencyKey` to dedupe retries. */
  createCheckout(
  ...
  /**
   * List the caller account's crypto orders (newest first).
   * Pass `status` to narrow the list to a single status.
   * Pass `cursor` from a prior page's `next_cursor` to
  ...
  /**
   * An async generator that walks every page until the
  ...
  /**
   * A cross-SDK naming alias for {@link listAll}. The Python
