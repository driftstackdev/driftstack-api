...
> **Frozen archive, split 2026-08-28 (V-2120).** Entries V-1707..V-1919 were moved here from
> [`verification-log.md`](./verification-log.md) so the live file stays under the 1,500,000-byte
> budget that `no-formatted-markdown-outgrows-the-format-hook` enforces. This file is frozen, is
> listed literally in `.prettierignore`, and is never hand-edited again. The charter above still
> governs both halves, and the guards that read this log read both — they discover archives by
> name, so nothing needed repointing.
>
> Named for the LAST entry it actually contains, not for an intended boundary. The previous
> archive is named `-through-v1671` but holds V-1500..V-1706; V-2120 records that.
...
## V-1845 — the predicate population outside `src/db` is closed, and a comment nearly cost me the answer

2026-08-26. Follow-through on the boundary V-1844 stated rather than leaving it as a caveat.

**34 `eq()` predicate sites live outside `apps/server/src/db`**, in the only two files importing
`drizzle-orm` there. Both are now triaged, one by A2 and one by me.

`services/durable-webhook-delivery.ts` (31 sites) — A2's: NOT a defect. On form it looks worse than I
described (`list()` filters on `webhookId` only; `get(deliveryId)` filters on `id` alone with no
tenancy predicate) but no route reaches it — it is the V-173 FORWARD path, with
`services/webhooks.ts` the inline implementation in production today, and the live customer path
`replayDeliveryAsCustomer` correctly does the ownership step in two. ⚠️ It is a CUTOVER hazard,
recorded before the cutover: whoever wires the durable path must add that step.

`services/email.ts` (3 sites) — mine: LIVE and WITNESSED. All three are in
`createDrizzleAccountEmailDeliveryTracker` — a lookup by normalised email, and two bare
`UPDATE accounts` scoped by `eq(accounts.id, accountId)`.

⛔ **AND THE DOC COMMENT WOULD HAVE SENT ME THE WRONG WAY.** It says "Not wired by default — pass via
`createEmailService`'s `accountEmailDeliveryTracker` option", which reads exactly like the unreachable
forward path above. It is wired: `bootstrap.ts:734`. **The comment describes the API default, not the
deployment**, and on the reachability rule I would have filed it unreachable on the strength of a
sentence. One grep of bootstrap separated them.

⭐ Witnessed by a spec that already asserts the property I would have written —
`db-account-email-delivery-tracker-drizzle` has a reachability arm, a case/whitespace-normalisation
arm, and one that says: "both writes are scoped to ONE account. These are bare UPDATEs on `accounts`;
a lost or wrong WHERE would mark every customer undeliverable at once."

⭐⭐ THE PAIR, which is the durable result of two sweeps meeting from opposite sides: **a column-NAME
sweep hands you identity keys dressed as boundaries** (V-1844's `recordHeartbeat`) **and a FORM sweep
hands you unreachable code dressed as exposure** (the durable-webhook path). Neither carries
REACHABILITY, and it is what separates a suspect from a finding. A2 has it in P-44.

⛔ INSTRUMENT NOTE — I read a log this run never wrote. The gate held on its own dirty-tree pre-flight
so the redirect line never executed, and the log PATH still held **22579 bytes from the previous day**,
reporting "3208 passed". Plausible, and wrong. Only the file count being 3208 rather than 3228 caught
it. **A log file that exists is not a log file this run wrote** — the path is now stamped with the
commit sha and its mtime checked before reading.

## V-1846 — W-10 re-measured: the 39 orphans are not unused, they are the SDK's vocabulary

2026-08-26. W-10 has stood open as "39 declared component schemas no operation `$ref`s, the fix
changes the published contract so it needs the owner's call". Re-measured, and the framing is
backwards in a way that changes what the owner is being asked.

**Count confirmed, transitively:** 83 declared component schemas, 44 reachable from an operation
(following `$ref` through components, not just at the top level), **39 orphans** — unchanged. They are
not obscure: `Account`, `Session`, `ApiKey`, `Recipe`, `WebhookEndpoint`, `CreateSessionRequest`,
`NavigateResponse`.

⭐⭐ **THEY ARE NOT UNUSED. THE CODE GENERATOR USES THEM.**
`packages/sdk-python/src/driftstack/_generated/models.py` is headed
`# generated by datamodel-codegen: filename: openapi.json`, and it emits a public class for these
names; the Go SDK emits structs for them too. **Removing the 39 would delete 39 public model classes
from the shipped Python and Go SDKs** — a breaking change for any customer importing them. "No
operation `$ref`s it" measures reachability from PATHS; the generator reads COMPONENTS.

**And the duplication is real, which is the other half:** operations carry 1186 `$ref` uses and 433
INLINE object schemas, and `POST /v1/sessions` inlines every property of `CreateSessionRequest`. So
the shape is declared twice — once inline for the operation, once in components for the generator.

⭐ WHAT THE OWNER IS ACTUALLY CHOOSING, now that it is measured: (a) leave it — duplication, but the
SDKs keep their model vocabulary; (b) make the operations `$ref` the components instead of inlining —
removes the duplication and keeps the SDK classes, but changes the published document's shape and can
move generated method signatures; (c) remove the components — deletes public SDK types. ⛔ The item as
written implies (c) is "the fix". It is the only one of the three that breaks customers.

## V-1847 — retracting most of V-1846: I read the row's first line, which is what the row warns against

2026-08-26, minutes after V-1846. Correcting it while it is still the last thing anyone would read.

⛔⛔ **WHAT I GOT WRONG.** V-1846 characterises W-10 as framed backwards, implying its proposed fix is
to REMOVE the 39 orphan components, and presents a three-way choice as though the owner had not been
given one. That is not what the row says. Its status column already records the fix as an OPERATION
change — **"33 of the document's 232 operations would change (31 responses + 11 request bodies)"** —
which does not remove a single component. The option I warned against is one nobody proposed.

⛔ **AND EVERY OTHER PART OF V-1846 WAS ALREADY IN THE ROW, MEASURED FURTHER:**

- The count re-check, including the exact drift I "found": V-1808, **the same day**, recorded the
  orphan set unchanged at 39 while the declared total moved 81 → 83, and that both new components are
  referenced so neither spec edit touched this.
- The CAUSE, which I never reached: `r.register('Name', Schema)` creates the component but does not
  tag the schema, so a route using the BARE schema still emits an inline object — 41 `.openapi('Name')`
  components are referenced, 39 of 40 `r.register` ones are orphaned.
- A decision inside the decision: `Account`/`AdminAccount` and `CreateSessionResponse`/`Session` have
  IDENTICAL property sets, and an inline shape can `$ref` only ONE name, so somebody must choose which
  name customers see in every generated client.
- **8 of the 39 are not part of the fix** — 4 have no property set, 2 appear only nested in list
  envelopes, 2 are query objects OpenAPI flattens into parameters.
- Its own boundary: matched by exact property-NAME set, "a fingerprint rather than proof of identity".

⭐⭐ **AND THE ROW ENDS BY DESCRIBING THIS EXACT FAILURE**, from a previous reader who did the same
thing: _"Everything else I 'found' ... was already measured above by V-1650 ... Rediscovered by reading
this row's first line instead of the row."_ I read the first line. The warning was in the sentence
after the one I stopped at.

⭐ **WHAT SURVIVES, and it is one fact.** Our OWN shipped SDKs are generated from that document:
`packages/sdk-python/src/driftstack/_generated/models.py` is headed
`# generated by datamodel-codegen: filename: openapi.json` and emits a public class per COMPONENT, Go
likewise. The row already states the consumer-side consequence (a customer generating a client gets
anonymous models for all 39, because the OPERATIONS inline). The generator-side complement is that the
components themselves are not inert — they are where our SDKs' named model vocabulary comes from.
⭐ That does not change the fix; it CONFIRMS its direction. Changing operations to `$ref` keeps the
components and therefore keeps those classes, which is the option the row already proposes.

⛔ **THE PROCESS LESSON, and it is the second time today.** V-1842 was the same shape — a mutation
ledger in `team-routes` recorded the finding and the warning, and I met it at the end of an
investigation instead of the start. Today's rule was "grep the target before forming a theory". This
