...
**Long-term mitigation — V-884: this has landed.** IP-based rate limiting on auth endpoints (V-246-P1-004) is live: `AUTH_IP_LIMITS` and `ipRateLimit` are applied across nine route modules including `auth.ts` and `auth-cli.ts`, and the security audit records the finding as resolved. This paragraph previously said "post-launch" and "when that lands". The follow-on it describes — replacing the logged email with a hash + IP — is therefore unblocked and still un-done; it is a change to what production logs, so it wants a deliberate decision rather than a sweep edit.
