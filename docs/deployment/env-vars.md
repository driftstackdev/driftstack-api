...
| `SESSION_PROXY_REQUIRED` | optional | per-env  | `false`      | Tri-state override (W632) for the EG-API-1.4 egress safeguard. **Unset** → inferred: a proxy is required on every `POST /v1/sessions` iff the egress backend is wired (it is, unconditionally) — i.e. unset = required. `false` → never required (the **current prod + staging posture**, set 2026-06-11): the egress feature (customer SOCKS5 config) is not yet GA, so requiring a proxy would 400 every session-create incl. onboarding. `true` → always required. **Re-engage** (remove the line / set `true`) once SOCKS5 egress is GA so sessions cannot egress unproxied. |
...
| `AUTH_VERIFY_EMAIL_URL`   | optional                 | per-env  | `https://app.driftstack.dev/verify-email`    | Override for the signup-verification link. Set only if the dashboard route differs from the conventional `/verify-email` path. Plaintext token appended as `?token=<...>`. Dev default: `http://localhost:5173/verify-email`.                                                                                                                                                                                                                                                                                                                                                                                              |
| `AUTH_MAGIC_LINK_URL`     | optional                 | per-env  | `https://app.driftstack.dev/auth/magic-link` | Override for the magic-link URL. Same shape as `AUTH_VERIFY_EMAIL_URL`. Dev default: `http://localhost:5173/auth/magic-link`. The customer-dashboard serves the consume page at `/auth/magic-link.astro`; override only if you move the dashboard route.                                                                                                                                                                                                                                                                                                                                                                   |
| `AUTH_PASSWORD_RESET_URL` | optional                 | per-env  | `https://app.driftstack.dev/reset-password`  | Override for the password-reset URL. Same shape as `AUTH_VERIFY_EMAIL_URL`. Dev default: `http://localhost:5173/reset-password`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
