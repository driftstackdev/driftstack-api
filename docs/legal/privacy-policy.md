...
Customer Data, Recording content, Session payloads, account
identifiers, or PII. The data shown is generated by Driftstack
...
control plane. If a Customer wants the session captured, the
existing V-054 Recording feature (§3.4) is the durable path; live-
session is the ephemeral path.
...
between peers; LiveKit's SFU is in the encrypted path but cannot
decrypt media when end-to-end encryption is enabled (V-306c will
ship E2EE on by default; counsel-verifies LiveKit's E2EE
