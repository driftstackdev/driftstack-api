...
| **Paddle.com Market Limited** (UK) — _contingency_    | Merchant-of-record fallback if Stripe declines underwriting. Inactive at time of writing; activated only if "Plan B" fires.                                                                                           | United Kingdom | UK GDPR adequacy regulations 2021 (UK is currently an Adequate Country under Commission Implementing Decision (EU) 2021/1772, valid through 27 June 2025 with renewal expected; counsel verifies current adequacy status). |
