...
client's session drawer.

The data passing through the network-log pipeline includes: the
request address (a URL, which can carry a query string), the HTTP
method, the response status code, the negotiated wire protocol
(HTTP/1.1, HTTP/2, HTTP/3) and ALPN token, the resource type and the
initiator, the transferred size in bytes, a timestamp and the basis it
was taken on, the load duration, and whether the response was served
from cache.
...
running on, and only while that Session is live; a report from any
other node, or for a Session already closed, is refused.
...
**Processor on Customer's behalf** under the [DPA](dpa.md), not as
Controller, on the same footing as live-session media and API Capture
requests.
