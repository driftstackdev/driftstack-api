...
- **Contradicted by the shipped system since 2026-08-10:** a customer-facing crypto rail
  ships alongside Stripe, so "sole payment processor" no longer describes what runs.
  ADR-002 carries the dated reality check with the specifics; a superseding ADR is owed
  and has no author yet. This entry states the fact, not the rationale — the latter
  belongs to whoever made the call.
...
- **Decision:** the operational-tooling workstream introduces:
  ...
  - **Audit-write before response, with three recorded exceptions.** Every admin endpoint that MUTATES writes the audit row inside the same handler that performs the action, and failure to audit fails the request — there is no "audit best-effort" path. V-1547 found the exceptions: `POST /v1/admin/oauth/clients`, `DELETE /v1/admin/oauth/clients/{id}` and `POST /v1/admin/oauth/clients/{id}/rotate-secret` write no audit row. They cannot simply be wired — the `admin_audit_log.action` enum carries no `oauth_client.*` value, and per the closed-enum clause above choosing that vocabulary is a migration-bearing change. Registering, revoking and re-secreting a third-party OAuth client are staff actions that leave no trace today; revoking one also revokes every access token it issued. Tests assert audit row presence as a regression catch (a future commit accidentally removing the audit call should make a test red, not green); V-820 added a route-level guard so a NEW admin mutation shipped without an audit call fails rather than joining silently.
  - **Admin READS are not audited, and that is a gap, not a design.** V-820 measured it, and V-1547 re-measured after widening the scan: **33** mutating `/v1/admin/*` routes exist and 30 write an audit row, and **none of the 35 admin GET routes do**. The earlier figures of 30 and 31 were not wrong so much as scoped — the guard behind them read only route files NAMED `admin*`, so seven `/v1/admin/` routes registered in `oauth.ts` and `internal-atlas-priority.ts` were outside its view. Several of those reads expose customer data — `GET /v1/admin/api-keys` lists every customer's keys, `GET /v1/admin/accounts/:id`, `/v1/admin/cost/accounts/:id` and `/v1/admin/usage/accounts/:id` expose per-account detail. A staff member reading a customer's record today leaves no trace. This clause used to read "Every admin endpoint writes the audit row", which a compliance reader would take as covering exactly the case it does not. **Whether to audit reads is an open decision** — it is a real cost (a row per list call on hot dashboard endpoints) against a real control, and it is not a documentation fix.
...
- **Decision:** account membership uses 4 roles — **owner** (single per account; billing + transfer + delete), **admin** (full operational; cannot delete account or transfer ownership), **member** (create/manage profiles + sessions; cannot manage billing or invite), **viewer** (read-only). Roles gate dashboard UI **and `/v1/*` API routes** — thirteen route modules resolve team membership, requiring `admin` for writes on another account and allowing any member to read — except `GET /v1/agent-sessions`, which requires `admin` because agent sessions carry transcripts and live control state (V-831; V-822 stated the read rule without that exception), and the pages in `GET /v1/profiles/:id/activity`, which come from those transcripts and are withheld from a member (security sweep #3, 2026-09-24), and `GET /v1/recipes` / `GET /v1/recipes/:id` in the owner's workspace, because a recipe is a saved copy of a session's steps (security sweep #15, 2026-09-24). API-key scopes (`read` / `write` / `admin`) gate routes as well; the two compose rather than dividing by layer. The team role also determines who can mint a key and what scopes they can grant. **V-822 corrected three claims here**: that roles gated the dashboard only (they gate the API too — the reading that would have an engineer skip the role check on a new route), that multi-seat was unimplemented (`team_members` + `team_invites` ship, with six `/v1/team/*` routes), and the table names, which are `team_members` / `team_invites` rather than `account_users` / `account_invites`. Note the shipped role enum is `['member', 'admin']`; `owner` is the team-owning account rather than a role value, and `viewer` is designed but not built.
...
## D-2026-08-24-01 — Unknown-field reporting on the anonymous auth routes that drop one (open)

- **Decision (OPEN — pending review):** whether `reportUnknownRequestFields` should be wired on the unauthenticated auth routes where a mistyped optional field is silently dropped. Three qualify: `POST /v1/auth/signup` (`name`), `POST /v1/auth/cli-authorize/initiate` (`client_label`), and `POST /v1/oauth/revoke` (`token_type_hint`, an RFC 7009 hint). Nothing is decided here; this entry records that the question exists and where its answer is checked.
- **Reasoning:** V-960 measured the anonymous exemption per route rather than per surface. A mistyped field can only be dropped in silence where the schema has an optional field to do without; everywhere else zod answers 400 naming the missing required field, so the exemption is earned. On these three it is real — signing up with `nam` instead of `name` returns 201 with the account created unnamed, and misspelling `token_type_hint` returns 200 with the hint discarded. Wiring the reporter is additive by construction: `lib/unknown-request-fields.ts` sets the `x-driftstack-unknown-fields` response header and logs, and states the response body is unchanged, so no existing integration can break on it. What makes it a decision rather than a fix is that it changes what an UNAUTHENTICATED caller is told.
- **Tier:** the reporter adds a response header on three public endpoints and leaves the body unchanged. Reviewer to place it: additive-header-only argues Tier 2, any change to what an unauthenticated surface returns argues Tier 3. Recorded unplaced rather than guessed.
- **V-log:** V-960 (the per-route measurement), V-1031 (which found the third route in neither list). Recorded in the register by V-1519.
- **Where it is checked:** `an-anonymous-exemption-is-earned-per-route` pins all three — the first two through `DROPS_SILENTLY`, and `/v1/oauth/revoke` in its own arm because its body is declared in the route file rather than api-types. If any schema stops dropping its field, or a route starts reporting, that guard fails and this entry is moot.

## D-2026-08-24-02 — Duration cap for paid sessions (open)

- **Decision (OPEN — pending review):** whether the seven paid tiers should have a session duration cap, and if so what it is. `free` is capped at 20 minutes; `solo_manual`, `team_manual`, `agency_manual`, `api_starter`, `api_builder`, `api_scale` and `enterprise` are all `null`. Nothing is decided here; this entry records the question and where the current posture is checked.
- **Reasoning:** verified in source rather than restated — `MAX_SESSION_MINUTES_PER_TIER` in `packages/api-types/src/common.ts` sets `free: 20` and `null` for every paid tier, and `durationCutoffsFor` in the session-duration sweeper skips a null cap outright with the comment "unlimited — never auto-destroyed". So no paid session is ever auto-destroyed on duration: one that is abandoned, or whose driver dies quietly, persists until someone notices. Session minutes are a billed dimension (`session_minute`), and `db/usage-repo.ts` records that lifecycle rows are the durable authority for them because production never had a complete per-minute writer. The billing consequence of an indefinitely-open session is analysed in the guard below rather than re-derived here.
- **What is actually undecided is the number.** Capping a paid tier is one value in one table — `durationCutoffsFor` iterates the whole tier enum and picks up a newly capped tier with no sweep change — so the engineering is trivial and the product call is not: a long-running session is a legitimate use of the product, and the threshold at which one counts as abandoned has real customer consequences.
- **Tier:** affects billing behaviour on paid tiers, so a reviewer places it; the code change it implies is a single table value. Recorded unplaced rather than guessed.
- **V-log:** the per-tier measurement and the liveness-signal analysis live with `an-unbounded-paid-session-is-a-visible-choice`. Recorded in the register by V-1520.
- **Where it is checked:** that guard holds the current posture visible — free capped, paid uncapped — and fails if any part of it changes, so approving a cap updates the record rather than diverging from it.

## D-2026-08-24-03 — Cross-SDK retry behaviour for a problem type the installed SDK predates (open)

- **Decision (OPEN — pending review):** whether the three SDKs should agree on retryability when a 5xx carries a problem type the installed SDK does not recognise. Today they do not: TypeScript retries it, Python and Go do not. Nothing is decided here; this entry gives the split a durable home and records where it is measured.
...
- **Decision (OPEN — pending review):** what should happen to an account's existing profiles when its tier drops below the number it holds. Today: nothing. Nothing is decided here; this entry records the question and the verified mechanism behind it.
- **Reasoning:** verified in source rather than restated. `profileLimitFor` has four call sites and every one is an acquisition path — `profiles.create`, restore ("Tier cap is shared with create — same enforcement path"), clone, and transfer against the _recipient's_ tier. Its definition comment says the cap is "enforced at the /v1/profiles creation gate". Neither `routes/profiles.ts` nor `routes/sessions.ts` consults it, so listing, loading, binding to a session and saving a retained profile are ungated. Of the ten sweepers in `services/`, none re-checks profile count on a tier change, and the crypto entitlement-expiry sweeper does not touch profiles.
- **The consequence is commercial, not a security hole.** An account that creates 500 profiles on `api_scale` and downgrades to `free` (cap 1) keeps all 500 and can use them exactly as before; only the 501st is refused. Subscribe for a month, create the profiles, downgrade, keep them.
- **The neighbouring caps are the control.** `maxSessionMinutes` has a create gate AND the session-duration sweeper, so a tier change reaches a session already running. `concurrentSessions` is create-only but drains on its own — sessions end, and a downgraded account cannot start more. `profiles` is the one that neither drains nor is swept.
- **What is undecided is the remedy, and each has a support story.** Deleting profiles on downgrade destroys data a customer may be paying to recover; freezing them, refusing to bind them to sessions, or granting a grace window are all defensible and all imply refund and support handling. Recorded unplaced by tier rather than guessed.
- **V-log:** measured by `a-numeric-tier-cap-that-only-guards-creation`, which derived the create-only property for every numeric cap rather than checking the one it suspected. V-1539 re-verified the call sites, the route-level absence and the sweeper set in source, and moved the open call here from that file's header.
- **Where it is checked:** that guard holds the current posture visible — `profiles` create-only, `maxSessionMinutes` create-plus-sweeper — and fails if a numeric cap gains or loses an enforcement site, so acting on this updates the record rather than diverging from it.
