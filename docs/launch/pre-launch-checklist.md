...
| Item                                       | Status          | Owner   | Blocks launch? | Notes                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------------ | --------------- | ------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Hetzner two-VM provisioning                | CONTRADICTED    | founder | yes            | V-869: this row says PENDING; `docs/progress/v278-final-state.md` records a LIVE sub-processor map with production CPX32 + staging CPX22 in Nuremberg NBG1, and the repo ships deploy tooling addressing both hosts. It also specs CCX13/CCX23 in Falkenstein, which is ADR-001's decision, not what that map records. Confirm which is authoritative |
| Cloudflare Pages 4 projects                | PENDING FOUNDER | founder | yes            | per V-259 runbook                                                                                                                                                                                                                                                                                                                                     |
| Neon EU databases                          | CONTRADICTED    | founder | yes            | V-869: same live sub-processor map records Neon Frankfurt eu-central-1 as in use (single database, prod + staging sharing `public` until the V-278.K split)                                                                                                                                                                                           |
| Upstash EU Redis                           | CONTRADICTED    | founder | yes            | V-869: `scripts/v278l-upstash-split-cutover.sh` SSH-swaps `REDIS_URL` on production, which presupposes a live Upstash database                                                                                                                                                                                                                        |
