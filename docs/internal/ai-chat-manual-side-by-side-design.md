...
**Date staged:** 2026-05-18. Slice 1 founder-verdict surface hoisted
2026-05-19; founder verdicts LOCKED 2026-05-19 (Wave 29-NNN ARC 3).

## ✅ Founder verdicts LOCKED 2026-05-19

| #   | Question                                 | Verdict | Implication                                                                                           |
| --- | ---------------------------------------- | ------- | ----------------------------------------------------------------------------------------------------- |
| 1   | Live transcript transport                | **A**   | SSE for transcript + POST `/v1/agent-sessions/:id/message` for customer input. Two channels.          |
| 2   | gui_control key minting policy           | **C**   | Per-session 24h-TTL auto-mint; stored server-side; never visible to customer.                         |
| 3   | `pair_mode_state` storage                | **A**   | New JSONB column on `agent_sessions.pair_mode_state` (migration TBD; auditable; survives Redis loss). |
| 4   | Per-session input-conflict lock location | **A**   | Redis-backed route-layer per-session lock (extends existing rate-limit Redis pattern, ~50 LOC).       |
| 5   | Per-turn cost surfacing in chat UI       | **A**   | Hide actual cost in customer-facing footer (consistent with bundled-LLM design v2-#6 verdict).        |

Slices 2-6 implementation UNBLOCKED. See "Engineering arc" section
below for the sequence. Bottom-of-doc "Founder verdicts needed" section
retained as the long-form decision-record context.
