...
The direct case (`https://10.0.0.5`, `https://localhost`, `https://[::1]` set
straight at create) is still allowed. Recommended layers:

1. **Create/update-time block (easy, zero legit false-positives):** reject when the
   URL host is `localhost`/`*.localhost` or a **literal IP** in a private/reserved
   range (IPv4: `127/8`, `10/8`, `172.16/12`, `192.168/16`, `169.254/16`, `0/8`;
   IPv6: `::1`, `fc00::/7` ULA, `fe80::/10` link-local, `::ffff:<private-v4>`). No
   legit webhook targets these. Use `net.isIP` + explicit, well-tested range
   predicates — get the boundaries exactly right (a bug here either misses a range or
   rejects a legitimate public IP → customer-facing breakage), which is why it's
   surfaced rather than shipped blind in a deep session.
2. **Connection-time resolution + pinning (DNS-rebind-complete):** a create-time
...
3. Reconcile the semi-blind oracle: the delivery-attempts log exposes per-attempt
   status/timing for a customer-controlled target — acceptable once (1)+(2) restrict
