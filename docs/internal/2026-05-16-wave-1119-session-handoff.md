...
| **OpenAPI union + invariant + post-deploy** | `725a1939` + `2048c45f` + `e15112a2`              | OpenAPI message-response as Zod discriminated union; cross-SDK BYOK invariant 7→8 cases (covers OpenAPI doc); `scripts/post-deploy-verify.mjs` 10→13 invariants (activation-gate stub checks)                                                   |
| **V-820 /v1/fleet/events route stub**       | `ae670c80`                                        | 5th activation-gate-pattern feature. Routes/fleet-events.ts wired+disabled pair (both 503 — WebSocket handler pending); cross-source invariant 21→26 cases; post-deploy invariants 13→14; Agent 1 V-820.B.1.b dep can wire against this URL now |
| **V-820 OpenAPI + contract refresh**        | this wave                                         | OpenAPI spec for /v1/fleet/events (operator-only fleet tag; 101+503); cross-agent-control-plane-contract.md updated for the route-stub status; activation-gate-pattern memory entry refreshed (4→5 features + runtime invariant section)        |
