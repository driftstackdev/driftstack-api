...
Session continuation after context compaction. **60 commits** landed
...
per-customer key-storage Tier-2 design.
...
| **AppDeps + cross-SDK invariant**   | this wave                                         | `AppDeps.fleetNodeAuth?` + `fleetNonceCache?` declared for future bootstrap wiring; cross-source invariant test pinning the `x-byok-anthropic-api-key` header name + opt-name convention across all 3 SDKs |
