...
121 surface URL, extracting the fingerprint value — lives on Agent 2's
side in the webkit-driftstack fork. This repo provides the
**orchestration service Agent 2 calls into**, never the capture code
itself.
...
| ----- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1     | `driftstack-api` (this repo)      | Orchestration: `RecaptureService` (V-179) + matrix runner (V-533.A) + atlas builder (V-533.B). Maintains per-run state, dedups comparisons, aggregates atlases for admin-panel display.                                                                 |
| 2     | `webkit-driftstack` (sister repo) | Fork-side capture worker: consumes queued runs from `RecaptureService.listRuns({status: 'queued'})`, walks file-121 surfaces against each archetype, calls `RecaptureService.recordComparison()` for each surface, calls `finalizeRun()` on completion. |

Per Rule G, this repo (Agent 1) does NOT touch the webkit-driftstack
repo. Agent 2 picks up the worker implementation in coordination with
this contract.
...
```
              ...
              │                                                       │
   (2) Agent 2 worker polls listRuns({status: 'queued'}) periodically │
              │                                                       │
              ↓                                                       │
   ┌──────────────────────────────┐                                   │
   │ Agent 2: webkit-driftstack   │                                   │
   │                              │                                   │
   │  for each queued run:        │                                   │
   │    open WKWebView            │                                   │
   │    for each surface in       │                                   │
   │      file-121 catalogue:     │                                   │
   │      capture value           │                                   │
   │      → recordComparison()    │ ─── HTTP/IPC ──→ Agent 1          │
   │    finalizeRun(completed)    │ ─── HTTP/IPC ──→ Agent 1          │
   └──────────────────────────────┘                                   │
                                                                      ↓
...
transport between Agent 2's fork worker and Agent 1's control-plane
...
Agent 2's worker stays a manual operator process. Both layers will swap
...
After fan-out, the N runs queue in status `'queued'`. Agent 2's worker
consumes them.
...
- **V-533.C (later):** admin routes — `/v1/admin/recapture/runs` GET +
  ...
  transport between Agent 2 worker + Agent 1 service.

## Change-management protocol

ANY change to the orchestration surface that affects how Agent 2's
...
The matrix runner + dedup are internal to this repo — Agent 2 never
