...
WebKit-side implementation is Agent 2's scope.
...
| ----- | --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1     | `driftstack-api` (this repo)      | Pipeline consumer side: `FrameSource` interface declaration + `MockFrameSource` for solo testing + `EncodePipeline` that consumes frames + emits WebRTC-ready chunks.                                                 |
| 2     | `webkit-driftstack` (sister repo) | Pipeline producer side: real `WkWebViewFrameSource` implementing the `FrameSource` interface contract, sourcing frames from `WKWebView` surface capture, shipping them across an IPC boundary into the control plane. |

Per Rule G, this repo (Agent 1) does NOT touch the webkit-driftstack repo.
Agent 2 picks up the WebKit-side implementation in coordination with this
contract.
...
the encode pipeline tests against. Agent 2's `WkWebViewFrameSource` must
...
Frames cross the boundary. Proposed envelope (Agent 2 confirms in their
implementation wave):
...
| 8..11  | timestampMicros | u32 low bits — combine with `timestampMicrosHi` in next frame OR widen to u64 (TBD with Agent 2) |
| 12..13 | width           | u16                                                                                              |
| 14..15 | height          | u16                                                                                              |
...
TBD with Agent 2 — the envelope is small enough that explicit per-frame
...
1. **Wave 17 (this wave):** Agent 1 ships the interface, mock, and
   pipeline (this directory). Pipeline tests pass against the mock.
   Cross-agent contract document published (this file).
2. **Wave 18 onward:** Agent 2 implements `WkWebViewFrameSource` in
   ...
   of the bridge.
3. **Integration wave (Agent 1 — TBD):** Agent 1 wires the real
...
If Agent 2 needs an IPC envelope tweak that doesn't change the language-
level interface, no Agent 1 work is needed — the envelope is hidden behind
...
- This document committed to `docs/internal/v531-cross-agent-contract.md`
  so Agent 2 can pick up the WebKit-side work next.
