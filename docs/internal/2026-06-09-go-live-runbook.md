...
| `DB_STATEMENT_TIMEOUT_MS`                                   | recommended (e.g. `30000`)  | Per-connection Postgres `statement_timeout` — kills a runaway query before it saturates the pool. Wired but OFF by default (config.ts:561 → bootstrap.ts:194); set it at go-live. Safe (legit queries are sub-second). |
