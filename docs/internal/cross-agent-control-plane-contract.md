...
| Nonce cache (Redis)                | Agent 2           | Pending; trivial extension once repo lands.                                                                         |
| `/v1/fleet/events` WebSocket route | Agent 2           | Pending; uses `FleetNodeAuth` at handshake.                                                                         |
...
| BYOK key path                 | Agent 2 | Pending — read customer's Anthropic key from a per-account stored secret (existing `accounts.anthropic_api_key_hash` pattern from V-204b or equivalent); plain text passed to runtime at request time, never logged. |
