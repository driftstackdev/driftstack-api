...
## V-2165 — the Simulator's file download could never have worked: `fs:allow-open` was granted nowhere (2026-08-30)

Investigating the owner's "the separate app doesn't work on the Windows build", the Windows-specific
hunt turned up something bigger and platform-INDEPENDENT.

**The chain, read from source rather than inferred.** `SimulatorWindow.tsx:85` imports
`downloadResponse`; `download.ts:126` calls `open()` from `@tauri-apps/plugin-fs`, which invokes
`plugin:fs|open`; that command requires `fs:allow-open` (tauri-plugin-fs 2.5.1
`permissions/autogenerated/commands/open.toml`). **No capability in the app grants it** — the only
`allow-open` anywhere is `shell:allow-open`, a different plugin. `downloadResponse`'s catch returns
`false` with no fallback, so downloading a file the device produced fails on macOS, Windows and Linux
alike, and the caller reports failure.

**And a second, platform-SPECIFIC gap underneath it.** `FileHandle extends Resource`
(plugin-fs dist-js) and `Resource.close()` invokes `plugin:resources|close`
(`@tauri-apps/api/core.js:272`). The macOS separate-app window (`sim-*`) holds
`core:resources:default`; the in-process window (`simulator-*`, i.e. the Windows/Linux path) did not.
So even once `open` is granted, closing the handle would fail only on Windows/Linux — and on Windows
an unclosed handle leaves the downloaded file locked.

**Why every test was green.** The download suites mock `@tauri-apps/plugin-fs`, so the mock answers
and the grant is never consulted — a faithful double hiding the real artifact. And an ungranted
permission does not warn: the IPC call simply rejects. This file's own header, written for the
`updater-check-macos` incident, says exactly that; the same silence hid a second bug in the same app.

**Fixed:** `fs:allow-open` added to BOTH simulator capabilities (still bounded by the existing
`fs:scope` to `$APPDATA/recordings/**` and `$DOWNLOAD/**` — the grant does not widen the reachable
paths), and `core:resources:default` added to the in-process window to match its sibling.

**Two new arms, both derived rather than pinned.** The first reads the fs functions the
simulator-reachable modules actually import and requires the matching `fs:allow-*` in both
capabilities — it fails on a NEW fs call that nobody granted, not merely on today's. The second
requires the two windows' permission sets to differ only by a stated, explained set (`core:default`,
`core:app:default`, each with why), so a silent divergence like the missing `core:resources` cannot
recur. Both carry a non-vacuity floor, since each enumerates its own inputs and would otherwise go
green when its scan breaks.

**Proofs.** 4/4 in the guard file. Mutations, snapshot-restored and cmp-verified: removing
`fs:allow-open` (the state shipped until today) → the first arm red naming the file; removing
`core:resources:default` from the in-process window (also the shipped state) → the second arm red.

**Boundary — this is NOT a claim that the Windows simulator now works.** It removes two real
blockers that were provable statically, both of which would break downloads there. I have no Windows
...
## V-2236 — sdk: TypeScript 0.3.0, Python 0.3.0, Go v0.4.0 and API types 0.2.0 released, verified from the registries (2026-09-22)

Commit `f0d4a071a` on origin/main, CI-green on all five jobs (build/typecheck/lint/unit+integration, end-to-end, Python SDK, Go SDK, perf advisory); the four annotated tags `packages/sdk-typescript/v0.3.0`, `packages/sdk-python/v0.3.0`, `packages/sdk-go/v0.4.0` and `packages/api-types/v0.2.0` name that commit. What the release carries: `egress_capabilities.safeguards`, `measured_by`, `direct_reading` / `website_like_reading`, the `page_unreadable` notice reason, and the TypeScript-only typed accessor on the concurrency error; two closed-string fields on the proxy test result are narrowed in the types with a migration note in every CHANGELOG (nothing removed at runtime: the coordinator diffed the published 0.2.0 declaration file and wheel against HEAD before choosing the minors). Gate/evidence: clean-clone gate `verify-suite: OK` 3787 files / 39,851 tests and Playwright green on `f0d4a071a`, after a first attempt at `49fd827d1` failed every CI workflow at `npm ci` because two workspaces pinned `@driftstack/api-types` to the 0.1 line — the gate clone never runs a clean install, so it could not see it; `f0d4a071a` widens both to the workspace link and the clone was re-gated after `npm ci`. api-types was published in the withheld shape (`build:publish` removed the seven pricing-module files; the packed tarball has 101 files and zero mentions of the unreleased feature, checked in the extracted `index.d.ts`, `index.js` and `problem.d.ts`). Registry checks that actually ran, each outside the repo: npm `@driftstack/sdk@0.3.0` (dist-tag latest) packs to `dist/` (js, cjs, d.ts, d.cts, maps), README, CHANGELOG, LICENSE and package.json and nothing else, installs as the SDK plus zod only, and `require` and `import` both load on Node 18, 20 and 22; npm `@driftstack/api-types@0.2.0` serves 101 files; PyPI `driftstack-sdk==0.3.0` installs into a clean venv, imports `Driftstack` and `AsyncDriftstack` and reports `__version__` 0.3.0, and the wheel carries `py.typed` and the licence; the Go proxy lists `v0.4.0` and `go get …/packages/sdk-go@v0.4.0` resolves it in a fresh module with no replace directive. One GitHub release per package with the CHANGELOG entry, the install line, the migration note and what is not in the release. Discrepancy found and corrected in the same minute: `gh release create` marks the newest release Latest by default, which moved the repository's `releases/latest` pointer off the desktop client for about forty seconds and made the desktop updater's `latest.json` unreachable; restored with `gh release edit gui-v0.1.70 --latest` and verified by the redirect and by parsing latest.json; the runbook now says `--latest=false` for every non-desktop release.

## V-2235 — sdk: TypeScript 0.2.0, Python 0.2.0 and Go v0.3.0 released, verified from the registries (2026-09-20)

Commits `4bb530539` (Go tag) and `a9120f1e1` (npm and PyPI; `packages/` identical between the two) on origin/main, both CI-green on all four jobs. The first SDK release since 2026-05-03: the published 0.1.6 / 0.1.5 / v0.1.6 had no AI-session surface at all. Gate/evidence: clean-clone gate `verify-suite: OK` 3718 files / 38,894 tests and 233 Playwright tests on `a9120f1e1`. Registry checks that actually ran, each in an empty project outside the repo: npm `@driftstack/sdk@0.2.0` (dist-tag latest) installs as the SDK plus zod and nothing else; `require` and `import` both load on Node 18.20.8, 20.20.2 and 22; a strict TypeScript consumer compiles under node16 and bundler resolution with skipLibCheck off; the tarball is 10 files with no test, source or env file. PyPI `driftstack-sdk==0.2.0` (wheel and sdist passed twine check) installs into a clean venv, the agent-session methods are present and py.typed ships. Go `packages/sdk-go/v0.3.0` resolves from proxy.golang.org in a module with no replace directive and reports version 0.3.0. Not run: the guide's three complete programs from the installed packages against a live account. Not published: `@driftstack/api-types`, which still carries an unreleased field; the TypeScript SDK bundles what it needs and no longer depends on it. GitHub releases for the three SDKs were created as not-latest, because the desktop updater reads the repository's latest release; `releases/latest` still names the desktop client afterwards.
