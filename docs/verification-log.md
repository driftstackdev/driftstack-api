...
## V-197 — Stripe webhook signature reference-vector tests + ops procedures (PRIORITY D phase 3)

### What

Closes the V-166 carry-forward in two complementary slices:

1. **Cross-implementation reference-vector unit tests** — `apps/server/tests/unit/stripe-signing-reference-vectors.test.ts`. Three HMAC-SHA256 fixtures pre-computed via `openssl dgst -sha256 -hmac` (independent of our `stripe-signing.ts` impl). For each fixture: verifier accepts the externally-signed payload, rejects single-char hex mutations, rejects with wrong secret. Plus tolerance-window edge tests (rejected at 360s, accepted at exactly 300s) and a forward-compat check (unknown `v2=...` keys ignored without breaking v1 validation).
2. **Operational procedures doc** — `docs/deployment/stripe-webhook-testing.md`. Local `stripe listen` workflow, staging cutover, production cutover sequence, replay procedures, signing-secret rotation. Cross-references the existing tests and notes what's already automated vs what requires manual ops verification.

### Why

The pre-V-197 test suite covered Stripe webhooks via sign-verify roundtrips (`signStripePayload` → `verifyStripeSignature`) — those would pass even if both functions had matching algorithm bugs. Reference vectors generated by an independent implementation (openssl) close that hole: if our impl ever drifts from Stripe's algorithm, the fixtures fail loud. The ops doc closes the OTHER gap — actual wire-delivery from Stripe to our endpoint can't be tested in CI; staff need a runbook for it.

### Files

- `apps/server/tests/unit/stripe-signing-reference-vectors.test.ts` — new file (~125 lines). 3 vectors × 3 cases each + 3 standalone (tolerance, edge, forward-compat) = 12 new tests.
- `docs/deployment/stripe-webhook-testing.md` — new ops doc (~150 lines).

### Verify

- `npm run typecheck`: clean.
- `npm run lint`: clean.
- `npm run format:check`: clean (after prettier --write on docs file).
- `npm test`: 652 / 652 passing across 67 files (12 new reference-vector tests pass).

### Notes

- Vectors regen: `echo -n "<timestamp>.<body>" | openssl dgst -sha256 -hmac "<secret>" -hex` — documented in the test file header comment so future engineers can extend without trial-and-error.
- `signStripePayload` is the only "sign" path in our codebase and it's only used in tests. Production never signs Stripe-ward payloads — Stripe signs, we verify. So the unit tests for the signing helper are not a separate gap.
- Ops doc covers the pre-launch state explicitly: live-mode webhook endpoints aren't created yet (gated on commercial activation per the locked publish-vs-commercial memory). The doc walks through "what to do when that gate opens" rather than describing current-state production wiring.
- The doc references a hypothetical `/v1/admin/audit-log?action=stripe_webhook.processed` query path — the audit row for processed webhooks doesn't exist today (the `processed_stripe_events` table is the source of truth, not the admin audit log). Flagged in the doc as "when this audit action exists" — minor follow-up if staff want webhook activity to surface in the admin audit feed.
...
## V-208 — customer-facing AI-built framing audit (clean)
...
Per the V-205 policy carve-out:
