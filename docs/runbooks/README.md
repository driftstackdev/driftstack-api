...
| [`agent-turn-monitoring.md`](agent-turn-monitoring.md)                               | AI turns failing, slow, or answering 409: the admin "AI turns" page, the `driftstack_agent_turn_*` series, and the four alert expressions.                                                                                                                                       |
...
| [`sdk-release.md`](sdk-release.md)                                                   | Publishing the TypeScript, Python and Go SDKs. The npm SDK carries its own contract types now, so it installs on its own; api-types still publishes first, for the people who depend on it directly. npm and PyPI versions are permanent, so every check runs before the upload. |
