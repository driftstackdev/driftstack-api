...
describe('W295.A customer-dashboard internal href integrity', () => {
  it('every <a href="/<path>"> (with no query/hash stripped) resolves to a real page', () => {
    ...
    for (const f of allAstro) {
      const body = read(f);
      const matches = [
        ...body.matchAll(/<a[^>]+href=["'](\/[a-z][a-z0-9/_-]*(?:\/[a-z0-9_-]+)*)["']/g),
      ];
      for (const m of matches) {
        const href = m[1]!.replace(/#.*$/, '').replace(/\?.*$/, '').replace(/\/$/, '') || '/';
        // Skip external-looking heuristics (rooted at /docs/ or /v1/)
        // — those are out-of-app links.
        if (href.startsWith('/v1/')) continue;
