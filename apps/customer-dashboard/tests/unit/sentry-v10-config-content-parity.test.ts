...
describe('customer dashboard Sentry v10 configuration', () => {
  ...
  it('keeps build-time controls in the Astro integration', () => {
    ...
    expect(astro).toMatch(/project:\s*'driftstack-dashboard'/);
  ...
  it('keeps client and server runtime settings identical', () => {
    for (const config of [client, server]) {
      ...
      expect(config).toMatch(/dsn:\s*import\.meta\.env\.PUBLIC_SENTRY_DSN_DASHBOARD/);
