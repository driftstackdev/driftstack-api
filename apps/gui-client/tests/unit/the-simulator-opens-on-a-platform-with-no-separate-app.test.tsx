...
describe('the simulator opens on a platform with no separate app', () => {
  ...
  it('T-22 (owner #8) the in-process window claims its OWN Windows taskbar identity AFTER it is created — invoke(claim_simulator_taskbar_identity) targeting THIS window’s label. Windows groups taskbar buttons by AppUserModelID, so without a distinct one the in-process simulator shares the main app’s process/AUMID and its button is grouped under the main app’s, behind the main icon — the exact "grouped together, not its own icon" the owner reported.', async () => {
    platformReports(false);
    const res = await openSimulatorWindow({ sessionId: 'agt_taskbar', info });
    expect(res.opened, `the in-process path failed: ${res.reason ?? ''}`).toBe(true);

    const claimIndex = invoke.mock.calls.findIndex(
      (c) => c[0] === 'claim_simulator_taskbar_identity',
    );
    expect(
      claimIndex,
      'the in-process window never claimed a distinct taskbar identity',
    ).toBeGreaterThanOrEqual(0);

    // It must target THIS session's window by label — a wrong or absent label
    // would rebrand nothing (or the wrong window, e.g. the main one, undoing the
    // main app's own grouping).
    const claimArgs = invoke.mock.calls[claimIndex]?.[1] as { label?: string } | undefined;
    expect(claimArgs?.label, 'the claim did not target this session window').toBe(
      'simulator-agt_taskbar',
    );

    // AFTER creation, never before: the AUMID is set on a window that must exist
    // first. invocationCallOrder is one global monotonic counter shared across
    // vitest mocks, so the ctor and the claim invoke compare directly.
    const ctorOrder = WebviewWindowCtor.mock.invocationCallOrder[0] ?? 0;
    const claimOrder = invoke.mock.invocationCallOrder[claimIndex] ?? 0;
    expect(ctorOrder, 'the in-process window was never constructed').toBeGreaterThan(0);
    expect(
      claimOrder,
      'the taskbar identity was claimed before the window existed',
    ).toBeGreaterThan(ctorOrder);
  });

  it('VACUITY CONTROL — T-22 the macOS SEPARATE-APP path does NOT claim a Windows taskbar identity. macOS already gets its own Dock icon from the separate bundle and the AUMID split is a Windows-only concern; a build that fired the claim unconditionally (making the arm above pass for the wrong reason) trips here, and so would one that ran it on the macOS path.', async () => {
    platformReports(true);
    const res = await openSimulatorWindow({ sessionId: 'agt_mac_ident', info });
    expect(res.opened, `the macOS path failed: ${res.reason ?? ''}`).toBe(true);
    expect(
      invoke.mock.calls.filter((c) => c[0] === 'claim_simulator_taskbar_identity').length,
      'the macOS separate-app path claimed a Windows taskbar identity it must not',
    ).toBe(0);
    // Only meaningful on the path that does NOT build an in-process window: the
    // separate app is what launched here.
    expect(launchCalls().length, 'the separate-app path was not taken').toBe(1);
  });
});
