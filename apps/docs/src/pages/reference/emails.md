...
| **Someone is trying to sign in to your Driftstack account**   | Someone got past the first step of signing in — with the account's password, a sign-in link sent to the address, or a linked Google or GitHub account — then entered the wrong two-factor code 10 times in 15 minutes. Two-factor sign-in that way is paused for 15 minutes; the other ways in still work. Sent once per pause. It names the way that was used and what to do if it wasn't you. |
| **The password on your Driftstack account was removed**       | A sign-in link was the first confirmation of the account's email address, and the account had a password — set before the address was confirmed — so the password was removed. Sent once. Set a new one with a password reset.                                                                                                                                                                  |
| **Two-factor sign-in is now on for your Driftstack account**  | Two-factor sign-in was turned on for the account. Sent every time, so you hear about it if it wasn't you.                                                                                                                                                                                                                                                                                       |
| **A sign-in method was removed from your Driftstack account** | A linked Google or GitHub sign-in was removed from the account's security page. If it wasn't you, reset your password.                                                                                                                                                                                                                                                                          |
...
| **Your webhook secret was auto-rotated for security** | NOT CURRENTLY SENT — automatic rotation is not enabled on this deployment, so no secret is rotated at the 91-day mark and this email cannot arrive. Rotate on your own cadence via `POST /v1/webhooks/{id}/rotate-secret`. If automatic rotation is turned on, this email carries the 7-day dual-signing grace deadline for rolling out the new secret. |
| **Webhook secret grace window expires in 24h**        | Sent inside the final ~24 hours of a rotation grace window if your verifier hasn't picked up the new secret yet. A grace window opens whenever a secret is rotated — today that means a rotation you request, since automatic rotation is not enabled.                                                                                                  |
