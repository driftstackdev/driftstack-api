...
| `driftstack_agent_tap_unoccluded_check_total`          | `why`, `result`                    | taps sent with the browser checking, at the moment of tapping, that the tap point is on the control. `why`: outside_viewport (the check before the tap could not see it) / consequential (a purchase, payment or deletion the customer approved). `result`: tapped / a refusal — hit_is_not_target_or_descendant, covered_at_enclosing_shadow_level, nothing_hit, tap_point_outside_viewport (something else is there; nothing tapped), target_not_resolved (the control went away; nothing tapped), occlusion_check_unavailable (the check could not run, so nothing was tapped — the fail-closed rate) / unrecognised_reason / failed_otherwise / no_answer. A browser too old to check taps without checking and counts as tapped                                                                                                                                                               |
