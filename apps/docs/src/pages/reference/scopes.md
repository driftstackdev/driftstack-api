...
| `read:webhooks`             | granular        | Read webhook endpoints **and their delivery history** — `GET /v1/webhooks`, `GET /v1/webhooks/:id`, and `GET /v1/webhooks/:id/deliveries`. The delivery rows carry `last_response_status`, `last_response_excerpt` and `last_error` from your own endpoint, so this scope sees more than the endpoint list.                                     |
