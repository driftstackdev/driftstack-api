...
- **Member account.** A separate account (own login, own email)
  joined to the owner's team. Deleting either account removes the
  membership.
- **Invite.** A pending invitation created by the owner and accepted
  by the invitee. Expires after 7 days.
...
- `/v1/agent-sessions` (GET collection / POST create / `:id`
  ...
  owner (counts against the owner's cap and uses the owner's profile
  data). `member` role gets `403` on this whole surface.
...
under the same owner is not yet possible; creating teams will arrive
together with support for more than one team per owner.
