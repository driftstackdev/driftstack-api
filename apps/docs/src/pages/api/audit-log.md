...
| `proxy.created`                       | customer | Saved-proxy minted (egress config). Payload: `{ proxy_id, label, type }` where `type` ∈ {`socks5`, `openvpn`, `wireguard`}. NEVER carries secret material (password / private key / .ovpn config).                                                                                                           |
| `proxy.deleted`                       | customer | Saved-proxy deleted. Payload: `{ proxy_id, label, type }`.                                                                                                                                                                                                                                                   |
