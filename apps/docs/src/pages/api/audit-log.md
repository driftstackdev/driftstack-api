...
| Action                                     | Origin             | Notes                                                                                                                                                                                                                                                                                           |
| ------------------------------------------ | ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `account.email_verified`                   | customer           | The address was proven: the verify-email link, a magic link that was the account's first proof (`payload.via` is `magic_link`; `payload.password_removed` says whether a password set before the proof was dropped), or a completed password reset (`payload.via` is `password_reset`)          |
...
| `account.mfa_sign_in_locked`               | system             | Ten wrong two-factor codes in fifteen minutes, so two-factor sign-in is paused for fifteen minutes. Once per pause; the account is emailed too. `payload.failed_codes`, `payload.paused_until`                                                                                                  |
...
{ "failed_codes": 10, "paused_until": "<ISO 8601>" }
