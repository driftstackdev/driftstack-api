...
| Action                                     | Origin             | Notes                                                                                                                                                                                                                                                                                           |
| ------------------------------------------ | ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `account.email_verified`                   | customer           | The address was proven: the verify-email link, a magic link that was the account's first proof (`payload.via` is `magic_link`; `payload.password_removed: true` when it dropped a password set before it), or a completed password reset (`payload.via` is `password_reset`)                    |
| `account.login`                            | customer           | Successful sign-in. `payload.method` ∈ {`password`, `magic_link`, `mfa_totp`, `mfa_recovery`}. OAuth sign-ins land a variant payload `{ kind: "oauth_callback", provider, session_id }` with no `method` field. A completed password reset emits `account.password_changed` instead.            |
| `account.logout`                           | customer or staff  | Web session revoked. Recorded as `staff` when Driftstack staff terminated the account (`payload.revoked_via` is `admin_account_deletion`).                                                                                                                                                      |
...
| `account.mfa_sign_in_locked`               | system             | Ten wrong two-factor codes in fifteen minutes by one way in (`payload.method`: `password`, `email_link`, `google`, `github`), so that way is paused for fifteen minutes; the others still work. Once per pause per way; the account is emailed. `payload.failed_codes`, `payload.paused_until`  |
| `account.mfa_enrollment_password_rejected` | customer           | A wrong current password while turning on two-factor. Five in fifteen minutes and the step is refused for a while                                                                                                                                                                               |
| `account.oauth_link_removed`               | customer           | A linked Google/GitHub sign-in was removed via `DELETE /v1/account/me/oauth-links/:id`. `target_resource_id` is the link's `ol_` id; `payload.provider`                                                                                                                                         |
| `api_key.minted`                           | customer           | POST /v1/api-keys                                                                                                                                                                                                                                                                               |
| `api_key.rotated`                          | customer           | POST /v1/api-keys/:id/rotate . 24h grace                                                                                                                                                                                                                                                        |
| `api_key.revoked`                          | customer or staff  | DELETE /v1/api-keys/:id. Also written for each desktop-app sign-in a password reset ends (`payload.revoked_via` is `password_reset`), and as `staff` when Driftstack staff revoke a key or terminate the account that created it.                                                               |
...
| `webhook_endpoint.deleted`                 | customer or staff  | DELETE /v1/webhooks/:id. Recorded as `staff` when Driftstack staff terminated the account.                                                                                                                                                                                                      |
...
| `account.ai_source_changed`                | customer           | Customer changed which source funds their AI agent turns — their own API key vs. Driftstack-provided. Payload: `{ from, to }`.                                                                                                                                                                  |
...
GET /v1/account/audit-log?action=account.mfa_sign_in_locked
```
...
```json
// account.login — password / magic-link / MFA sign-ins
{ "method": "password" | "magic_link" | "mfa_totp" | "mfa_recovery" }
...
{ "failed_codes": 10, "paused_until": "<ISO 8601>", "method": "password" | "email_link" | "google" | "github" }
