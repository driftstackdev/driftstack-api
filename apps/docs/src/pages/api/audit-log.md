...
| `profile.created`                 | customer | POST /v1/profiles, /clone (V-313 — `payload.cloned_from: "profile_<uuid>"`), or /v1/profile-snapshots/:id/restore (V-312 — `payload.restored_from_snapshot: "psnap_<uuid>"`). Pre-existing format asymmetry: cloned*from uses an internal `profile*`prefix; restored_from_snapshot uses the public`psnap\_` prefix. |
