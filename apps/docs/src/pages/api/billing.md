...
portal-session, billing-portal) do not act in another member's
workspace: the Stripe portal endpoints refuse a request whose
`X-Driftstack-Account` names another account with `400`, so a member
acting as the owner is never sent to their own payment details by
mistake. Only the owner manages the owner's billing.
