...
- `session.profile_save_failed` — a profile-backed session did not replace the stored profile at teardown. Failure reasons are terminal and the next restore will be stale; `superseded` is harmless and means a newer save of the same profile landed first, and `profile_not_loaded` means the session started without its saved profile, so its changes were not saved over it and the saved profile is kept as it was.
