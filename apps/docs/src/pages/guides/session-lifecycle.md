...
- `session.profile_save_failed` — a profile-backed session could not persist its updated profile store at teardown (the session itself succeeded; terminal — the next restore of that profile will be stale).
