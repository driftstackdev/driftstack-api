...
describe('V-666.V GET /v1/admin/crypto-orders.csv', () => {
  ...
  it('200 text/csv with header row and one row per order, newest-first', async () => {
    ...
    expect(lines[0]).toBe(
      'order_id,account_id,product,price_cents,price_currency,status,payment_id,customer_note,refund_requested_at,refund_reason,internal_note,created_at,updated_at',
    );
