...
describe('Free desktop credential — fail-closed route surface', () => {
  it('every allowlisted template remains disjoint from the independent deny-set', () => {
    // 67: 61 since P-17 added POST /v1/agent-sessions/:id/egress, +5 from the GUI
    // audit (resume; stop + captures; the proxy list + PATCH /v1/account/me), +1
    // for DELETE /v1/recipes/:id (a Free user can remove a recipe it saved).
