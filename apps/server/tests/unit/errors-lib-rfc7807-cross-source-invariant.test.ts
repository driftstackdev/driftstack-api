...
describe('W965 errors lib RFC 7807 cross-source invariant', () => {
  ...
  it('CRITICAL errors.ts declares EXACTLY 30 ApiError subclasses — covers BadRequest + Validation + auth (5x 401) + Forbidden + NotFound + Conflict + 2x rate-limit + Tier + 16 domain-specific errors + Internal. Q.1.d added ByokAnthropicRequiredError (2026-05-17); subsequent additions through 2026-05-20 (egress + agent + activation-gate family) grew the taxonomy to 29 subclasses + 1 base.', () => {
    const p = read(resolve(REPO_ROOT, 'apps/server/src/lib/errors.ts'));
    const classes = p.match(/export class \w+Error extends ApiError/g) ?? [];
    expect(classes.length).toBe(29); // 29 subclasses, plus ApiError base = 30 total
  });
