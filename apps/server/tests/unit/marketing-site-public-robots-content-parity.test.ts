...
//
//   • driftstack.io/robots.txt canonical URL doc-comment.
//   • Subdomain-isolation framing: app.driftstack.io + admin.driftstack.io
...
//   • Sitemap: https://driftstack.io/sitemap-index.xml reference.
...
describe('W524.B apps/marketing-site/public/robots.txt content parity', () => {
  ...
  it("Canonical robots.txt URL + subdomain-isolation framing pinned: '# https://driftstack.io/robots.txt' + 'All public pages crawlable. Subdomains (app.driftstack.io for the customer dashboard, admin.driftstack.io for the customer dashboard, admin.driftstack.io for the admin panel) carry their own robots — both noindex.' — pinned so the canonical-marketing-site-robots + 2-subdomain-isolation (app + admin) + both-noindex commitment survives (drift to indexing app/admin subdomains would expose customer-only routes to search engines)", () => {
    expect(body).toMatch(/# https:\/\/driftstack\.io\/robots\.txt/);
    expect(body).toMatch(
      /# All public pages crawlable\. Subdomains \(app\.driftstack\.io for the\s*# customer dashboard, admin\.driftstack\.io for the admin panel\) carry\s*# their own robots — both noindex\./,
    );
  });
  ...
  it("Sitemap reference framing pinned: 'Sitemap: https://driftstack.io/sitemap-index.xml' — pinned so the sitemap-index pointer at the canonical marketing-domain commitment survives (drift to dropping the sitemap reference would break crawler discovery of indexed pages)", () => {
    expect(body).toMatch(/^Sitemap: https:\/\/driftstack\.io\/sitemap-index\.xml$/m);
  });
