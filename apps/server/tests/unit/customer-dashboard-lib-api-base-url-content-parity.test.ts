...
describe('W462.B apps/customer-dashboard/src/lib/api-base-url.ts content parity', () => {
  ...
  it("W192 framing pinned: 'W192 — single source of truth for the customer-dashboard's API base URL. Mirrors the server-side V-079.B / W190 pattern: one helper, one rule, prod build refuses to ship without the env var set.'", () => {
    expect(body).toMatch(
      /\/\/ W192 — single source of truth for the customer-dashboard's API base\s*\n?\s*\/\/ URL\. Mirrors the server-side V-079\.B \/ W190 pattern: one helper, one\s*\n?\s*\/\/ rule, prod build refuses to ship without the env var set\./,
    );
  });
  ...
  it("Before-state framing pinned: '20 .astro files as the inline expression `const apiBaseUrl = import.meta.env.PUBLIC_API_BASE_URL ?? 'http://localhost:3000';` which made the localhost fallback silent in prod — a single missed env var on deploy would point every customer browser at http://localhost:3000 and the dashboard would 404 every API call.'", () => {
    expect(body).toMatch(
      /\/\/ Before this lived in 20 \.astro files as the inline expression\s*\n?\s*\/\/\s*`const apiBaseUrl = import\.meta\.env\.PUBLIC_API_BASE_URL \?\? 'http:\/\/localhost:3000';`\s*\n?\s*\/\/ which made the localhost fallback silent in prod — a single missed\s*\n?\s*\/\/ env var on deploy would point every customer browser at\s*\n?\s*\/\/ `http:\/\/localhost:3000` and the dashboard would 404 every API call\./,
    );
  });
  ...
  it("Helper-contract 3-bullet framing pinned: returns PUBLIC_API_BASE_URL with trailing slash stripped + dev fallback to localhost:3000 'to keep npm run dev zero-config' + prod throws at evaluation time 'fails the static-prerender pass during build — deploys don't ship a broken bundle.'", () => {
    expect(body).toMatch(
      /\/\/ The helper:\s*\n?\s*\/\/\s+- returns `PUBLIC_API_BASE_URL` when set \(trailing slash stripped\)\s*\n?\s*\/\/\s+- in dev mode \(`import\.meta\.env\.DEV`\) falls back to\s*\n?\s*\/\/\s+`http:\/\/localhost:3000` to keep `npm run dev` zero-config\s*\n?\s*\/\/\s+- in prod mode throws at evaluation time when the env var is\s*\n?\s*\/\/\s+unset, which fails the static-prerender pass during build —\s*\n?\s*\/\/\s+deploys don't ship a broken bundle\./,
    );
  });
