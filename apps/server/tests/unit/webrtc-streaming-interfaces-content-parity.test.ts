...
describe('W457.C packages/webrtc-streaming/src/interfaces.ts content parity', () => {
  ...
  it("V-149 framing pinned: 'V-149 — WebRTC streaming interfaces.' + 'Phase 3+ work fills these in with browser-side WebRTC + server-side signaling. Today the seam exists so consumers (GUI client LiveSessionView, future customer-dashboard live preview) can integrate against the contract.'", () => {
    ...
    expect(body).toMatch(
      /\/\/ Phase 3\+ work fills these in with browser-side WebRTC \+ server-\s*\n?\s*\/\/ side signaling\. Today the seam exists so consumers \(GUI client\s*\n?\s*\/\/ LiveSessionView, future customer-dashboard live preview\) can\s*\n?\s*\/\/ integrate against the contract\./,
    );
  });
  ...
  it("CreateStreamOpts: 2-field (sessionId + config); CreateStreamResult: 2-field (streamId + offer 'Server-side SDP offer. Caller's WebRTC peer answers via negotiate.')", () => {
    expect(body).toMatch(
      /\/\*\* Options for creating a new stream against a session\. \*\/\s*\n?\s*export interface CreateStreamOpts \{[\s\S]*?sessionId: string;[\s\S]*?config: StreamConfig;\s*\n?\s*\}/,
    );
    expect(body).toMatch(
      /\/\*\* Result of CreateStream — caller exchanges SDP via the same StreamId\. \*\/\s*\n?\s*export interface CreateStreamResult \{\s*\n?\s*streamId: StreamId;\s*\n?\s*\/\*\* Server-side SDP offer\. Caller's WebRTC peer answers via `negotiate`\. \*\/\s*\n?\s*offer: SdpPayload;\s*\n?\s*\}/,
    );
  });
  ...
  it("WebRtcStreamingService framing pinned: 'Top-level streaming service. GUI client + customer dashboard both go through this interface; the production implementation runs on the Mac mini fleet (where the browser session lives) and exposes its signaling via a server-side relay.'", () => {
    expect(body).toMatch(
      /\* Top-level streaming service\. GUI client \+ customer dashboard both go\s*\n?\s*\*\s*through this interface; the production implementation runs on the\s*\n?\s*\*\s*Mac mini fleet \(where the browser session lives\) and exposes its\s*\n?\s*\*\s*signaling via a server-side relay\./,
    );
  });
  ...
  it("createStream framing pinned: 'Begin streaming a session's browser tab. Returns the stream id + server-side SDP offer. Caller's peer connection answers via negotiate(streamId, answer).' + signature returns CreateStreamResult", () => {
    expect(body).toMatch(
      /\*\s*Begin streaming a session's browser tab\. Returns the stream id \+\s*\n?\s*\*\s*server-side SDP offer\. Caller's peer connection answers via\s*\n?\s*\*\s*`negotiate\(streamId, answer\)`\./,
    );
  ...
  it("negotiate framing pinned: 'Exchange the caller's SDP answer (or a renegotiation offer mid-stream). Resolves to the server's response payload (answer when caller offered, answer when caller answered the original — implementations may need a follow-up offer for codec changes).'", () => {
    expect(body).toMatch(
      /\*\s*Exchange the caller's SDP answer \(or a renegotiation offer mid-stream\)\.\s*\n?\s*\*\s*Resolves to the server's response payload \(answer when caller offered,\s*\n?\s*\*\s*answer when caller answered the original — implementations may need\s*\n?\s*\*\s*a follow-up offer for codec changes\)\./,
    );
    expect(body).toMatch(
  ...
  it("submitIceCandidate framing pinned: 'Submit an ICE candidate from the caller's side. The server multiplexes caller candidates into the active peer connection.'", () => {
    expect(body).toMatch(
      /\*\s*Submit an ICE candidate from the caller's side\. The server multiplexes\s*\n?\s*\*\s*caller candidates into the active peer connection\./,
    );
    expect(body).toMatch(
  ...
  it("subscribe framing pinned: 'Returns an unsubscribe fn. Implementations push state changes, periodic stats samples, ICE candidates from the server side (which the caller then adds to its peer connection), and errors. Polling-based fallbacks implement this as a periodic stats fetch + state change diff.' + signature returns () => void", () => {
    expect(body).toMatch(
      /\*\s*Subscribe to stream events\. Returns an unsubscribe fn\.\s*\n?\s*\*\s*\n?\s*\*\s*Implementations push state changes, periodic stats samples,\s*\n?\s*\*\s*ICE candidates from the server side \(which the caller then adds\s*\n?\s*\*\s*to its peer connection\), and errors\. Polling-based fallbacks\s*\n?\s*\*\s*implement this as a periodic stats fetch \+ state change diff\./,
    );
    expect(body).toMatch(
  ...
  it("getStats framing pinned: 'Look up the current state + stats for a stream without subscribing. Used by the customer dashboard for at-a-glance metrics on the sessions list page (V-139 /sessions).'", () => {
    expect(body).toMatch(
      /\*\s*Look up the current state \+ stats for a stream without subscribing\.\s*\n?\s*\*\s*Used by the customer dashboard for at-a-glance metrics on the\s*\n?\s*\*\s*sessions list page \(V-139 \/sessions\)\./,
    );
  ...
  it("close framing pinned: 'End a stream cleanly. Implementations close the peer connection, release server-side capture resources, drop the signaling channel. Idempotent — calling close on an already-closed stream is a no-op.'", () => {
    expect(body).toMatch(
      /\*\s*End a stream cleanly\. Implementations close the peer connection,\s*\n?\s*\*\s*release server-side capture resources, drop the signaling channel\.\s*\n?\s*\*\s*Idempotent — calling close on an already-closed stream is a no-op\./,
    );
  ...
  it("StreamRegistry framing pinned 'Stream registry — read-only enumeration for admin observability.' + list signature with optional accountId + state filter; returns readonly StreamStats[]", () => {
    expect(body).toMatch(
      /\/\*\* Stream registry — read-only enumeration for admin observability\. \*\/\s*\n?\s*export interface StreamRegistry \{\s*\n?\s*\/\*\* List active streams, optionally scoped to an account\. \*\/\s*\n?\s*list\(opts\?: \{ accountId\?: string; state\?: StreamState \}\): Promise<readonly StreamStats\[\]>;\s*\n?\s*\}/,
    );
  });
