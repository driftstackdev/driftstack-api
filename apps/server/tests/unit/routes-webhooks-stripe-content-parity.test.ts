...
describe('W411.B apps/server/src/routes/webhooks-stripe.ts content parity', () => {
  ...
  it('V-080 framing pinned: POST /v1/webhooks/stripe + public no-auth + Stripe-Signature header IS auth + sha256 HMAC', () => {
    ...
    expect(body).toMatch(
      /Public, no auth — Stripe-Signature header IS the auth\. The route\s*\n?\s*\/\/\s*captures the raw request body \(Stripe verifies a sha256 HMAC over\s*\n?\s*\/\/\s*`<timestamp>\.<raw body>`\) and rejects everything that doesn't\s*\n?\s*\/\/\s*pass signature verification before reaching the dispatch layer\./,
    );
  });
  ...
  it('V-666 raw-body framing pinned: shared content-type parser; Fastify one-parser-per-content-type; registerWebhookRawBodyParser sanctioned', () => {
    expect(body).toMatch(
      /Body parsing: V-666 — the raw-body content-type parser is shared\s*\n?\s*\/\/\s*across all webhook routes \(Stripe \+ NowPayments \+ future\) via\s*\n?\s*\/\/\s*`registerWebhookRawBodyParser`\. Fastify only allows ONE parser per\s*\n?\s*\/\/\s*content-type, so the shared module is the only sanctioned path\./,
    );
  ...
  it('RegisterStripeWebhookRoutesDeps: service + signingSecret (whsec_) + logger', () => {
    ...
    expect(body).toMatch(
      /\/\*\* Webhook signing secret as configured in Stripe \(`whsec_\.\.\.`\)\. \*\/\s*\n?\s*signingSecret: string;/,
    );
  ...
  it('verifyStripeSignature: rawBody + header + secret; on !ok warn-log reason + opaque 401 "Invalid Stripe signature." (no leak)', () => {
    expect(body).toMatch(
      /const verified = verifyStripeSignature\(\{\s*\n?\s*rawBody,\s*\n?\s*header: sigHeader,\s*\n?\s*secret: deps\.signingSecret,\s*\n?\s*\}\);/,
    );
    expect(body).toMatch(
      /deps\.logger\.warn\(\s*\n?\s*\{ component: 'stripe-webhooks', reason: verified\.reason \},\s*\n?\s*'Stripe webhook signature verification failed',\s*\n?\s*\);/,
    );
    expect(body).toMatch(
      /\/\/ Don't leak which check failed in the response — Stripe's docs\s*\n?\s*\/\/ say "any 4xx" causes a retry, so 401 is fine\. Log the reason\./,
    );
