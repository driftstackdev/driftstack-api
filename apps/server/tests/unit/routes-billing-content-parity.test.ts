...
describe('W418.C apps/server/src/routes/billing.ts content parity', () => {
  ...
  it('V-082 framing pinned: 4 routes (checkout-session + portal-session + state + the account/me billing-portal alias); trial_pack checkout retired 2026-05-27', () => {
    ...
    expect(body).toMatch(
      /All auth-gated\. The one-time trial_pack checkout was retired 2026-05-27\s*\/\/\s*in favour of the perpetual free tier/,
    );
  });
  ...
  it('Hardcoded-allowlist rationale pinned: anchors security guarantee; env typo would silently re-introduce open-redirect', () => {
    expect(body).toMatch(
      /\/\/ The allowlist is hardcoded rather than env-driven because it\s*\/\/ anchors the security guarantee — a typo in env config would silently\s*\/\/ re-introduce the open-redirect\. Founder edits this list when a\s*\/\/ legitimate origin needs to be added \(paired with PR review\)\./,
    );
  });
  ...
  it('Checkout-session: validates retry identity, V-248 allowlists return URLs, and forwards both to the service', () => {
    ...
    expect(body).toMatch(
      /if \(idempotency\.kind === 'invalid'\) \{\s*throw new BadRequestError\('Invalid Idempotency-Key header\.'\);\s*\}/,
    );
    ...
    expect(body).toMatch(
      /return \{\s*checkout_url: result\.url,\s*checkout_session_id: result\.sessionId,\s*\};/,
    );
  });

  it('Portal session: service.createPortalSession(accountId); reply { portal_url }', () => {
    expect(body).toMatch(
      /const result = await service\.createPortalSession\(ctx\.account\.id\);\s*return \{ portal_url: result\.url \};/,
    );
  });
  ...
  it('GET billing state: V-326c act-as resolution (resolveEffectiveAccount + X-Driftstack-Account header) → getBillingState(effective.accountId); subscription null-handled via publicSubscription (trial_pack envelope removed 2026-05-27)', () => {
    ...
    expect(body).toMatch(
      /const effective = resolveEffectiveAccount\(ctx, readEffectiveAccountHeader\(req\)\);\s*const state = await service\.getBillingState\(effective\.accountId\);\s*return \{\s*subscription: state\.subscription !== null \? publicSubscription\(state\.subscription\) : null,\s*\};/,
    );
  });
  ...
  it('Slice 1119.2 B1 server-side leg: registerBillingDisabledRoutes wires the same 4 paths to 503 FeatureUnavailable stubs when Stripe env is unconfigured (so the customer dashboard 503-detection leg in select-tier.astro gets a machine-readable signal instead of 404)', () => {
    expect(body).toMatch(
      /\/\/ Slice 1119\.2 B1 server-side leg — when Stripe env is not\s*\/\/ configured \(no STRIPE_SECRET_KEY \/ DRIFTSTACK_TIER_PRICE_IDS\),\s*\/\/ `registerBillingRoutes` doesn't run and the `\/v1\/billing\/\*` paths\s*\/\/ fall through to the global 404 handler\./,
    );
    expect(body).toMatch(
      /export function registerBillingDisabledRoutes\(app: FastifyInstance\): void \{/,
    );
    expect(body).toMatch(
      /const detail =\s*'Billing is not configured on this server\. Reach out to support@driftstack\.dev if you expected to use this endpoint\.';/,
    );
    expect(body).toMatch(
      /const stub = \(\): never => \{\s*throw new FeatureUnavailableError\(detail\);\s*\};/,
    );
