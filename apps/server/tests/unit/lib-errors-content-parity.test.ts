...
describe('W389.A apps/server/src/lib/errors.ts content parity', () => {
  ...
  it('RFC 7807 problem+json framing pinned in module comment', () => {
    expect(body).toMatch(
      /The error middleware \(apps\/server\/src\/middleware\/\s*\/\/\s*error-handler\.ts\) converts them to RFC 7807 problem\+json responses/,
    );
  });

  it('"never leak raw error messages to clients" posture pinned', () => {
    expect(body).toMatch(
      /a TypeError, a Drizzle error, a pino crash —\s*\/\/\s*is logged at error level and replied as Internal \(500\) with a stable\s*\/\/\s*problem-type\. We never leak raw error messages to clients/,
    );
  });
  ...
  it('BadRequestError = 400 BadRequest', () => {
    expect(body).toMatch(
      /export class BadRequestError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.BadRequest,\s*title: 'Bad Request',\s*status: 400,/,
    );
  });

  it('ValidationError = 400 ValidationFailed (carries issues extension)', () => {
    expect(body).toMatch(
      /export class ValidationError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.ValidationFailed,\s*title: 'Validation Failed',\s*status: 400,[\s\S]+?extensions: \{ issues \},/,
    );
  });
  ...
  it('UnauthorizedError = 401 Unauthorized (default "API key missing or invalid.")', () => {
    expect(body).toMatch(
      ...
      /export class UnauthorizedError extends ApiError \{[\s\S]+?constructor\(detail = 'API key missing or invalid\.', extensions\?: Record<string, unknown>\)[\s\S]+?type: PROBLEM_TYPES\.Unauthorized,\s*title: 'Unauthorized',\s*status: 401,/,
    );
  });

  it('InvalidKeyError + RevokedKeyError + ExpiredKeyError = 401 trio', () => {
    expect(body).toMatch(
      /export class InvalidKeyError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.InvalidKey,\s*title: 'Invalid API key',\s*status: 401,/,
    );
    expect(body).toMatch(
      /export class RevokedKeyError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.RevokedKey,\s*title: 'API key revoked',\s*status: 401,/,
    );
    expect(body).toMatch(
      /export class ExpiredKeyError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.ExpiredKey,\s*title: 'API key expired',\s*status: 401,/,
    );
  });

  it('ForbiddenError = 403 Forbidden', () => {
    expect(body).toMatch(
      /export class ForbiddenError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.Forbidden,\s*title: 'Forbidden',\s*status: 403,/,
    );
  });

  it('NotFoundError = 404 NotFound', () => {
    expect(body).toMatch(
      /export class NotFoundError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.NotFound,\s*title: 'Not Found',\s*status: 404,/,
    );
  });

  it('ConflictError = 409 Conflict', () => {
    expect(body).toMatch(
      /export class ConflictError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.Conflict,\s*title: 'Conflict',\s*status: 409,/,
    );
  });

  it('RateLimitedError = 429 + retry_after_seconds extension', () => {
    expect(body).toMatch(
      /export class RateLimitedError extends ApiError \{[\s\S]+?constructor\(retryAfterSeconds: number, detail = 'Rate limit exceeded\.'\)[\s\S]+?type: PROBLEM_TYPES\.RateLimited,\s*title: 'Too Many Requests',\s*status: 429,[\s\S]+?extensions: \{ retry_after_seconds: retryAfterSeconds \},/,
    );
  });
  ...
  it('TierLimitError = 429 TierLimit', () => {
    expect(body).toMatch(
      /export class TierLimitError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.TierLimit,\s*title: 'Tier limit reached',\s*status: 429,/,
    );
  });

  it('SessionDestroyedError = 410 Gone-equivalent (SessionDestroyed)', () => {
    expect(body).toMatch(
      /export class SessionDestroyedError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.SessionDestroyed,\s*title: 'Session destroyed',\s*status: 410,/,
    );
  });

  it('SessionTimeoutError = 504 + timeout_ms extension (distinguished from DriverError)', () => {
    expect(body).toMatch(
      /SessionTimeoutError — distinguished from DriverError so customers\s*\/\/\s*can react specifically to "the operation didn't finish within the\s*\/\/\s*per-call timeout I supplied"/,
    );
    expect(body).toMatch(
      /export class SessionTimeoutError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.SessionTimeout,\s*title: 'Session timeout',\s*status: 504,[\s\S]+?extensions: \{ timeout_ms: timeoutMs \},/,
    );
  });

  it('DriverError = 502 DriverError', () => {
    expect(body).toMatch(
      /export class DriverError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.DriverError,\s*title: 'Driver error',\s*status: 502,/,
    );
  });

  it('DriverNotIntegratedError = 503 DriverNotIntegrated', () => {
    expect(body).toMatch(
      /export class DriverNotIntegratedError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.DriverNotIntegrated,\s*title: 'Driver not integrated',\s*status: 503,[\s\S]+?detail:\s*'The selected browser driver does not implement this operation in this deployment\.',/,
    );
  ...
  it('V-352b FeatureUnavailableError = 503 (deploy-time-disabled features, e.g. avatar upload)', () => {
    expect(body).toMatch(
      /V-352b — 503 when an optional feature is disabled at deploy-time\s*\/\/\s*\(e\.g\. avatar upload when the public R2 bucket isn't configured\)/,
    );
    expect(body).toMatch(
      /export class FeatureUnavailableError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.FeatureUnavailable,\s*title: 'Feature unavailable',\s*status: 503,/,
    );
  });

  it('V-353e MfaStepUpRequiredError = 403 + requires_mfa_step_up extension + 2-reason union', () => {
    expect(body).toMatch(
      /V-353e — step-up MFA challenge required to run the requested op\.\s*\/\/\s*Status is 403 \(the caller is authenticated; they just need to prove\s*\/\/\s*MFA again within the 15-min freshness window\)/,
    );
    expect(body).toMatch(/reason: 'never_satisfied' \| 'expired'/);
    expect(body).toMatch(
      /export class MfaStepUpRequiredError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.MfaStepUpRequired,\s*title: 'MFA step-up required',\s*status: 403,[\s\S]+?extensions: \{ requires_mfa_step_up: true, reason \},/,
    );
  });

  it('LegalAcceptanceRequiredError = 409 + pending_acceptances extension (avoids extra GET round-trip)', () => {
    expect(body).toMatch(
      /LegalAcceptanceRequiredError — 409 when an operation is gated on\s*\/\/\s*the customer accepting one or more legal documents \(ToS, Privacy,\s*\/\/\s*DPA, AUP\)/,
    );
    expect(body).toMatch(
      /export class LegalAcceptanceRequiredError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.LegalAcceptanceRequired,\s*title: 'Legal acceptance required',\s*status: 409,[\s\S]+?extensions: \{ pending_acceptances: pendingAcceptances \},/,
    );
    expect(body).toMatch(
  ...
  it('V-079 auth-flow error cluster: EmailAlreadyRegistered / InvalidCredentials / InvalidAuthToken / EmailNotVerified', () => {
    ...
    expect(body).toMatch(
      /export class EmailAlreadyRegisteredError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.EmailAlreadyRegistered,\s*title: 'Email already registered',\s*status: 409,/,
    );
    expect(body).toMatch(
      /export class InvalidCredentialsError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.InvalidCredentials,\s*title: 'Invalid credentials',\s*status: 401,/,
    );
    expect(body).toMatch(
      /export class InvalidAuthTokenError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.InvalidAuthToken,\s*title: 'Invalid auth token',\s*status: 400,/,
    );
    expect(body).toMatch(
      /export class EmailNotVerifiedError extends ApiError \{[\s\S]+?type: PROBLEM_TYPES\.EmailNotVerified,\s*title: 'Email not verified',\s*status: 403,/,
    );
  });

  it('InternalError = 500 Internal (last-resort catch-all)', () => {
    expect(body).toMatch(
      /export class InternalError extends ApiError \{[\s\S]+?constructor\(detail = 'An unexpected error occurred\.', cause\?: unknown\)[\s\S]+?type: PROBLEM_TYPES\.Internal,\s*title: 'Internal Server Error',\s*status: 500,/,
    );
  });
