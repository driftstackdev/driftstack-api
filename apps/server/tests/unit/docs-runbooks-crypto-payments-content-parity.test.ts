...
// here either misstates the persistence posture (V-799: orders are in
// the crypto_orders table and DO survive deploys; the runbook used to
// say the opposite), drops the forward-only state-machine invariant
...
//   • crypto_orders is a Postgres table; orders survive restarts.
//   • V-666.E table wiring is DONE, not a remaining prerequisite.
...
//   • DO NOT edit crypto_orders rows by hand.
...
describe('W555.B /docs/runbooks/crypto-payments.md content parity', () => {
  ...
  it("Header + V-675 + V-666 family framing pinned: '# Crypto-payments operator runbook (V-675)' + 'Operational reference for the V-666 family — NowPayments IPN ingestion, the `CryptoOrdersService` order state machine, the customer-facing `/v1/billing/crypto-checkout` route (V-666.C), and the admin lookup routes (V-666.D).' + 'A customer reports \"I sent the crypto payment but my account is still pending.\"' — pinned so the V-675-runbook + V-666-family-NowPayments-IPN-CryptoOrdersService + V-666.C-crypto-checkout + V-666.D-admin-lookup + still-pending-customer-trigger commitment survives", () => {
    ...
    expect(body).toMatch(/ingestion, the `CryptoOrdersService` order state machine,/);
    expect(body, 'the in-memory framing must not return').not.toMatch(
      /in-memory `CryptoOrdersService`/,
    );
    expect(body).toMatch(
  ...
  it("HMAC-SHA512 + in-memory posture framing pinned: 'POST /v1/billing/crypto-checkout  (V-666.C)' + 'creates CryptoOrder { status:'pending', provider:'stub' }' + 'POST /v1/webhooks/nowpayments  (V-666 + V-666.B)' + 'HMAC-SHA512 signature verification gate' + '**Persistence.** Orders live in the `crypto_orders` table and survive deploys and restarts.' (V-799 retracted the in-memory posture) + 'The V-666.E follow-up wires a `crypto_orders` table when live volume justifies it.' — pinned so the V-666.C-crypto-checkout-stub-pending + V-666.B-NowPayments-webhook + HMAC-SHA512-signature-gate + Map-process-memory-drops-on-deploy + V-666.E-crypto_orders-table-follow-up commitment survives", () => {
    ...
    expect(body).toMatch(
      /> \*\*Persistence\.\*\* Orders live in the `crypto_orders` table and survive\s*\n?> deploys and restarts\./,
    );
    expect(body, 'the required-repo proof, so this cannot drift back').toMatch(
      /takes `repo` as a\s*\n?> REQUIRED constructor field/,
    );
    expect(body).not.toMatch(/\*\*In-memory posture\.\*\*/);
    expect(body).not.toMatch(/all crypto orders are dropped/);
  ...
  it("Refund procedure + when-merchant-account-lands framing pinned: 'Refunds are issued by the founder via the NowPayments dashboard, not by Driftstack.' + 'Founder issues the refund in NowPayments (asset + amount + the customer's forwarding address).' + 'Our applyIpnStatus maps `refunded` → `failed`. The order moves to terminal `failed`.' + 'Do NOT edit `crypto_orders` rows by hand' + '## When the merchant account lands (V-666.E follow-up)' + 'NowPayments merchant account is approved + API keys minted.' + '`NOWPAYMENTS_API_KEY` env var is set in production.' + 'The `/v1/billing/crypto-checkout` route's stubbed `payment_address: null` response is replaced with a real NowPayments `POST /v1/payment` call' + 'The customer-facing crypto checkout flow is unblocked in the GUI (V-534.J button + view).' + '~~A `crypto_orders` table replaces the in-memory repo~~ **Done**' — pinned so the founder-issues-via-NowPayments + refunded→failed-mapping + DO-NOT-mutate-by-hand + V-666.E-5-step-go-live + V-534.J-GUI-button commitment survives", () => {
    ...
    expect(body).toMatch(/Do NOT edit `crypto_orders` rows by hand/);
    expect(body).not.toMatch(/mutate the in-memory store/);
    ...
    // V-799 — this was listed as a REMAINING go-live prerequisite. It is done.
    expect(body).toMatch(/~~A `crypto_orders` table replaces the in-memory repo \(V-666\.E/);
    expect(body).toMatch(/\*\*Done\*\* — the table exists and bootstrap wires/);
  });
