...
describe('docs/api/byok-anthropic content parity', () => {
  ...
  it("Q4=A design verdict 2026-05-16 framing pinned: 'BYOK always wins over bundled-LLM in the resolution chain — per Driftstack design verdict Q4=A (2026-05-16), BYOK is the v1.0 primary path; bundled-LLM is the no-BYOK fallback.' — V-211 anonymity rewrote 'founder verdict' to 'Driftstack design verdict' in customer-facing copy; pinned so the Q4=A verdict-date + BYOK-primary-path + bundled-LLM-fallback contract stays documented.", () => {
    expect(body).toMatch(
      /BYOK always wins over bundled-LLM\s*in the resolution chain — per Driftstack design verdict Q4=A\s*\(2026-05-16\), BYOK is the v1\.0 primary path; bundled-LLM is the\s*no-BYOK fallback\./,
    );
  });
  ...
  it('AES-256-GCM at-rest envelope, storage column, and shared GUI-control-key authority stay documented without internal work-item labels', () => {
    expect(body).toMatch(
      /encrypted at rest via AES-256-GCM \(sealed\s*with `MFA_ENCRYPTION_KEY`\)/,
    );
    expect(body).toMatch(
      /The\s*canonical blob shape is `\[12-byte IV \| 16-byte auth tag \|\s*ciphertext\]`\. Storage column: `accounts\.byok_anthropic_key_blob`\s*\(bytea\)\./,
    );
    expect(body).toMatch(
      /deployment's `MFA_ENCRYPTION_KEY` env var \(shared with encrypted\s*GUI control keys\)/,
    );
  });
  ...
  it('Q3 account_owner-vs-members framing pinned for mutations; GET requires broad read because credential timestamps are account-wide metadata', () => {
    expect(body).toMatch(
      /Required scope: `account_owner` \(team members can USE the resolved\s*key but cannot manage it — Q3 verdict\)\./,
    );
    expect(body).toMatch(
  ...
  it('test response never echoes provider material and the server probe stays fixed, no-inference, and body-blind', () => {
    expect(body).toMatch(
      /The test response NEVER echoes any part of the key, Anthropic response\s*body, or native transport error\./,
    );
    expect(body).toMatch(
    ...
    expect(body).toMatch(/Audit and metrics retain\s*only a bounded outcome/);
  });
  ...
  it('secret-redaction framing keeps plaintext out of logs and Sentry breadcrumbs', () => {
    expect(body).toMatch(
      /- The plaintext key is encrypted at rest \+ never logged\. Sentry\s*breadcrumbs around the route paths use the shared secret-redaction\s*filter\./,
    );
  });
});
