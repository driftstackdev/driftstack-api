...
describe('W476.A apps/gui-client/src/components/CryptoOrderSummaryCard.tsx content parity', () => {
  ...
  it("V-534.AF framing pinned: 'V-534.AF — presentational summary card for a single crypto order.' + V-534.BF framing 'V-534.BF — surfaces V-666.AV expires_at as a human-readable \"pay before X\" countdown row when the order is pending.' + 'Replaces the inline <dl> block previously embedded in CryptoOrderDetailView. Pure presentational; no fetching, no actions. Callers compose this with cancel buttons / receipt panels around it.'", () => {
    ...
    expect(body).toMatch(
      /\/\/ V-534\.BF — surfaces V-666\.AV expires_at as a human-readable\s*\/\/\s+"pay before X" countdown row when the order is pending\./,
    );
    expect(body).toMatch(
      /\/\/ Replaces the inline <dl> block previously embedded in\s*\/\/ CryptoOrderDetailView\. Pure presentational; no fetching, no actions\.\s*\/\/ Callers compose this with cancel buttons \/ receipt panels around it\./,
    );
  });
  ...
  it("CryptoOrderSummaryCardProps 3-field: order: CryptoOrderData + footer? React.ReactNode 'Optional content rendered below the summary fields (cancel button, etc.).' + nowFn? V-534.BF 'testing seam so the countdown is deterministic. Defaults to Date.now.'", () => {
    expect(body).toMatch(
      /export interface CryptoOrderSummaryCardProps \{\s*order: CryptoOrderData;\s*\/\*\* Optional content rendered below the summary fields \(cancel button, etc\.\)\. \*\/\s*footer\?: React\.ReactNode;\s*\/\*\* V-534\.BF — testing seam so the countdown is deterministic\. Defaults to Date\.now\. \*\/\s*nowFn\?: \(\) => number;\s*\}/,
    );
  });
  ...
  it("Render shape: <section> rounded-md border + <header> with 'Order' h3 + order_id font-mono + CryptoOrderStatusBadge status delegation + <dl> grid-cols-2 + Product/Amount(formatCents)/payment_id conditional/Created/Updated rows; Pay by row only when showExpiry with (nowFn ?? Date.now)() invocation at render time + footer !== undefined conditional render", () => {
    expect(body).toMatch(
      /<h3 className="text-base font-semibold">Order<\/h3>\s*<p className="font-mono text-xs text-ink-secondary">\{order\.order_id\}<\/p>/,
    );
    ...
    expect(body).toMatch(
      /\{order\.payment_id !== null && \(\s*<>\s*<dt className="text-ink-secondary">Payment id<\/dt>\s*<dd className="font-mono text-xs">\{order\.payment_id\}<\/dd>\s*<\/>\s*\)\}/,
    );
    expect(body).toMatch(
      /\{showExpiry && \(\s*<>\s*<dt className="text-ink-secondary">Pay by<\/dt>\s*<dd>\s*<span>\{formatTimestamp\(order\.expires_at as string\)\}<\/span>\{' '\}\s*<span className="text-xs text-ink-secondary">\s*\(\{describeExpiry\(order\.expires_at as string, \(nowFn \?\? Date\.now\)\(\)\)\}\)\s*<\/span>\s*<\/dd>\s*<\/>\s*\)\}/,
    );
