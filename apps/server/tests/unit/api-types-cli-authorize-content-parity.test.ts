...
describe('W433.B packages/api-types/src/cli-authorize.ts content parity', () => {
  ...
  it('V-266 framing pinned: replaces paste-key flow with OAuth-style browser handshake; 4-step rationale', () => {
    ...
    expect(body).toMatch(
      /\/\/\s*1\. CLI\/GUI calls \/v1\/auth\/cli-authorize\/initiate, gets a one-shot\s*\/\/\s+`code`, a device-displayed `user_code`, and a `browser_url` to open\./,
    );
    expect(body).toMatch(
      /\/\/\s*2\. CLI\/GUI opens browser_url; user signs in to the dashboard if\s*\/\/\s+not already, types the user_code shown by their device, and clicks\s*\/\/\s+Authorize\. The browser URL alone cannot approve another device\./,
    );
    expect(body).toMatch(
      /\/\/\s*3\. Dashboard calls \/v1\/auth\/cli-authorize\/bind-device-code with web-session\s*\/\/\s+bearer auth plus the user_code; server mints a scoped API key and\s*\/\/\s+stores only its encrypted envelope under `sha256\(code\)` \(Redis,\s*\/\/\s+2-minute bound TTL\)\./,
    );
    expect(body).toMatch(
      /\/\/\s*4\. CLI\/GUI polls \/v1\/auth\/cli-authorize\/exchange until status\s*\/\/\s+flips from `pending` → `bound` → returns plaintext API key\./,
    );
  });
  ...
  it('code + state CSRF rationale pinned: code 32+ byte URL-safe; state CSRF nonce supplied on initiate, echoed in browser_url, verified at bind, re-verified at exchange (defends against wrong-session binding)', () => {
    expect(body).toMatch(
      /\/\/ `code` is a 32\+ byte random URL-safe string\. `state` is a CSRF nonce\s*\/\/ supplied by the CLI\/GUI on initiate, echoed in the browser URL,\s*\/\/ verified by the dashboard on bind, and re-verified by the CLI\/GUI\s*\/\/ on exchange — defends against the dashboard binding a code that\s*\/\/ wasn't issued in the same session\./,
    );
  });
  ...
  it('CliAuthorizeInitiateRequest: state 16..128 (CSRF nonce) + optional client_label 1..120 ("Driftstack desktop app on John\'s MacBook Pro" rationale)', () => {
    expect(body).toMatch(
      /\/\*\* Client-supplied CSRF nonce — echoed back in the browser URL and\s*\*\s*re-verified at exchange time\. Min 16 chars of URL-safe entropy\s*\*\s*\(the CLI\/GUI generates this via crypto-random\)\. \*\/\s*state: z\.string\(\)\.min\(16\)\.max\(128\),/,
    );
    expect(body).toMatch(
      /\/\*\* Human-friendly label that appears on the dashboard's confirmation\s*\*\s*screen \("Driftstack desktop app on John's MacBook Pro"\) so the\s*\*\s*user knows what they're authorizing\. \*\/\s*client_label: z\.string\(\)\.min\(1\)\.max\(120\)\.optional\(\),/,
    );
  });

  it('CliAuthorizeInitiateResponse: opaque URL code + separate displayed user_code + browser URL', () => {
    expect(body).toMatch(
      /\/\*\* One-shot opaque device code; never displayed to the user\. \*\/\s*code: z\.string\(\),[\s\S]*?user_code: CliAuthorizeUserCodeSchema,[\s\S]*?browser_url: z\.string\(\)\.url\(\),[\s\S]*?expires_at: Iso8601Schema/,
    );
  });
  ...
  it('CliAuthorizeBindRequest requires the device-displayed user_code', () => {
    ...
    expect(body).toMatch(
      /export const CliAuthorizeBindRequestSchema = z\.object\(\{\s*code: z\.string\(\)\.min\(16\)\.max\(128\),\s*state: z\.string\(\)\.min\(16\)\.max\(128\),[\s\S]*?user_code: CliAuthorizeUserCodeSchema,[\s\S]*?scopes: ApiKeyScopeListRequestSchema\.optional\(\),\s*\}\);/,
    );
  });
  ...
  it('CliAuthorizeBindResponse: ok literal true + account_id (echoed for dashboard confirmation UI; plaintext key NEVER returned here — only via /exchange) + expires_at', () => {
    expect(body).toMatch(
      /export const CliAuthorizeBindResponseSchema = z\.object\(\{\s*ok: z\.literal\(true\),\s*\/\*\* Echoed for the dashboard's confirmation UI \("Authorized as\s*\*\s*acc_…"\)\. The plaintext key NEVER returns through this endpoint —\s*\*\s*only the CLI\/GUI receives it via \/exchange\. \*\/\s*account_id: z\.string\(\),\s*expires_at: Iso8601Schema,\s*\}\);/,
    );
  });
  ...
  it('CliAuthorizeExchangeResponse: discriminated-union on status; pending branch (status only); bound branch (api_key + account_id one-shot); expired branch (status only)', () => {
    expect(body).toMatch(
      /export const CliAuthorizeExchangePendingResponseSchema = z\.object\(\{\s*status: z\.literal\('pending'\),\s*\}\);/,
    );
    expect(body).toMatch(
      /export const CliAuthorizeExchangeBoundResponseSchema = z\.object\(\{\s*status: z\.literal\('bound'\),\s*api_key: z\.string\(\),\s*account_id: z\.string\(\),\s*\}\);/,
    );
    expect(body).toMatch(
      /export const CliAuthorizeExchangeExpiredResponseSchema = z\.object\(\{\s*status: z\.literal\('expired'\),\s*\}\);/,
    );
    expect(body).toMatch(
      /export const CliAuthorizeExchangeResponseSchema = z\.discriminatedUnion\('status', \[\s*CliAuthorizeExchangePendingResponseSchema,\s*CliAuthorizeExchangeBoundResponseSchema,\s*CliAuthorizeExchangeExpiredResponseSchema,\s*\]\);/,
    );
  });
