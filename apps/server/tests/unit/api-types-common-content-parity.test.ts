...
describe('W436.A packages/api-types/src/common.ts content parity', () => {
  ...
  it('PaginationQuery framing pinned: opaque cursor strings; servers may swap encoding later without breaking clients; limit coerce 1..100 default 50 + cursor min-1 max-512 optional (slice 148 cap)', () => {
    expect(body).toMatch(
      /\/\/ Cursor pagination — opaque cursor strings; servers may swap encoding later\s*\/\/ without breaking clients\./,
    );
    ...
    expect(body).toMatch(
      /export const PaginationQuerySchema = z\.object\(\{\s*limit: z\.coerce\.number\(\)\.int\(\)\.min\(1\)\.max\(100\)\.default\(50\),\s*cursor: z\.string\(\)\.min\(1\)\.max\(512\)\.optional\(\),\s*\}\);/,
    );
  });

  it('PaginatedListSchema: generic factory (item: T) → data + has_more + next_cursor nullable', () => {
    expect(body).toMatch(
      /export const PaginatedListSchema = <T extends z\.ZodTypeAny>\(item: T\) =>\s*z\.object\(\{\s*data: z\.array\(item\),\s*has_more: z\.boolean\(\),\s*next_cursor: z\.string\(\)\.nullable\(\),\s*\}\);/,
    );
  });
  ...
  it('PrefixedId framing pinned: public-facing prefixed strings ("acc_…", "key_…", "ses_…") stored as UUIDs in Postgres; mapping at route boundary; service+DB use raw UUIDs', () => {
    expect(body).toMatch(
      /\/\/ Public-facing IDs are prefixed strings \("acc_…", "key_…", "ses_…"\) even\s*\/\/ though stored as UUIDs in Postgres\. The mapping happens at the route\s*\/\/ boundary; service layers and DB use raw UUIDs\./,
    );
    expect(body).toMatch(
      /export const PrefixedId = \(prefix: string\): z\.ZodString =>\s*z\s*\.string\(\)\s*\.regex\(new RegExp\(`\^\$\{prefix\}_\[0-9a-f\]\{8\}-\[0-9a-f\]\{4\}-\[0-9a-f\]\{4\}-\[0-9a-f\]\{4\}-\[0-9a-f\]\{12\}\$`\), \{\s*message: `must start with "\$\{prefix\}_" followed by a UUID`,\s*\}\);/,
    );
  ...
  it('Manual ladder rationale pinned: GUI client humans; profile count tier-defining; solo $79/$758 / team $249/$2390 / agency $699/$6710 with 10/50/200 profiles + 1/3/8 concurrent + unlimited hours', () => {
    expect(body).toMatch(
      /\/\/ Manual ladder \(humans clicking GUI client; profile count tier-defining\):\s*\/\/\s*- solo_manual\s+\$79\/mo\s+\(\$758\/yr = \$63\/mo\)\s+— 10 profiles\s+\/ 1 concurrent \/ unlimited hours\s*\/\/\s*- team_manual\s+\$249\/mo\s+\(\$2,390\/yr = \$199\/mo\)\s+— 50 profiles\s+\/ 3 concurrent \/ unlimited hours\s*\/\/\s*- agency_manual\s+\$699\/mo\s+\(\$6,710\/yr = \$559\/mo\)\s+— 200 profiles \/ 8 concurrent \/ unlimited hours/,
    );
  });
  ...
  it('API ladder rationale pinned: programmatic SDK access; concurrent caps tier-defining; starter $149/builder $499/scale $1499 with 25/100/500 profiles + 2/8/24 concurrent; enterprise from $4000/mo annual-only + negotiated', () => {
    expect(body).toMatch(
      /\/\/ API ladder \(programmatic SDK access; concurrent caps tier-defining\):\s*\/\/\s*- api_starter\s+\$149\/mo\s+\(\$1,430\/yr = \$119\/mo\)\s+— 25 profiles\s+\/ 2 concurrent\s+\/ unlimited hours\s*\/\/\s*- api_builder\s+\$499\/mo\s+\(\$4,790\/yr = \$399\/mo\)\s+— 100 profiles \/ 8 concurrent\s+\/ unlimited hours\s*\/\/\s*- api_scale\s+\$1,499\/mo \(\$14,390\/yr = \$1,199\/mo\) — 500 profiles \/ 24 concurrent \/ unlimited hours\s*\/\/\s*- enterprise\s+from \$4,000\/mo annual only — custom profiles \+ concurrent, negotiated/,
    );
  });
  ...
  it('Annual 20% off + concurrent caps the ONLY metering primitive on paid tiers + free tier has no usage metering + profile-count enforced at /v1/profiles creation gate', () => {
    expect(body).toMatch(
      /\/\/ Annual is 20% off across all tiers\. Concurrent caps are the\s*\/\/ only metering primitive on paid tiers; the free tier has no\s*\/\/ usage metering at all \(no credit, no hours cap, no expiry\)\./,
    );
  ...
  it('AccountTier enum: 8 values pinned (free/solo_manual/team_manual/agency_manual/api_starter/api_builder/api_scale/enterprise)', () => {
    expect(body).toMatch(
      /export const AccountTierSchema = z\.enum\(\[\s*'free',\s*'solo_manual',\s*'team_manual',\s*'agency_manual',\s*'api_starter',\s*'api_builder',\s*'api_scale',\s*'enterprise',\s*\]\);/,
    );
  });
  ...
  it('PROFILES_PER_TIER: single source of truth for marketing/dashboard/server enforcement; mirrored in marketing-site pricing.ts API_TIERS field profiles; enterprise = "custom"', () => {
    expect(body).toMatch(
      /\*\s*Profile-count limits per tier — single source of truth for\s*\*\s*marketing-site, customer-dashboard, and server-side enforcement\./,
    );
    expect(body).toMatch(
      /export const PROFILES_PER_TIER: Record<AccountTier, number \| 'custom'> = \{\s*free: 1,\s*solo_manual: 10,\s*team_manual: 50,\s*agency_manual: 200,\s*api_starter: 25,\s*api_builder: 100,\s*api_scale: 500,\s*enterprise: 'custom',\s*\};/,
    );
  });

  it('6.g PROXIES_PER_TIER: free 1 (BYO SOCKS5; every session needs a proxy) / solo 10 / scaling to enterprise custom', () => {
    expect(body).toMatch(
      /export const PROXIES_PER_TIER: Record<AccountTier, number \| 'custom'> = \{\s*free: 1,\s*solo_manual: 10,\s*team_manual: 25,\s*agency_manual: 50,\s*api_starter: 25,\s*api_builder: 100,\s*api_scale: 500,\s*enterprise: 'custom',\s*\};/,
    );
    expect(body).toMatch(
      /owner-scoped saved-proxy create gate enforces every numeric\s*\* tier atomically/,
    );
  ...
  it('6.g MAX_SESSION_MINUTES_PER_TIER: free 20-min auto-destroy cap; paid tiers null (unlimited)', () => {
    expect(body).toMatch(
      /export const MAX_SESSION_MINUTES_PER_TIER: Record<AccountTier, number \| null> = \{\s*free: 20,\s*solo_manual: null,\s*team_manual: null,\s*agency_manual: null,\s*api_starter: null,\s*api_builder: null,\s*api_scale: null,\s*enterprise: null,\s*\};/,
    );
  });
  ...
  it('BucketLimitConfig interface + TIER_RATE_LIMIT_DEFAULTS Record values pinned (trial 60/1 + 5/(1/60) + msg 20/(1/5) + input 240/60; api_scale 6000/100 + 120/2 + msg 1000/10 + input 1200/300; enterprise 60000/1000 + 600/10 + msg 10000/100 + input 12000/3000)', () => {
    expect(body).toMatch(
      /export interface BucketLimitConfig \{\s*capacity: number;\s*refill_per_second: number;\s*\}/,
    );
    expect(body).toMatch(
      /export const TIER_RATE_LIMIT_DEFAULTS: Record<\s*AccountTier,\s*Record<\s*'global' \| 'sessions:create' \| 'agent_sessions:message' \| 'agent_sessions:input_event',\s*BucketLimitConfig\s*>\s*> = \{/,
    );
    expect(body).toMatch(
      /free: \{\s*global: \{ capacity: 60, refill_per_second: 1 \},\s*'sessions:create': \{ capacity: 5, refill_per_second: 1 \/ 60 \},[\s\S]*?'agent_sessions:message': \{ capacity: 20, refill_per_second: 1 \/ 5 \},[\s\S]*?'agent_sessions:input_event': \{ capacity: 240, refill_per_second: 60 \},\s*\},/,
    );
    expect(body).toMatch(
      /api_scale: \{\s*global: \{ capacity: 6_000, refill_per_second: 100 \},\s*'sessions:create': \{ capacity: 120, refill_per_second: 2 \},\s*'agent_sessions:message': \{ capacity: 1_000, refill_per_second: 10 \},\s*'agent_sessions:input_event': \{ capacity: 1_200, refill_per_second: 300 \},\s*\},/,
    );
    expect(body).toMatch(
      /enterprise: \{\s*global: \{ capacity: 60_000, refill_per_second: 1_000 \},\s*'sessions:create': \{ capacity: 600, refill_per_second: 10 \},\s*'agent_sessions:message': \{ capacity: 10_000, refill_per_second: 100 \},\s*'agent_sessions:input_event': \{ capacity: 12_000, refill_per_second: 3_000 \},\s*\},/,
    );
  });
  ...
  it('TIER_FEATURES Record: free (test env + apiAccess false + aiAgent false) + team_manual (apiAccess true + aiAgent true + byok_only) + api_builder (byok_or_bundled) + enterprise (custom profiles + byok_or_bundled_custom)', () => {
    expect(body).toMatch(
      /free: \{\s*concurrentSessions: 1,\s*profiles: 1,\s*apiKeyEnvironment: 'test',\s*apiAccess: false,\s*aiAgent: false,\s*llmBilling: null,\s*vpnEgress: false,\s*\},/,
    );
    expect(body).toMatch(
      /team_manual: \{\s*concurrentSessions: 3,\s*profiles: 50,\s*apiKeyEnvironment: 'live',\s*apiAccess: true,\s*aiAgent: true,\s*llmBilling: 'byok_only',\s*vpnEgress: true,\s*\},/,
    );
    expect(body).toMatch(
      /api_builder: \{\s*concurrentSessions: 8,\s*profiles: 100,\s*apiKeyEnvironment: 'live',\s*apiAccess: true,\s*aiAgent: true,\s*llmBilling: 'byok_or_bundled',\s*vpnEgress: true,\s*\},/,
    );
    expect(body).toMatch(
      /enterprise: \{\s*concurrentSessions: 32,\s*profiles: 'custom',\s*apiKeyEnvironment: 'live',\s*apiAccess: true,\s*aiAgent: true,\s*llmBilling: 'byok_or_bundled_custom',\s*vpnEgress: true,\s*\},/,
    );
  });

  it('TierBooleanFeature type + tierFeatures() pure lookup + tierHasFeature(tier, feature) boolean predicate', () => {
    expect(body).toMatch(
      /export type TierBooleanFeature = \{\s*\[K in keyof TierFeatures\]: TierFeatures\[K\] extends boolean \? K : never;\s*\}\[keyof TierFeatures\];/,
    );
    expect(body).toMatch(
      /export function tierFeatures\(tier: AccountTier\): TierFeatures \{\s*return TIER_FEATURES\[tier\];\s*\}/,
    );
    expect(body).toMatch(
      /export function tierHasFeature\(tier: AccountTier, feature: TierBooleanFeature\): boolean \{\s*return TIER_FEATURES\[tier\]\[feature\];\s*\}/,
    );
  });
  ...
  it('LOCKED_ARCHETYPE_ID + V-136 rename rationale pinned: was iphone16pro_ios26_4_1 (conflated Safari 26.4 with fictional iOS 26.4.1); corrected to iphone16pro_ios18_7_safari26_4; iOS major bump cycles BOTH values; Safari version part of identifier so Safari-only updates ship without touching iOS framing', () => {
    expect(body).toMatch(
      /\*\s*The identifier \(`iphone16pro_ios18_7_safari26_4`\) is what the API\s*\*\s*accepts on `\/v1\/sessions \{ archetype \}` and what the server stores\s*\*\s*on session \+ profile rows\./,
    );
    expect(body).toMatch(
    ...
    expect(body).toMatch(
      /export const ARCHETYPE_DISPLAY_LABEL: Record<string, string> = Object\.fromEntries\(\s*ARCHETYPE_REGISTRY\.map\(\(a\) => \[a\.id, a\.displayLabel\]\),\s*\);/,
    );
    expect(body).toMatch(
      /export function archetypeDisplayLabel\(id: string\): string \{\s*return ARCHETYPE_DISPLAY_LABEL\[id\] \?\? id;\s*\}/,
    );
  });
  ...
  it('ARCHETYPE_REGISTRY is the multi-archetype catalogue (NOT a single hardcoded device): ArchetypeConfig shape + status enum + the locked id (iphone17, post-2026-06-11 cutover) is the sole status:launch entry + the full 81-slug Agent-1 catalog folds in as `available` + a legacy reference baseline retained', () => {
    ...
    expect(body).toMatch(
      /id: LOCKED_ARCHETYPE_ID,\s*displayLabel: LOCKED_ARCHETYPE_DISPLAY_LABEL,/,
    );
  ...
  it('V-174 scope split framing pins legacy admin as customer-only and exact staff authority for /v1/admin/*', () => {
    ...
    expect(body).toMatch(
      /Cross-account access is\s*\*\s*therefore possible BY DESIGN and membership-gated/,
    );
    expect(body).toMatch(/`ctx\.teams` is\s*\*\s*server-derived and never client-supplied/);
  ...
  it('V-481 ApiKeyScope enum: broad (read|write|admin|account_owner|driftstack_internal_admin|gui_control) + granular verb:resource set with backward-compat framing pinned (broad satisfies granular via verb-prefix in requireScope/auth; granular does NOT satisfy broad — narrow keys stay narrow)', () => {
    ...
    expect(body).toMatch(/'read:sessions',\s*'write:sessions',/);
    expect(body).toMatch(/'read:profiles',\s*'write:profiles',\s*'admin:profiles',/);
    expect(body).toMatch(/'read:webhooks',\s*'write:webhooks',\s*'admin:webhooks',/);
    expect(body).toMatch(/'read:api-keys',\s*'admin:api-keys',/);
    expect(body).toMatch(/'read:billing',\s*'admin:billing',/);
  ...
  it('parseGranularScope: returns null for non-granular (no colon); returns {verb, resource} for read|write|admin granular', () => {
    expect(body).toMatch(
      /export function parseGranularScope\(\s*scope: ApiKeyScope,\s*\): \{ verb: 'read' \| 'write' \| 'admin'; resource: string \} \| null \{\s*const idx = scope\.indexOf\(':'\);\s*if \(idx === -1\) return null;\s*const verb = scope\.slice\(0, idx\);\s*if \(verb !== 'read' && verb !== 'write' && verb !== 'admin'\) return null;\s*return \{ verb, resource: scope\.slice\(idx \+ 1\) \};\s*\}/,
    );
  });
