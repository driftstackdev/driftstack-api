...
describe('W949 webhooks service cross-source invariant', () => {
  ...
  it('CRITICAL WebhookEventType 10 values — 5 customer-subscribable (session.completed / session.failed / quota.warning_80pct / quota.exceeded / api_key.revoked) + 1 V-356 test-only (test.ping) + 1 Arc 5 EGRESS (session.egress_capability_changed) + 2 V-666 crypto-order (crypto.order.paid / crypto.order.failed) + 1 W393 challenge-handling (session.challenge_detected). The 5+1+1+2+1 split lets test pings dispatch without subscription, lets EGRESS observe capability shifts, lets customers subscribe to crypto-order terminal-state transitions, and surfaces harness-detected bot-challenges.', () => {
    const p = read(resolve(REPO_ROOT, 'apps/server/src/services/webhooks.ts'));
