...
describe('W714 server-side request-id + V-251 IP rate-limit middleware parity', () => {
  ...
  it('CRITICAL AUTH_IP_LIMITS 7-endpoint roster pinned with capacity + refillPerSecond per endpoint. Each row sets the per-IP cap; drift to widening would let abusers fire more requests per minute. magicLink added 2026-05-15 (#190) since each call fires a Postmark send.', () => {
    const src = read(IP_RATE_LIMIT);
  ...
  it('CRITICAL "1-minute window" sustained-rate framing pinned. Every AUTH_IP_LIMITS entry sets refillPerSecond = capacity / 60 — drift to a different divisor would silently change customer expectations about per-minute caps.', () => {
    const src = read(IP_RATE_LIMIT);

    // All 7 entries use the same `/ 60` divisor.
    const refillEntries = src.match(/refillPerSecond: \d+ \/ 60/g) ?? [];
    expect(refillEntries.length, '/ 60 sustained-rate uses').toBe(7);
  });
