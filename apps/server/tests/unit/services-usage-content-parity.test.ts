...
describe('W408.B apps/server/src/services/usage.ts content parity', () => {
  ...
  it('Period definition framing pinned: calendar month UTC + Phase 6 default + per-account billing-anchor deferred', () => {
    expect(body).toMatch(
      /Period definition \(Phase 6 default\): calendar month UTC\. The period start\s*\n?\s*\/\/\s*is `YYYY-MM-01T00:00:00Z`; end is the start of the next month\. Customers\s*\n?\s*\/\/\s*who need finer granularity can wire in a per-account billing-anchor field\s*\n?\s*\/\/\s*later \(out of scope for Phase 6\)\./,
    );
  });

  it('FUTURE-SELF NOTE framing pinned: session_minute → browser_hour rename deferred to Workstream D Stripe Meter', () => {
    expect(body).toMatch(
      /FUTURE-SELF NOTE — `session_minute` rename to `browser_hour` is\s*\n?\s*\/\/\s*deferred to Workstream D \(Stripe Meter integration\)\./,
    );
    expect(body).toMatch(
      /The customer-facing meter is\s*\n?\s*\/\/\s*browser-hours \(file 127 \+ V-061\), so summary-layer code rolls this\s*\n?\s*\/\/\s*up via `floor\(session_minute_total \/ 60\) = browser_hour_total`\./,
    );
    expect(body).toMatch(
      /The\s*\n?\s*\/\/\s*rename is a coordinated breaking change \(Postgres enum migration \+\s*\n?\s*\/\/\s*3-SDK regen \+ OpenAPI version bump\) and bundles cleanly with the\s*\n?\s*\/\/\s*Stripe Meter event-name introduction in Workstream D/,
    );
  });

  it('UsageRecordType: 6-literal union (session_minute / navigate / interact / wait / state_capture / screenshot_capture)', () => {
    expect(body).toMatch(
      /export type UsageRecordType =\s*\n?\s*\| 'session_minute'\s*\n?\s*\| 'navigate'\s*\n?\s*\| 'interact'\s*\n?\s*\| 'wait'\s*\n?\s*\| 'state_capture'\s*\n?\s*\| 'screenshot_capture';/,
    );
  });

  it('ALL_TYPES array mirrors 6-literal union order (session_minute first)', () => {
    expect(body).toMatch(
      /const ALL_TYPES: UsageRecordType\[\] = \[\s*\n?\s*'session_minute',\s*\n?\s*'navigate',\s*\n?\s*'interact',\s*\n?\s*'wait',\s*\n?\s*'state_capture',\s*\n?\s*'screenshot_capture',\s*\n?\s*\];/,
    );
  });
  ...
  it('V-073 NOTE framing pinned: map preserved with null rather than removed so /v1/usage response shape unchanged', () => {
    expect(body).toMatch(
      /\/\/ V-073 NOTE: this map is preserved with `null` values rather than\s*\n?\s*\/\/ removed entirely so the `\/v1\/usage` summary response shape \(which\s*\n?\s*\/\/ returns `quotas: Record<UsageRecordType, number \| null>`\) doesn't\s*\n?\s*\/\/ change\./,
    );
  });
  ...
  it('summaryFor: monthStartUtc(now) + nextMonthStartUtc(periodStart); fullTotals defaults all 6 types to 0; quotas mirrors TIER_QUOTAS[tier]', () => {
    expect(body).toMatch(
      /const periodStart = monthStartUtc\(now\);\s*\n?\s*const periodEnd = nextMonthStartUtc\(periodStart\);/,
    );
    expect(body).toMatch(
      /const fullTotals: Record<UsageRecordType, number> = \{\s*\n?\s*session_minute: 0,\s*\n?\s*navigate: 0,\s*\n?\s*interact: 0,\s*\n?\s*wait: 0,\s*\n?\s*state_capture: 0,\s*\n?\s*screenshot_capture: 0,\s*\n?\s*\};/,
    );
    expect(body).toMatch(
      /for \(const t of ALL_TYPES\) \{\s*\n?\s*fullTotals\[t\] = totals\[t\] \?\? 0;/,
    );
  ...
  it('V-170 dailySeries: clampedDays = max(1, min(days, 90)); fills missing buckets with empty (sparkline gap-free)', () => {
    ...
    expect(body).toMatch(
      /\/\/ Fill missing days with empty buckets so the response is contiguous\s*\n?\s*\/\/ \(sparkline rendering doesn't need to handle gaps\)\./,
    );
    expect(body).toMatch(
  ...
  it("V-170 dailySeries empty-buckets framing: writers aren't wired in production today (V-014/V-015 amendment); endpoint returns contract shape with zeros", () => {
    expect(body).toMatch(
      /Today the buckets are all empty because usage_records writers\s*\n?\s*\*\s*aren't wired in production code \(per V-014\/V-015 amendment \+\s*\n?\s*\*\s*usage\.ts:51-53 comment\)\. The endpoint returns the contract shape\s*\n?\s*\*\s*with zeros; once writers land, the dashboard auto-populates\./,
    );
  });
  ...
  it("V-330e dailySeries: effectiveAccountId pulls OWNER's daily buckets via team RBAC; tier-quotas not needed (just bucket counts)", () => {
    expect(body).toMatch(
      /\/\/ V-330e — pull the OWNER's daily buckets when called via team\s*\n?\s*\/\/ RBAC\. Tier-derived quotas don't apply to the series response\s*\n?\s*\/\/ shape \(it's just bucket counts\), so we don't need the owner's\s*\n?\s*\/\/ tier here\./,
    );
  ...
  it('UsageDailyBucket: date YYYY-MM-DD UTC + totals Partial<Record>; dailyBucketsForRange returns INCLUDING zero-usage days', () => {
    ...
    expect(body).toMatch(
      /export interface UsageDailyBucket \{\s*\n?\s*date: string;\s*\n?\s*totals: Partial<Record<UsageRecordType, number>>;\s*\n?\s*\}/,
    );
    expect(body).toMatch(
      /V-170 — daily aggregation in `\[fromDate, toDate\)` \(toDate exclusive\)\.\s*\n?\s*\*\s*Returns one bucket per UTC day, INCLUDING days with zero usage/,
    );
  });

  it('dayStartUtc + monthStartUtc + nextMonthStartUtc helpers: Date.UTC constructor (no local-tz drift)', () => {
    expect(body).toMatch(
      /function dayStartUtc\(now: Date\): Date \{\s*\n?\s*return new Date\(Date\.UTC\(now\.getUTCFullYear\(\), now\.getUTCMonth\(\), now\.getUTCDate\(\)\)\);\s*\n?\s*\}/,
    );
    expect(body).toMatch(
      /function monthStartUtc\(now: Date\): Date \{\s*\n?\s*return new Date\(Date\.UTC\(now\.getUTCFullYear\(\), now\.getUTCMonth\(\), 1\)\);\s*\n?\s*\}/,
    );
    expect(body).toMatch(
      /function nextMonthStartUtc\(monthStart: Date\): Date \{\s*\n?\s*return new Date\(Date\.UTC\(monthStart\.getUTCFullYear\(\), monthStart\.getUTCMonth\(\) \+ 1, 1\)\);/,
    );
  });
