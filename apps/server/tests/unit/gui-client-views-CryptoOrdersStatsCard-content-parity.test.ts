...
describe('W482.A apps/gui-client/src/views/CryptoOrdersStatsCard.tsx content parity', () => {
  ...
  it("V-534.AI + V-534.AP framing pinned: 'V-534.AI — admin stats summary card for crypto orders.' + 'Surfaces the /v1/admin/crypto-orders/stats response (V-666.N + V-666.W) as a compact at-a-glance card: total orders, per-status counts, paid revenue per currency, and the avg time-to-paid KPI.' + 'V-534.AP — adds per-product revenue + ARPU breakdown using the V-666.AE fields. Pure read-only; no actions.'", () => {
    ...
    expect(body).toMatch(
      /\/\/ Surfaces the \/v1\/admin\/crypto-orders\/stats response \(V-666\.N \+\s*\n?\s*\/\/ V-666\.W\) as a compact at-a-glance card: total orders, per-status\s*\n?\s*\/\/ counts, paid revenue per currency, and the avg time-to-paid KPI\./,
    );
    expect(body).toMatch(
      /\/\/ V-534\.AP — adds per-product revenue \+ ARPU breakdown using the\s*\n?\s*\/\/ V-666\.AE fields\. Pure read-only; no actions\./,
    );
  });
  ...
  it('STATUS_LABELS 6-entry + STATUS_ORDER 6-tuple (same as V-534.U/.AH union order: pending/confirming/paid/failed/partial/cancelled)', () => {
    expect(body).toMatch(
      /const STATUS_LABELS: Record<AdminCryptoStatsStatus, string> = \{\s*\n?\s*pending: 'Pending',\s*\n?\s*confirming: 'Confirming',\s*\n?\s*paid: 'Paid',\s*\n?\s*failed: 'Failed',\s*\n?\s*partial: 'Partial',\s*\n?\s*cancelled: 'Cancelled',\s*\n?\s*\};/,
    );
    expect(body).toMatch(
      /const STATUS_ORDER: AdminCryptoStatsStatus\[\] = \[\s*\n?\s*'pending',\s*\n?\s*'confirming',\s*\n?\s*'paid',\s*\n?\s*'failed',\s*\n?\s*'partial',\s*\n?\s*'cancelled',\s*\n?\s*\];/,
    );
  });
  ...
  it("formatDurationMs piecewise: ms < 60_000 → `${Math.round(ms/1_000)}s` + ms < 60 * 60_000 → `${minutes.toFixed(1)}m` + else `${hours.toFixed(1)}h` — pinned so a 4h delay doesn't render '14400000s' because the >60min branch never fired", () => {
    expect(body).toMatch(
      /function formatDurationMs\(ms: number\): string \{\s*\n?\s*if \(ms < 60_000\) return `\$\{Math\.round\(ms \/ 1_000\)\.toString\(\)\}s`;\s*\n?\s*if \(ms < 60 \* 60_000\) \{\s*\n?\s*const minutes = ms \/ 60_000;\s*\n?\s*return `\$\{minutes\.toFixed\(1\)\}m`;\s*\n?\s*\}\s*\n?\s*const hours = ms \/ \(60 \* 60_000\);\s*\n?\s*return `\$\{hours\.toFixed\(1\)\}h`;\s*\n?\s*\}/,
    );
  });
  ...
  it("State-machine early returns: idle|loading → 'Loading stats…' empty state + error → ErrorBanner with retry-on-dismiss=>refetch + ready → full card; revenueEntries = Object.entries(paid_revenue_cents); productRows: paid_revenue_by_product ?? {} fallback + Object.values reduce total + paid_count_by_product?.[product] ?? 0 fallback + sort by total desc — pinned so highest-grossing tier stays at top", () => {
    expect(body).toMatch(
      /if \(state\.kind === 'idle' \|\| state\.kind === 'loading'\) \{\s*\n?\s*return \(\s*\n?\s*<div className="rounded-md border border-surface-divider bg-surface-inset p-4 text-sm text-ink-secondary">\s*\n?\s*Loading stats…\s*\n?\s*<\/div>\s*\n?\s*\);\s*\n?\s*\}/,
    );
    expect(body).toMatch(
      /if \(state\.kind === 'error'\) \{\s*\n?\s*return <ErrorBanner message=\{state\.message\} onDismiss=\{\(\) => void refetch\(\)\} \/>;\s*\n?\s*\}/,
    );
    expect(body).toMatch(
      /\/\/ V-534\.AP — sort product rows by total cents desc so the highest-\s*\n?\s*\/\/ grossing tier is first\. Multi-currency totals are summed for the\s*\n?\s*\/\/ sort key only; display preserves each currency separately\./,
    );
    expect(body).toMatch(
      /const productRows = Object\.entries\(data\.paid_revenue_by_product \?\? \{\}\)\s*\n?\s*\.map\(\(\[product, byCurrency\]\) => \{\s*\n?\s*const total = Object\.values\(byCurrency\)\.reduce\(\(a, b\) => a \+ b, 0\);\s*\n?\s*const count = data\.paid_count_by_product\?\.\[product\] \?\? 0;\s*\n?\s*return \{ product, byCurrency, total, count \};\s*\n?\s*\}\)\s*\n?\s*\.sort\(\(a, b\) => b\.total - a\.total\);/,
    );
  });
  ...
  it("4-stat top grid (Total + Paid + Pending + Avg time-to-pay) with avg_time_to_paid_ms !== null formatDurationMs delegation else '—' em-dash fallback for null", () => {
    expect(body).toMatch(
      /<p className="text-xs uppercase text-ink-secondary">Total orders<\/p>\s*\n?\s*<p className="text-xl font-semibold">\{data\.total\}<\/p>/,
    );
    expect(body).toMatch(
      /<p className="text-xs uppercase text-ink-secondary">Avg time-to-pay<\/p>\s*\n?\s*<p className="text-xl font-semibold">\s*\n?\s*\{data\.avg_time_to_paid_ms !== null \? formatDurationMs\(data\.avg_time_to_paid_ms\) : '—'\}\s*\n?\s*<\/p>/,
    );
  });
  ...
  it("By-status dl: STATUS_ORDER.map iteration with STATUS_LABELS[s] dt + data.by_status[s] dd; Paid revenue section: revenueEntries.length === 0 → 'No paid orders in scope.' fallback else <ul> with formatCents(cents, currency) per entry", () => {
    expect(body).toMatch(
      /\{STATUS_ORDER\.map\(\(s\) => \(\s*\n?\s*<div key=\{s\} className="flex items-center justify-between gap-2">\s*\n?\s*<dt className="text-ink-secondary">\{STATUS_LABELS\[s\]\}<\/dt>\s*\n?\s*<dd>\{data\.by_status\[s\]\}<\/dd>\s*\n?\s*<\/div>\s*\n?\s*\)\)\}/,
    );
    expect(body).toMatch(
      /\{revenueEntries\.length === 0 \? \(\s*\n?\s*<p className="text-sm text-ink-secondary">No paid orders in scope\.<\/p>\s*\n?\s*\) : \(\s*\n?\s*<ul className="flex flex-wrap gap-3 text-sm">\s*\n?\s*\{revenueEntries\.map\(\(\[currency, cents\]\) => \(\s*\n?\s*<li key=\{currency\} className="font-mono">\s*\n?\s*\{formatCents\(cents, currency\)\}/,
    );
  });
  ...
  it("Per-product table: data-testid='paid-by-product' container + only renders when productRows.length > 0 + Product/Count/Revenue 3-col header + per-row data-testid=`product-row-${product}` + multi-currency Object.entries map inside Revenue cell (each currency rendered separately even though sort key was summed)", () => {
    expect(body).toMatch(
      /\{productRows\.length > 0 && \(\s*\n?\s*<div data-testid="paid-by-product">/,
    );
    expect(body).toMatch(
      /<tr>\s*\n?\s*<th className="py-1 pr-3 font-medium">Product<\/th>\s*\n?\s*<th className="py-1 pr-3 font-medium">Count<\/th>\s*\n?\s*<th className="py-1 font-medium">Revenue<\/th>\s*\n?\s*<\/tr>/,
    );
    expect(body).toMatch(/data-testid=\{`product-row-\$\{row\.product\}`\}/);
    expect(body).toMatch(
      /\{Object\.entries\(row\.byCurrency\)\.map\(\(\[currency, cents\]\) => \(\s*\n?\s*<span key=\{currency\}>\{formatCents\(cents, currency\)\}<\/span>\s*\n?\s*\)\)\}/,
    );
  });
  ...
  it("truncated warning surface: 'Stats scanned {data.scanned} orders and stopped at the scan-window limit. Numbers may be undercounts; widen the analytics window if this becomes routine.' in text-status-warning — pinned so admin sees a clear undercount note when V-666.N scan window was hit", () => {
    expect(body).toMatch(
      /\{data\.truncated && \(\s*\n?\s*<p className="text-xs text-status-warning">\s*\n?\s*Stats scanned \{data\.scanned\} orders and stopped at the scan-window limit\. Numbers may be\s*\n?\s*undercounts; widen the analytics window if this becomes routine\.\s*\n?\s*<\/p>\s*\n?\s*\)\}/,
    );
  });
