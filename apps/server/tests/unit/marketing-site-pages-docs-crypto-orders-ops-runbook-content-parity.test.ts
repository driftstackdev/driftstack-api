...
describe('W513.C apps/marketing-site/src/pages/docs/crypto-orders-ops-runbook.astro content parity', () => {
  ...
  it("V-721 customer-facing-published rationale pinned — pinned so the V-721 anchor + admin-API-link + non-refundable-auditability rationale all survive. Re-enabled by slice 200 after verifying both V-721 + 'Customer-facing? Yes' comments exist at crypto-orders-ops-runbook.astro:4-12 with the matching shape", () => {
    expect(body).toMatch(
      /\/\/ V-721 — ops runbook for the crypto-orders surface\. Internal-\s*\/\/ facing page \(linked from \/docs\/admin-api\) that walks support\s*\/\/ through the common drill-downs/,
    );
    expect(body).toMatch(
      /\/\/ Customer-facing\? Yes — the page lives on the public docs site\s*\/\/ rather than the internal wiki\. Reasoning: founder wants the\s*\/\/ support runbook to be auditable by customers, since the\s*\/\/ non-refundable policy means every ops decision is consequential\./,
    );
  });
  ...
  it("payment_id reverse-lookup framing pinned: 'the admin list endpoint accepts an exact-match payment_id query param' + 'GET /v1/admin/crypto-orders?payment_id=np_abc123' + admin GUI 'Payment ID' filter input + distinct-from-fuzzy-search framing — pinned so the exact-match + fuzzy-distinction + admin-GUI-input survive (drift to claiming the search param walks payment_id would mislead support about the routing)", () => {
    expect(body).toMatch(
      /The fastest reverse-lookup path: the admin list endpoint\s*accepts an exact-match <code>payment_id<\/code> query param\./,
    );
    expect(body).toMatch(/GET \/v1\/admin\/crypto-orders\?payment_id=np_abc123/);
    expect(body).toMatch(
      /Available in the\s*admin GUI as the "Payment ID" filter input next to the search\s*box\. Distinct from the fuzzy <code>search<\/code> param \(which\s*walks order_id \/ product \/ customer_note\)/,
    );
  });
  ...
  it("Stuck-in-pending 4-step playbook + 60-min pay-window + sweep-expired endpoint + apply-ipn endpoint with payment_id + provider_status: 'finished' framing pinned — pinned so the 4-step playbook + 60-min-pay-window + 2-admin-endpoint (sweep-expired + apply-ipn) + provider_status finished commitment all survive (drift to a different pay-window would let support sweep customers prematurely; drift to renaming the admin endpoints would create marketing↔admin-route divergence)", () => {
    expect(body).toMatch(
      /Pay windows are ~60 minutes\. Orders past\s*that without an IPN are candidates for sweep\s*\(<code>POST \/v1\/admin\/crypto-orders\/sweep-expired<\/code>\)/,
    );
    expect(body).toMatch(
      /manually\s*replay the IPN:\s*<code>POST \/v1\/admin\/crypto-orders\/:id\/apply-ipn<\/code> with\s*the recorded <code>payment_id<\/code> \+\s*<code>provider_status: 'finished'<\/code>\./,
    );
  });
  ...
  it('Duplicate-charge idempotency and reconciliation framing pinned', () => {
    ...
    expect(body).toMatch(
      /Check the <code>idempotency-metrics<\/code> endpoint —\s*does the <code>replays<\/code> count match/,
    );
    expect(body).toMatch(
      /this is a customer-side\s*bug \(their integration sent two distinct Idempotency-Keys\s*for what they intended as one intent\)\. Crypto is non-\s*refundable; the resolution is to credit the customer's\s*next billing cycle, not refund\./,
    );
  });
  ...
  it("Timeline /events endpoint + 5 source tags pinned: GET /v1/admin/crypto-orders/:id/events + 'append-only event log oldest-first' + 'each event carries the destination status, an ISO-8601 timestamp, and a source tag — create, ipn, cancel, expired, or swept' — pinned so the events-endpoint + append-only-oldest-first + ISO-8601 + 5-source-tag enum (create/ipn/cancel/expired/swept) all survive (drift to dropping any source tag would shrink the audit surface)", () => {
    ...
    expect(body).toMatch(
      /Returns the order's append-only event log oldest-first\. Each\s*event carries the destination status, an ISO-8601 timestamp,\s*and a <code>source<\/code> tag — <code>create<\/code>,\s*<code>ipn<\/code>, <code>cancel<\/code>, <code>expired<\/code>,\s*or <code>swept<\/code>\./,
    );
  });
  ...
  it('Failed-order 3-cause framing pinned: NowPayments terminal failure (failed/expired/refunded) + pay window elapsed + admin sweep + crypto.order.failed reason field with ipn/expired/swept + customer-subscribable framing — pinned so the 3-cause + 3-reason-enum + subscribable status survive. 2026-05-22 — migration 0064 + bootstrap wire (ee725ba3) flipped the event LIVE; doc + pin flipped accordingly. Anti-drift: claiming the event is NOT subscribable would re-create the pre-0064 stale framing.', () => {
    expect(body).toMatch(
      /NowPayments reported a terminal failure\s*\(<code>failed<\/code> \/ <code>expired<\/code> \/\s*<code>refunded<\/code>\)/,
    );
    expect(body).toMatch(
      /Server-side, the <code>crypto\.order\.failed<\/code> event\s*emitted by these transitions carries a <code>reason<\/code>\s*field with one of <code>ipn<\/code> \/ <code>expired<\/code> \/\s*<code>swept<\/code>\./,
    );
  ...
  it('CSV export date-range and status-filter framing pinned', () => {
    expect(body).toMatch(
      /Scope the export to a date window with\s*<code>\?created_after<\/code> \+ <code>\?created_before<\/code>\s*\(ISO 8601\)\. Both work on the JSON list, the CSV endpoint, and\s*the admin GUI's From\/To inputs\. Combine with\s*<code>\?status=paid<\/code> for a nightly paid-only reconcile\./,
    );
    expect(body).toMatch(
