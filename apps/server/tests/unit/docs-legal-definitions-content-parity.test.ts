...
describe('W574.B /docs/legal/definitions.md content parity', () => {
  ...
  it('Header + Version-1.0 + 2026-05-07 + 4-bound-docs + Parties + Authorized-User binds Customer framing pinned', () => {
  ...
  it('Service-Platform + API + API-Key + GUI-Client + SDK + Session + Recording + Capture + Subscription tier-pricing framing pinned', () => {
    ...
    expect(body).toMatch(/under the `\/v1\/` path on Driftstack's API endpoint and documented at/);
    expect(body).toMatch(
    ...
    expect(body).toMatch(/Keys are scoped \(read, write, admin, gui_control\) and revocable\./);
    expect(body).toMatch(
      /\*\*"GUI Client"\*\* means the self-hosted Tauri-based desktop application/,
    );
    expect(body).toMatch(/distributed by Driftstack that connects to a Driftstack API server\./);
    expect(body).toMatch(
    ...
    expect(body).toMatch(
      /\*\*"Recording"\*\* means a Customer-controlled archive of polled frames/,
    );
    expect(body).toMatch(/captured during a Session\./);
    expect(body).toMatch(/Recordings exist only when Customer/);
    expect(body).toMatch(/explicitly enables recording for the Session\./);
    expect(body).toMatch(/\*\*"Capture"\*\* means a one-shot screenshot, DOM snapshot, or PDF/);
    expect(body).toMatch(/generated by the API's `capture` endpoint during a Session\./);
    expect(body).toMatch(
    ...
    expect(body).toMatch(/under one of Driftstack's published tiers \(Free, \$39\/mo, \$99\/mo,/);
    expect(body).toMatch(
