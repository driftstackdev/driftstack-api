...
describe('W436.C packages/api-types/src/crypto-orders.ts content parity', () => {
  ...
  it('V-666 framing pinned + 5 customer endpoints listed (POST checkout / GET list / GET one / PATCH note / POST cancel) + non-refundable rationale + cancel halts pending pay window but does NOT refund settled payment', () => {
    ...
    expect(body).toMatch(
      /\/\/ Crypto payments are non-refundable\. Cancellation halts a pending\s*\/\/ order's pay window but does NOT refund a settled payment\./,
    );
  });
  ...
  it('CryptoOrderStatus enum: 6 values (pending|confirming|paid|failed|partial|cancelled) in exact order', () => {
    expect(body).toMatch(
      /export const CryptoOrderStatusSchema = z\.enum\(\[\s*'pending',\s*'confirming',\s*'paid',\s*'failed',\s*'partial',\s*'cancelled',\s*\]\);/,
    );
  });
  ...
  it('V-666.AU CryptoOrderEventSource framing pinned: swept mapped to expired server-side before serialization; customer-facing surface only sees 4 sources (create|ipn|cancel|expired)', () => {
    ...
    expect(body).toMatch(
      /export const CryptoOrderEventSchema = z\.object\(\{\s*status: CryptoOrderStatusSchema,\s*at: z\.string\(\)\.describe\('ISO-8601 UTC timestamp of the transition\.'\),\s*source: CryptoOrderEventSourceSchema,\s*\}\);/,
    );
  });
  ...
  it('CreateCryptoCheckoutRequest: product is the shared PURCHASABLE_TIERS enum (V-924 — free and enterprise excluded, the same set as the Stripe sibling in billing.ts) + price_cents int positive max 1M + price_currency 3-letter uppercase ISO regex /^[A-Z]{3}$/', () => {
    ...
    expect(body).toMatch(
      /price_currency: z\s*\.string\(\)\s*\.length\(3\)\s*\.regex\(\/\^\[A-Z\]\{3\}\$\/, 'price_currency must be a 3-letter uppercase ISO code'\),\s*\}\);/,
    );
  });
  ...
  it('CreateCryptoCheckoutResponse: order_id + product + price_cents + price_currency + status + provider stub|nowpayments + payment_address nullable + pay_currency nullable + pay_amount nullable (the crypto amount to send; returned by the route + documented in api/billing-crypto) + created_at; stub is the support-assisted fallback', () => {
    expect(body).toMatch(
      /export const CreateCryptoCheckoutResponseSchema = z\.object\(\{\s*order_id: z\.string\(\),\s*product: z\.string\(\),\s*price_cents: z\.number\(\)\.int\(\),\s*price_currency: z\.string\(\),\s*status: CryptoOrderStatusSchema,\s*\/\*\* Payment rail used for this checkout; `stub` is the support-assisted fallback\. \*\/\s*provider: z\.enum\(\['stub', 'nowpayments'\]\),\s*payment_address: z\.string\(\)\.nullable\(\),\s*pay_currency: z\.string\(\)\.nullable\(\),[\s\S]*?pay_amount: z\.number\(\)\.nullable\(\),\s*created_at: z\.string\(\),\s*\}\);/,
    );
  });
  ...
  it('V-666.BR ListCryptoOrdersQuery + V-666.BU forward cursor (treat as opaque) + V-666.BX created_after/before half-open window ISO 8601; ListCryptoOrdersResponse: orders + next_cursor nullable optional', () => {
    ...
    expect(body).toMatch(
      /export const ListCryptoOrdersQuerySchema = z\.object\(\{\s*limit: z\.number\(\)\.int\(\)\.min\(1\)\.max\(100\)\.optional\(\),\s*status: CryptoOrderStatusSchema\.optional\(\),\s*[\s\S]*?cursor: z\.string\(\)\.min\(1\)\.max\(512\)\.optional\(\),\s*\/\*\* V-666\.BX — half-open window on created_at; ISO 8601 strings\. \*\/\s*created_after: z\.string\(\)\.datetime\(\)\.optional\(\),\s*created_before: z\.string\(\)\.datetime\(\)\.optional\(\),\s*\}\);/,
    );
    expect(body).toMatch(
  ...
  it('UpdateCryptoOrderNote: customer_note max 500 nullable; CancelCryptoOrderResponse = CryptoOrderEnvelopeSchema alias', () => {
    expect(body).toMatch(
      /export const UpdateCryptoOrderNoteRequestSchema = z\.object\(\{\s*customer_note: z\.string\(\)\.max\(500\)\.nullable\(\),\s*\}\);/,
    );
    expect(body).toMatch(
  ...
  it('CryptoQuoteRequest (product + optional 3-letter uppercase price_currency) + exact pricing-only response', () => {
    expect(body).toMatch(
      ...
      /export const CryptoQuoteRequestSchema = z\.object\(\{[\s\S]*?product: z\s*\.enum\(PURCHASABLE_TIERS[\s\S]*?price_currency: z\s*\.string\(\)\s*\.length\(3\)\s*\.regex\(\/\^\[A-Z\]\{3\}\$\/\)\s*\.optional\(\),\s*\}\);/,
    );
    expect(body).toMatch(
      /export const CryptoQuoteResponseSchema = z\.object\(\{\s*product: z\.string\(\),\s*price_cents: z\.number\(\)\.int\(\)\.positive\(\),\s*price_currency: z\.string\(\),\s*\}\);/,
    );
  ...
  it('V-666.AY AdminCryptoOrderEnvelope framing pinned: extends customer envelope with account_id nullable (pre-signup checkouts) + internal_note (admin-only, never returned on customer surface)', () => {
    ...
    expect(body).toMatch(
      /export const AdminCryptoOrderEnvelopeSchema = CryptoOrderEnvelopeSchema\.extend\(\{\s*\/\*\* Owning account; null for pre-signup checkouts\. \*\/\s*account_id: z\.string\(\)\.nullable\(\),\s*\/\*\* Admin-only operations note\. Never returned on the customer surface\. \*\/\s*internal_note: z\.string\(\)\.nullable\(\),\s*\}\);/,
    );
  });
  ...
  it('AdminListCryptoOrdersResponse: orders[] + next_cursor nullable; AdminCryptoOrderEventsResponse: events with 5-value internal source including "swept" variant', () => {
    expect(body).toMatch(
      /export const AdminListCryptoOrdersResponseSchema = z\.object\(\{\s*orders: z\.array\(AdminCryptoOrderEnvelopeSchema\),\s*next_cursor: z\.string\(\)\.nullable\(\),\s*\}\);/,
    );
    expect(body).toMatch(
      /export const AdminCryptoOrderEventsResponseSchema = z\.object\(\{\s*events: z\.array\(\s*z\.object\(\{\s*status: CryptoOrderStatusSchema,\s*at: z\.string\(\),\s*\/\*\* Admin source includes the internal 'swept' variant\. \*\/\s*source: z\.enum\(\['create', 'ipn', 'cancel', 'expired', 'swept'\]\),\s*\}\),\s*\),\s*\}\);/,
    );
  });
  ...
  it('AdminUpdateInternalNote (max 2000 nullable) + AdminSweepExpired (older_than_hours int 1..8760 optional + limit int 1..500 optional) → response (expired count + capped bool)', () => {
    expect(body).toMatch(
      /export const AdminUpdateInternalNoteRequestSchema = z\.object\(\{\s*internal_note: z\.string\(\)\.max\(2000\)\.nullable\(\),\s*\}\);/,
    );
    expect(body).toMatch(
      /export const AdminSweepExpiredRequestSchema = z\.object\(\{\s*older_than_hours: z\.number\(\)\.int\(\)\.min\(1\)\.max\(8760\)\.optional\(\),\s*limit: z\.number\(\)\.int\(\)\.min\(1\)\.max\(500\)\.optional\(\),\s*\}\);/,
    );
    expect(body).toMatch(
      /export const AdminSweepExpiredResponseSchema = z\.object\(\{\s*expired: z\.number\(\)\.int\(\)\.nonnegative\(\),\s*capped: z\.boolean\(\),\s*\}\);/,
    );
  });
  ...
  it('Admin daily breakdown (rows: UTC YYYY-MM-DD date + status + count; truncated) + pending age (buckets + pending_value_cents + total + truncated + scanned; NOT total_pending, which the route never returned) + apply IPN (provider_status + payment_id) + idempotency metrics (replays + first_writes + body_mismatches) + stats (total + by_status + revenue + avg/sample + by_product + truncated + scanned)', () => {
    expect(body).toMatch(
      /export const AdminCryptoDailyBreakdownResponseSchema = z\.object\(\{\s*rows: z\.array\(\s*z\.object\(\{\s*date: z\.string\(\)\.describe\('UTC YYYY-MM-DD\.'\),\s*status: CryptoOrderStatusSchema,\s*count: z\.number\(\)\.int\(\)\.nonnegative\(\),\s*\}\),\s*\),\s*truncated: z\.boolean\(\),\s*\}\);/,
    );
    expect(body).toMatch(
      /export const AdminCryptoPendingAgeResponseSchema = z\.object\(\{\s*buckets: z\.record\(z\.string\(\), z\.number\(\)\.int\(\)\.nonnegative\(\)\),[\s\S]*?pending_value_cents: z\.record\([\s\S]*?total: z\.number\(\)\.int\(\)\.nonnegative\(\),[\s\S]*?truncated: z\.boolean\(\),[\s\S]*?scanned: z\.number\(\)\.int\(\)\.nonnegative\(\),\s*\}\);/,
    );
    expect(body).toMatch(
      ...
      // route's own limits.
      /export const AdminApplyIpnRequestSchema = z\.object\(\{\s*provider_status: z\.string\(\)\.min\(1\)\.max\(64\),\s*payment_id: z\.string\(\)\.min\(1\)\.max\(128\),\s*\}\);/,
    );
    expect(body).toMatch(
      /export const AdminIdempotencyMetricsResponseSchema = z\.object\(\{\s*replays: z\.number\(\)\.int\(\)\.nonnegative\(\),\s*first_writes: z\.number\(\)\.int\(\)\.nonnegative\(\),\s*body_mismatches: z\.number\(\)\.int\(\)\.nonnegative\(\),\s*\}\);/,
    );
    expect(body).toMatch(
      /export const AdminCryptoStatsResponseSchema = z\.object\(\{\s*total: z\.number\(\)\.int\(\)\.nonnegative\(\),\s*by_status: z\.record\(z\.string\(\), z\.number\(\)\.int\(\)\.nonnegative\(\)\),\s*paid_revenue_cents: z\.record\(z\.string\(\), z\.number\(\)\.int\(\)\.nonnegative\(\)\),\s*avg_time_to_paid_ms: z\.number\(\)\.int\(\)\.nullable\(\),\s*paid_sample: z\.number\(\)\.int\(\)\.nonnegative\(\),/,
    );
    expect(body).toMatch(
      /truncated: z\.boolean\(\),\s*scanned: z\.number\(\)\.int\(\)\.nonnegative\(\),\s*\}\);/,
    );
  });
