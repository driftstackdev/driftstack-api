...
describe('W617 apps/gui-client/src-tauri/ content parity', () => {
  ...
  it('Cargo.toml: driftstack-gui crate 0.1.5 MIT non-publish + lib triplet (staticlib/cdylib/rlib) + Tauri 2.0 + plugins + zeroize process-memory control keys + V-241 keyring + custom-protocol pinned', () => {
    const body = read(T('Cargo.toml'));
    expect(body).toMatch(/^\[package\]$/m);
    expect(body).toMatch(/^name = "driftstack-gui"$/m);
    expect(body).toMatch(/^version = "0\.1\.5"$/m);
  ...
  it('tauri.conf.json: productName=Driftstack 0.1.5 + identifier dev.driftstack.gui + frontendDist ../dist + devUrl localhost:1420 + 1 main window (1280×800 / min 960×600 / Overlay titleBarStyle / hiddenTitle / #0b0f14) + 5-target bundle (app/dmg/nsis/appimage/deb) DeveloperTool + macOS minimumSystemVersion 12.0 + Entitlements.plist + V-243 updater endpoint github releases latest + $TAURI_UPDATER_PUBKEY placeholder + V-328 deep-link scheme driftstack desktop-only pinned', () => {
    ...
    expect(body).toMatch(/"version": "0\.1\.5"/);
