...
describe('W392.C apps/server/src/lib/errors-helpers.ts content parity', () => {
  ...
  it('Module framing: helpers next to errors.ts so services skip auth-service import', () => {
    expect(body).toMatch(
      /Helpers that need the AccountContext type but live next to errors\.ts so\s*\/\/\s*services can import without pulling the auth service/,
    );
  });
  ...
  it('V-174 + V-481 scope-check framing pinned (exact / customer-only alias / broad-satisfies-granular)', () => {
    ...
    expect(body).toMatch(
      /2\. V-174 legacy customer alias — `'admin'`-scoped keys satisfy\s*\*\s*`'account_owner'` so pre-split customer automation keeps its own-account\s*\*\s*authority\. The expired migration bridge to\s*\*\s*`'driftstack_internal_admin'` is deliberately closed: only the exact\s*\*\s*staff scope can authorize cross-account operations/,
    );
    expect(body).toMatch(
      /3\. V-481 broad-satisfies-granular — when the required scope is\s*\*\s*granular \(`read:sessions`, `admin:profiles`, etc\.\), the key's\s*\*\s*broad scopes can satisfy it on the same verb:/,
    );
  });

  it('V-481 verb-table framing: read/write/admin granular each satisfied by matching broad or account_owner', () => {
    expect(body).toMatch(
      /required `read:X` is satisfied by any of `read`,\s*\*\s*`account_owner` \(read implied by full account access\)/,
    );
  ...
  it('"granular do NOT satisfy broad" framing pinned (narrow keys stay narrow)', () => {
    expect(body).toMatch(
      /Granular scopes do NOT satisfy broad checks — a key with\s*\*\s*`read:sessions` cannot pass `requireScope\('read'\)`\. That's\s*\*\s*the point of granular scoping; narrow keys stay narrow/,
    );
  });

  it('Mirrored-in-auth-service framing pinned (services/auth.ts::requireScope same predicate)', () => {
    expect(body).toMatch(
      /Mirrored in `services\/auth\.ts::requireScope` \(kept in sync — same\s*\*\s*predicate evaluated at both call sites\)/,
    );
  });

  it('requireScope: throws ForbiddenError with "requires X scope" detail', () => {
    expect(body).toMatch(
      /export function requireScope\(ctx: AccountContext, required: ApiKeyScope\): void \{\s*if \(hasScope\(ctx, required\)\) return;\s*throw new ForbiddenError\(`This action requires the "\$\{required\}" scope\.`\);/,
    );
  });

  it('hasScope: pure predicate keeps the V-174 alias customer-only before the V-481 verb-table', () => {
    expect(body).toMatch(
      /V-481 — pure predicate version of \{@link requireScope\}\. Returns\s*\*\s*true iff the key satisfies the required scope/,
    );
    ...
    expect(body).toMatch(
      /if \(required === 'account_owner' && scopes\.includes\('admin'\)\) \{\s*return true;\s*\}/,
    );
  ...
  it('hasScope: verb switch — read/write/admin each → includes(matching broad) || includes(account_owner)', () => {
    expect(body).toMatch(
      /case 'read':\s*return scopes\.includes\('read'\) \|\| scopes\.includes\('account_owner'\);/,
    );
    expect(body).toMatch(
      /case 'write':\s*return scopes\.includes\('write'\) \|\| scopes\.includes\('account_owner'\);/,
    );
    expect(body).toMatch(
      /case 'admin':\s*return scopes\.includes\('admin'\) \|\| scopes\.includes\('account_owner'\);/,
    );
  });

  it('hasScope: exhaustive default branch with _exhaustive: never assertion', () => {
    expect(body).toMatch(
      /default: \{\s*const _exhaustive: never = granular\.verb;\s*return _exhaustive;\s*\}/,
    );
  });

  it('V-485 requireTierFeature framing + the all-three-boolean-features-gated note (V-763 corrected this from only-aiAgent)', () => {
    expect(body).toMatch(
      /V-485 — per-tier feature guard\. Throws `ForbiddenError` when the\s*\*\s*given tier does NOT have the requested boolean feature enabled/,
    );
    expect(body).toMatch(
      /The single guard call replaces\s*\*\s*`if \(tier === 'X' \|\| tier === 'Y'\) throw …` style scattered\s*\*\s*conditionals — when a tier's feature row in\s*\*\s*`packages\/api-types\/src\/common\.ts:TIER_FEATURES` flips, every\s*\*\s*call site picks it up automatically/,
    );
  ...
  it('requireTierFeature: signature + TIER_FEATURES[tier][feature] check + ForbiddenError detail', () => {
    expect(body).toMatch(
      /export function requireTierFeature\(tier: AccountTier, feature: TierBooleanFeature\): void \{\s*if \(TIER_FEATURES\[tier\]\[feature\]\) return;\s*throw new ForbiddenError\(\s*`The "\$\{feature\}" feature is not available on the "\$\{tier\}" tier\. ` \+\s*`Upgrade to a tier that includes this feature\.`,/,
    );
  });
