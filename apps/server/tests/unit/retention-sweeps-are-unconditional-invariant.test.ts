...
/** Registrations that arm a recurring retention / lifecycle job. */
const RETENTION_JOB_RE = /register\w*(?:Sweep|Purge|Reap|Prune)\w*Job\(/;
...
function registrations(): Registration[] {
  ...
  lines.forEach((line, i) => {
    if (!RETENTION_JOB_RE.test(line)) return;
    const name = /register(\w+)\(/.exec(line)?.[1] ?? line.trim();
    out.push({ line: i + 1, name, indent: line.length - line.trimStart().length });
  });
  return out;
}
...
describe('retention and lifecycle sweeps are wired unconditionally', () => {
  ...
  it('CRITICAL the scan found the retention registrations. An empty scan would make the check below vacuously true — and the failure this guards against is itself a silent absence, so a silent scan failure would be doubly useless.', () => {
    ...
    expect(
      found.map((r) => r.name),
      'the account-deletion purge must be among them',
    ).toContain('AccountDeletionPurgeJob');
    expect(
      found.map((r) => r.name),
      'so must the profile trash purge',
    ).toContain('ProfileTrashPurgeJob');
  });
