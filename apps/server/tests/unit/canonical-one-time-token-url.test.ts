  ...
  it('URL-encodes the token without corrupting existing query or fragment state', () => {
    expect(
      canonicalOneTimeTokenUrl(
        'https://app.driftstack.io/auth/magic-link?campaign=launch#continue',
        'token with + / & ?',
      ),
    ).toBe(
      'https://app.driftstack.io/auth/magic-link/?campaign=launch&token=token+with+%2B+%2F+%26+%3F#continue',
    );
  });

  it('replaces an operator-supplied token parameter instead of duplicating it', () => {
    const result = canonicalOneTimeTokenUrl(
      'https://app.driftstack.io/reset-password?token=stale&campaign=launch',
      'fresh',
    );
    expect(result).toBe('https://app.driftstack.io/reset-password/?token=fresh&campaign=launch');
