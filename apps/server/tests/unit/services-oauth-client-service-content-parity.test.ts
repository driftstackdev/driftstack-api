...
describe('services/oauth-client-service content parity', () => {
  ...
  it("V-667.C implementation framing pinned: 'OAuth-client service implementation. Composes the 2 repos (oauth-links + pending-links) + a minimal accounts lookup + a token generator to fulfil the founder-verdict-locked flow.' + 3-verdict catalog. — pinned so the V-667.C anchor + 2-repo + accounts-lookup + token-generator composition + 3-verdict catalog (60-min collision + IDP-revocation marker + avatar-source-enum-not-re-pull-every-login) all stay documented", () => {
    expect(body).toMatch(
      /\/\/ V-667\.C — OAuth-client service implementation\. Composes the 2 repos\s*\n?\s*\/\/ \(oauth-links \+ pending-links\) \+ a minimal accounts lookup \+ a token\s*\n?\s*\/\/ generator to fulfil the founder-verdict-locked flow:/,
    );
    expect(body).toMatch(
      /\/\/ {3}Verdict 1: existing-email collision → MERGE-WITH-VERIFICATION\s*\n?\s*\/\/ {5}\(60-min single-use token sent to existing account's email\)/,
    );
    expect(body).toMatch(/\/\/ {3}Verdict 2: IDP-revocation marker on next-login-failure/);
    expect(body).toMatch(
      /\/\/ {3}Verdict 3: avatar\/name sync first-link-only \+ user-overridable\s*\n?\s*\/\/ {5}\(driven by accounts\.avatar_source enum, NOT by re-pull every\s*\n?\s*\/\/ {5}login\)/,
    );
  });
  ...
  it("AccountsLookup 2-method dep interface pinned: findIdByEmail (case-insensitive) + createFromIdp (sets avatar_source='idp' when avatarUrl provided, Verdict 3). + 'Insert a new account from an IDP signup. Sets avatar_source to idp when avatarUrl is provided + name from IDP per Verdict 3.' framing — pinned so the Verdict-3 avatar_source='idp' contract stays explicit (drift to dropping the avatar_source set would break the user-override-wins precedence)", () => {
    ...
    expect(body).toMatch(
      /\* Insert a new account from an IDP signup\. Sets avatar_source\s*\n?\s*\*\s+to 'idp' when avatarUrl is provided \+ name from IDP per\s*\n?\s*\*\s+Verdict 3\. Returns the new account id\./,
    );
    expect(body).toMatch(
      /createFromIdp\(args: \{\s*\n?\s*email: string;\s*\n?\s*name: string \| null;\s*\n?\s*avatarUrl: string \| null;\s*\n?\s*\}\): Promise<string>;/,
    );
  });
  ...
  it("OAuthPendingMailer fire-and-forget framing pinned: 'Send the Verdict-1 verify-merge email with the plaintext token in the URL. Fire-and-forget; failures are surfaced via the passed-in logger but not awaited (matches the rest of the email-service posture).' — pinned so the fire-and-forget contract + logger-not-throw + matches-rest-of-email-service rationale all stay documented (drift to awaiting would couple link-or-create latency to Postmark delivery latency)", () => {
    ...
    expect(body).toMatch(
      /\* Send the Verdict-1 verify-merge email with the plaintext token\s*\n?\s*\*\s+in the URL\. Fire-and-forget; failures are surfaced via the\s*\n?\s*\*\s+passed-in logger but not awaited \(matches the rest of the\s*\n?\s*\*\s+email-service posture\)\./,
    );
    expect(body).toMatch(
      /sendVerifyMergeEmail\(args: \{\s*\n?\s*to: string;\s*\n?\s*provider: 'google' \| 'github';\s*\n?\s*plaintextToken: string;\s*\n?\s*expiresAt: Date;\s*\n?\s*\}\): Promise<void>;/,
    );
  });
  ...
  it('OAuthClientServiceDeps 4-field shape pinned: links + pending + accounts + mailer. Drift to dropping a dep would break the 3-step linkOrCreateAccount dispatch that depends on all 4 collaborators', () => {
    expect(body).toMatch(
      /export interface OAuthClientServiceDeps \{\s*\n?\s*links: OAuthLinksRepo;\s*\n?\s*pending: OAuthPendingLinksRepo;\s*\n?\s*accounts: AccountsLookup;\s*\n?\s*mailer: OAuthPendingMailer;\s*\n?\s*\}/,
    );
  });
  ...
  it('linkOrCreateAccount 3-step dispatch pinned: Step 1 fast-path findByProviderSub + Verdict-2 fork on lastRevokedAt + signed-in-existing-link on success. Drift to checking email-collision BEFORE existing-link would re-prompt verify-merge on every login for already-linked accounts (UX regression + extra email spam)', () => {
    expect(body).toMatch(
      /\/\/ Step 1 — fast path: existing link for this IDP identity\?\s*\n?\s*const existing = await this\.deps\.links\.findByProviderSub\(args\.provider, args\.providerSub\);/,
    );
    expect(body).toMatch(
      /\/\/ Verdict 2 fork — previously marked revoked\. Don't auto-sign-in;\s*\n?\s*\/\/ the route prompts the user to re-link or fall back to password\.\s*\n?\s*if \(existing\.lastRevokedAt !== null\) \{/,
    );
    expect(body).toMatch(
      /return \{\s*\n?\s*kind: 'signed-in-existing-link',\s*\n?\s*accountId: existing\.accountId,\s*\n?\s*linkId: existing\.id,\s*\n?\s*\};/,
    );
  });
  ...
  it("Step 2 email-collision pinned: findIdByEmail → if non-null, generate plaintext + sha256-hash + 60-min expiresAt + insertPending + fire-and-forget mailer + return collision-pending-verification. + 'Fire-and-forget per email-service posture; route-layer logs any send error via Sentry-pinned warn handler.' — pinned so the void-the-mailer + Sentry-warn-handler-route-logs contract stays documented", () => {
    expect(body).toMatch(
      /\/\/ Step 2 — no link\. Is there an existing account with this email\?\s*\n?\s*const collidingAccountId = await this\.deps\.accounts\.findIdByEmail\(args\.email\);/,
    );
    expect(body).toMatch(
      /\/\/ Verdict 1 — collision\. Issue a pending-link token, send email\.\s*\n?\s*const plaintext = generatePlaintextToken\(\);\s*\n?\s*const tokenHash = sha256Hex\(plaintext\);\s*\n?\s*const expiresAt = new Date\(now\.getTime\(\) \+ PENDING_TTL_MS\);/,
    );
    expect(body).toMatch(
      /\/\/ Fire-and-forget per email-service posture; route-layer logs\s*\n?\s*\/\/ any send error via Sentry-pinned warn handler\.\s*\n?\s*void this\.deps\.mailer\.sendVerifyMergeEmail\(\{/,
    );
    expect(body).toMatch(
      /return \{\s*\n?\s*kind: 'collision-pending-verification',\s*\n?\s*pendingLinkId: pending\.id,\s*\n?\s*expiresAt,\s*\n?\s*\};/,
    );
  });
  ...
  it('Step 3 create-new-account pinned: no link + no collision → createFromIdp + insertLink + markLoginAt + return created-new-account. Drift to skipping markLoginAt on the new-account path would leave lastLoginAt NULL after the very first sign-in (defeats the audit-trail purpose)', () => {
    expect(body).toMatch(
      /\/\/ Step 3 — no link, no colliding account → create new from IDP\.\s*\n?\s*const newAccountId = await this\.deps\.accounts\.createFromIdp\(\{/,
    );
    expect(body).toMatch(
      /const link = await this\.deps\.links\.insertLink\(\{\s*\n?\s*accountId: newAccountId,/,
    );
    expect(body).toMatch(/await this\.deps\.links\.markLoginAt\(link\.id, now\);/);
    expect(body).toMatch(
      /return \{\s*\n?\s*kind: 'created-new-account',\s*\n?\s*accountId: newAccountId,\s*\n?\s*linkId: link\.id,\s*\n?\s*\};/,
    );
  });
  ...
  it("generatePlaintextToken 32-byte → 64-hex framing pinned: 'Same shape as the existing auth-flow plaintext tokens (email_verify, magic_link, password_reset) so the per-token entropy ceiling stays consistent across the auth surface.' — pinned so the cross-token-shape uniformity contract + the existing 3-auth-flow-token cross-reference (email_verify + magic_link + password_reset) all stay documented", () => {
    expect(body).toMatch(
      /\/\*\* 32 random bytes → 64 hex chars\. Same shape as the existing\s*\n?\s*\*\s+auth-flow plaintext tokens \(email_verify, magic_link, password_reset\)\s*\n?\s*\*\s+so the per-token entropy ceiling stays consistent across the auth\s*\n?\s*\*\s+surface\. \*\/\s*\n?\s*function generatePlaintextToken\(\): string \{\s*\n?\s*return randomBytes\(32\)\.toString\('hex'\);\s*\n?\s*\}/,
    );
  });
  ...
  it('sha256Hex helper pinned: createHash(sha256).update(value).digest(hex). Drift to a weaker hash (md5/sha1) would let attackers brute-force the pending-link token store; drift to a different encoding (base64) would break the at-rest comparison contract', () => {
    expect(body).toMatch(
      /function sha256Hex\(value: string\): string \{\s*\n?\s*return createHash\('sha256'\)\.update\(value\)\.digest\('hex'\);\s*\n?\s*\}/,
    );
  });
});
