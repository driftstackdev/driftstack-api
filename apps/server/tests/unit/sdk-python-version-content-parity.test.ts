...
//     __future__).
//   • __version__ = "0.3.0" — bump in lockstep with package.json /
...
describe('W587.D packages/sdk-python/src/driftstack/_version.py content parity', () => {
  ...
  it('__version__ = "0.3.0" pinned (drift here = unintentional version bump; expected to change only via lockstep release)', () => {
    expect(body).toMatch(/^from __future__ import annotations$/m);
    expect(body).toMatch(/^__version__ = "0\.3\.0"$/m);
  });
