...
describe('W436.B packages/api-types/src/accounts.ts content parity', () => {
  ...
  it('AccountStatus enum (active|suspended|deleted) + AccountSchema 7-field shape (id + email + name nullable + tier + status + 2 timestamps)', () => {
    ...
    expect(body).toMatch(
      /export const AccountSchema = z\.object\(\{\s*id: AccountIdSchema,\s*email: z\.string\(\)\.email\(\),\s*name: z\.string\(\)\.nullable\(\),\s*tier: AccountTierSchema,\s*status: AccountStatusSchema,\s*created_at: Iso8601Schema,\s*updated_at: Iso8601Schema,\s*\}\);/,
    );
  });
  ...
  it('OptOutableEmailEvent enum: 6 values pinned (signup-welcome / session-failed-first / V-304a session-success-first / tier-changed / billing-receipt / V-304b billing-renewal-reminder Stripe invoice.upcoming). Discrete pins (no long \\s* chain) per the catastrophic-backtracking lesson.', () => {
    ...
    expect(body).toMatch(
      /export const EmailPreferenceSchema = z\.object\(\{\s*event_type: OptOutableEmailEventSchema,\s*opted_in: z\.boolean\(\),\s*\}\);/,
    );
  });
  ...
  it('V-352 UpdateAccountMe framing + V-298a slug regex (3..32 lowercase a-z+0-9+hyphen no leading/trailing/consecutive) + V-298b region enum us|eu|apac; UpdateAccountMeRequest at-least-one-field refine + IANA timezone Intl-validity refine (2026-06-03: replaced a regex that wrongly rejected single-segment zones like UTC/GMT/Japan AND wrongly accepted non-zones like Foo/Bar)', () => {
    ...
    expect(body).toMatch(
      /timezone: z\s*\.string\(\)\s*\.trim\(\)\s*\.min\(1\)\s*\.max\(64\)\s*\.refine\(isValidIanaTimeZone, \{\s*message: 'Must be a valid IANA timezone name like "Europe\/Amsterdam" or "UTC"\.',\s*\}\)/,
    );
    ...
    expect(body).toMatch(
      /function isValidIanaTimeZone\(tz: string\): boolean \{\s*try \{\s*new Intl\.DateTimeFormat\('en-US', \{ timeZone: tz \}\);\s*return true;\s*\} catch \{\s*return false;\s*\}\s*\}/,
    );
    expect(body).toMatch(
  ...
  it('V-352b avatar upload framing pinned: 2 MiB cap + base64 inline + 3 content-types + R2 public-snapshot bucket + V-294 disclosure scope updated atomically; AVATAR_MAX_BYTES + AVATAR_ALLOWED_CONTENT_TYPES + UploadAvatarRequest base64 size cap (Math.ceil((MAX*4)/3)+4)', () => {
    ...
    expect(body).toMatch(
      /export const UploadAvatarRequestSchema = z\.object\(\{\s*content_type: AvatarContentTypeSchema,\s*data_base64: z\s*\.string\(\)\s*\.min\(4\)\s*\.max\(Math\.ceil\(\(AVATAR_MAX_BYTES \* 4\) \/ 3\) \+ 4\)\s*\.regex\(\/\^\[A-Za-z0-9\+\/=\]\+\$\/, 'Must be base64-encoded\.'\),\s*\}\);/,
    );
  });
  ...
  it('V-353b MFA enrollment: status (enrolled + 2 nullable timestamps + unused_recovery_codes nonneg) + start (otpauth + secret_base32 + SHA1 + 6 digits + 30s period literals) + complete (6-digit code regex; response 10 recovery codes) + regenerate recovery codes (10)', () => {
    expect(body).toMatch(
      /export const MfaStatusResponseSchema = z\.object\(\{\s*enrolled: z\.boolean\(\),\s*enrolled_at: Iso8601Schema\.nullable\(\),\s*last_used_at: Iso8601Schema\.nullable\(\),\s*unused_recovery_codes: z\.number\(\)\.int\(\)\.nonnegative\(\),\s*\}\);/,
    );
    expect(body).toMatch(
    ...
    expect(body).toMatch(
      /export const RegenerateMfaRecoveryCodesResponseSchema = z\.object\(\{\s*recovery_codes: z\.array\(z\.string\(\)\)\.length\(10\),\s*\}\);/,
    );
  });
  ...
  it('V-216 AccountAuditAction enum framing pinned: closed enum of customer-visible actions; adding new event type is Class A schema migration (additive enum value)', () => {
    expect(body).toMatch(
      /\*\s*Closed enum of customer-visible audit actions\. Adding a new event\s*\*\s*type is a Class A schema migration \(additive enum value\)\./,
    );
  });
  ...
  it('AccountAuditActorType (customer|system|staff) + AccountAuditEntry shape: id uuid + account_id + actor_type + actor_account_id/key_id nullable + action + target_resource_id nullable + payload record nullable + ip_address nullable + user_agent nullable + timestamp', () => {
    ...
    expect(body).toMatch(
      /export const AccountAuditEntrySchema = z\.object\(\{\s*id: z\.string\(\)\.uuid\(\),\s*account_id: z\.string\(\),\s*actor_type: AccountAuditActorTypeSchema,\s*actor_account_id: z\.string\(\)\.nullable\(\),\s*actor_key_id: z\.string\(\)\.nullable\(\),\s*action: AccountAuditActionSchema,\s*target_resource_id: z\.string\(\)\.nullable\(\),\s*payload: z\.record\(z\.unknown\(\)\)\.nullable\(\),\s*ip_address: z\.string\(\)\.nullable\(\),\s*user_agent: z\.string\(\)\.nullable\(\),\s*timestamp: Iso8601Schema,\s*\}\);/,
    );
  });
  ...
  it('V-484 ListAccountAuditLogQuery: limit + cursor + action + from/to coerce.date (handles YYYY-MM-DD + full ISO) + actor_type + target_resource_id 1..200', () => {
    ...
    expect(body).toMatch(
      /export const ListAccountAuditLogResponseSchema = z\.object\(\{\s*data: z\.array\(AccountAuditEntrySchema\),\s*next_cursor: z\.string\(\)\.nullable\(\),\s*\}\);/,
    );
  });
  ...
  it('V-297 GDPR Article 20 portability framing pinned: format=json returns shape / format=csv text/csv not surfaced through typed SDK methods (browsers hit directly); 10,000-row truncated ceiling; export query default json; export response generated_at + account_id + row_count + truncated + data', () => {
    ...
    expect(body).toMatch(
      /\/\*\* True when the row count hit the 10,000-row server-side ceiling\s*\*\s*and older entries were not included\. Customers needing the full\s*\*\s*history should narrow the date window or use the paginated\s*\*\s*\/v1\/account\/audit-log read endpoint\. \*\//,
    );
    expect(body).toMatch(
      /export const ExportAccountAuditLogResponseSchema = z\.object\(\{\s*generated_at: Iso8601Schema,\s*account_id: z\.string\(\),\s*row_count: z\.number\(\)\.int\(\)\.nonnegative\(\),/,
    );
    expect(body).toMatch(
      /truncated: z\.boolean\(\),\s*data: z\.array\(AccountAuditEntrySchema\),\s*\}\);/,
    );
  });
  ...
  it('V-219 RateLimitBucket: bucket_key enum (global|sessions:create|agent_sessions:message|agent_sessions:input_event — the customer-read GET /v1/account/rate-limits returns all four) + capacity positive int + refill_per_second positive + source enum (tier_default|override) + override_expires_at nullable; GetAccountRateLimitsResponse: tier + buckets', () => {
    ...
    expect(body).toMatch(
      /\*\s*`'tier_default'` when the value comes from the locked tier table;\s*\*\s*`'override'` when an admin-set override is currently in effect\./,
    );
    expect(body).toMatch(/source: z\.enum\(\['tier_default', 'override'\]\),/);
    expect(body).toMatch(
      /export const GetAccountRateLimitsResponseSchema = z\.object\(\{\s*tier: AccountTierSchema,\s*buckets: z\.array\(RateLimitBucketSchema\),\s*\}\);/,
    );
  });
