...
describe('W480.A apps/gui-client/src/views/CryptoReceiptView.tsx content parity', () => {
  ...
  it("V-534.AB/.BM/.BN triple-framing pinned: 'V-534.AB — Crypto receipt view.' + 'V-534.BM — adds a \"Download PDF\" button that fetches /receipt.pdf (V-666.U) as a blob + triggers an anchor click. The endpoint is auth-gated, so a plain link wouldn't work.' + 'V-534.BN — adds a sibling \"Download .txt\" button for the plain-text variant (V-666.P).' + 'Renders a receipt for a specific order id using useCryptoReceipt (V-534.AA). Includes a \"Copy to clipboard\" button that uses formatReceiptForClipboard. Empty / loading / error / ready states rendered consistently with the rest of the V-534.* view family.'", () => {
    ...
    expect(body).toMatch(
      /\/\/ V-534\.BM — adds a "Download PDF" button that fetches\s*\/\/\s+\/receipt\.pdf \(V-666\.U\) as a blob \+ triggers an anchor\s*\/\/\s+click\. The endpoint is auth-gated, so a plain link\s*\/\/\s+wouldn't work\./,
    );
    expect(body).toMatch(
      /\/\/ V-534\.BN — adds a sibling "Download \.txt" button for the\s*\/\/\s+plain-text variant \(V-666\.P\)\./,
    );
    expect(body).toMatch(
      /\/\/ Renders a receipt for a specific order id using useCryptoReceipt\s*\/\/ \(V-534\.AA\)\. Includes a "Copy to clipboard" button that uses\s*\/\/ formatReceiptForClipboard\. Empty \/ loading \/ error \/ ready states\s*\/\/ rendered consistently with the rest of the V-534\.\* view family\./,
    );
  });
  ...
  it("CryptoReceiptViewProps: orderId 'The order id to render. Pass null to show the empty state.' nullable + ReceiptBody internal subcomponent receives {data: CryptoReceiptData}", () => {
    expect(body).toMatch(
      /interface CryptoReceiptViewProps \{\s*\/\*\* The order id to render\. Pass null to show the empty state\. \*\/\s*orderId: string \| null;\s*\}/,
    );
    expect(body).toMatch(
  ...
  it('Copy-to-clipboard flow is single-flight, timer-safe, and visibly retryable', () => {
    ...
    expect(body).toMatch(
      /if \(copyState === 'copying'\) return;\s*setCopyState\('copying'\);[\s\S]*?await navigator\.clipboard\.writeText\(formatReceiptForClipboard\(data\)\);[\s\S]*?setCopyState\('copied'\);[\s\S]*?catch \{\s*setCopyState\('failed'\);/,
    );
  ...
  it("3-button row: 'Download PDF' onClick=>pdf.download(order_id, 'pdf') + 'Download .txt' onClick=>pdf.download(order_id, 'txt') + 'Copy to clipboard'/'Copied' toggle; both download buttons disabled while pdf.state.kind === 'downloading' + label switches to 'Downloading…'", () => {
    expect(body).toMatch(
      /onClick=\{\(\) => void pdf\.download\(data\.order_id, 'pdf'\)\}\s*disabled=\{pdf\.state\.kind === 'downloading'\}/,
    );
    expect(body).toMatch(
      /onClick=\{\(\) => void pdf\.download\(data\.order_id, 'txt'\)\}\s*disabled=\{pdf\.state\.kind === 'downloading'\}/,
    );
  ...
  it("PDF-failure surface: pdf.state.kind === 'failed' → ErrorBanner with `PDF download failed: ${pdf.state.message}` message + onDismiss=>pdf.reset() (retry-on-dismiss); dl rows: Order/Status/Product/Amount(formatCents)/paid_at conditional (formatTimestamp)/payment_id conditional/Issued", () => {
    expect(body).toMatch(
      /\{pdf\.state\.kind === 'failed' && \(\s*<ErrorBanner\s*message=\{`\$\{pdf\.state\.format === 'pdf' \? 'PDF' : 'Text receipt'\} download failed: \$\{pdf\.state\.message\}`\}/,
    );
    expect(body).toMatch(/<dd>\{formatCents\(data\.price_cents, data\.price_currency\)\}<\/dd>/);
    expect(body).toMatch(
      /\{data\.paid_at !== null && \(\s*<>\s*<dt className="text-ink-secondary">Paid at<\/dt>\s*<dd>\{formatTimestamp\(data\.paid_at\)\}<\/dd>\s*<\/>\s*\)\}/,
    );
    expect(body).toMatch(
      /\{data\.payment_id !== null && \(\s*<>\s*<dt className="text-ink-secondary">Payment id<\/dt>\s*<dd className="font-mono text-xs">\{data\.payment_id\}<\/dd>\s*<\/>\s*\)\}/,
    );
  });
  ...
  it("4-state early returns at view level: orderId === null → 'Pick an order to view its receipt.' + loading|idle → 'Loading receipt…' + error → <ErrorBanner message + onDismiss={() => void refetch()}> (Dismiss retries the receipt fetch instead of dead-ending the panel) + ready → <ReceiptBody data={state.data} />", () => {
    expect(body).toMatch(
      /if \(props\.orderId === null\) \{\s*return \(\s*<div className="rounded-md border border-surface-divider bg-surface-inset p-4 text-sm text-ink-secondary">\s*Pick an order to view its receipt\.\s*<\/div>\s*\);\s*\}/,
    );
    expect(body).toMatch(
      /if \(state\.kind === 'loading' \|\| state\.kind === 'idle'\) \{\s*return \(\s*<div className="rounded-md border border-surface-divider bg-surface-inset p-4 text-sm text-ink-secondary">\s*Loading receipt…\s*<\/div>\s*\);\s*\}/,
    );
    expect(body).toMatch(
      /if \(state\.kind === 'error'\) \{\s*\/\/ Dismiss retries the receipt fetch instead of dead-ending the panel\.\s*return <ErrorBanner message=\{state\.message\} onDismiss=\{\(\) => void refetch\(\)\} \/>;\s*\}\s*return <ReceiptBody data=\{state\.data\} \/>;/,
    );
  });
