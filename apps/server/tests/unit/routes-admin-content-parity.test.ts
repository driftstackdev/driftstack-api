...
describe('W420.A apps/server/src/routes/admin.ts content parity', () => {
  ...
  it('V-326e6 framing pinned: admin-only on team-scoped key writes (POST/DELETE/rotate); GET role-agnostic; member → 403', () => {
    expect(body).toMatch(
      /\/\/ V-326e6 — admin-only gate for api-keys writes \(POST \/ DELETE \/\s*\/\/ rotate\)\. Read \(GET \/v1\/api-keys\) is role-agnostic\./,
    );
    expect(body).toMatch(
      /if \(effective\.role !== 'admin'\) \{\s*throw new ForbiddenError\('API key writes on a team owner require admin role on that team\.'\);/,
    );
  });
  ...
  it('V-330e effective-account framing pinned: GET /v1/usage + /v1/usage/series honor X-Driftstack-Account; owner tier is quota-cap source', () => {
    expect(body).toMatch(
      /\/\/ V-330e — honors X-Driftstack-Account: a team member with a valid\s*\/\/ membership reads the OWNER's usage\. The OWNER's tier is the\s*\/\/ quota-cap source \(members don't override the cap by being on the\s*\/\/ team\); we look it up from the auth repo when the header is set\./,
    );
  ...
  it('Owner-account-deleted-mid-request → 403 ForbiddenError ("Owner account no longer exists.") on /v1/usage', () => {
    expect(body).toMatch(
      /\/\/ Membership references an account that's been deleted between\s*\/\/ the auth-cache load and the route call\. Surface as 403 — the\s*\/\/ membership is effectively invalid\./,
    );
  ...
  it('effectiveAccountIdForKeyWrite: returns undefined on self-account; team-with-admin-role → accountId; team-with-non-admin-role → ForbiddenError', () => {
    expect(body).toMatch(
      /function effectiveAccountIdForKeyWrite\(\s*request: FastifyRequest,\s*ctx: AccountReq,\s*\): string \| undefined \{\s*const effective = resolveEffectiveAccount\(ctx, readEffectiveAccountHeader\(request\)\);\s*if \(effective\.kind !== 'team'\) return undefined;\s*if \(effective\.role !== 'admin'\) \{\s*throw new ForbiddenError\('API key writes on a team owner require admin role on that team\.'\);\s*\}\s*return effective\.accountId;/,
    );
  });
  ...
  it('publicUsage: 5-field shape (period_start/end ISO + tier + totals + quotas)', () => {
    expect(body).toMatch(
      /function publicUsage\(s: UsageSummary\): Record<string, unknown> \{\s*return \{\s*period_start: s\.periodStart\.toISOString\(\),\s*period_end: s\.periodEnd\.toISOString\(\),\s*tier: s\.tier,\s*totals: s\.totals,\s*quotas: s\.quotas,\s*\};/,
    );
  });
  ...
  it('POST /v1/api-keys: V-326e6 admin-only gate; owner tier-derived test/live env switch; spread effectiveAccountId/effectiveTier when team', () => {
    expect(body).toMatch(
      /\/\/ V-326e6 — admin-only on team-scoped requests\. The key is minted\s*\/\/ on the OWNER's account; tier-derived test\/live env switch uses\s*\/\/ the OWNER's tier\./,
    );
    expect(body).toMatch(
      /if \(eff !== undefined\) \{\s*const owner = await authRepo\.getAccount\(eff\);\s*if \(!owner\) throw new ForbiddenError\('Owner account no longer exists\.'\);\s*createOpts = \{ effectiveAccountId: owner\.id, effectiveTier: owner\.tier \};\s*\}/,
    );
    expect(body).toMatch(
      /return reply\.code\(201\)\.send\(\{\s*\.\.\.publicApiKey\(created\.row\),\s*plaintext: created\.plaintext,\s*\}\);/,
    );
  });
  ...
  it('V-296 rotate framing pinned: customer self-service; fresh plaintext shown-once; 24h grace via expires_at; optional name rename', () => {
    expect(body).toMatch(
      /\/\/ V-296 — customer self-service rotation\. Mints a fresh plaintext \(shown\s*\/\/ once\); old key continues working for 24h grace period via\s*\/\/ expires_at-driven auth gate\. Optional `name` lets the caller rename\s*\/\/ the new key \(useful when rotating "production-2024" → "production-2025"\)\./,
    );
    expect(body).toMatch(
      /return reply\.code\(201\)\.send\(\{\s*\.\.\.publicApiKey\(result\.newRow\),\s*plaintext: result\.plaintext,\s*rotated_from: `key_\$\{result\.oldKey\.id\}`,\s*grace_period_ends_at: result\.gracePeriodEndsAt\.toISOString\(\),\s*\}\);/,
    );
  });
  ...
  it('GET /v1/api-keys: role-agnostic effective-account read (both member and admin); apiKeysService.list with effectiveAccountId when team', () => {
    expect(body).toMatch(
      /\/\/ V-326e6 — read role-agnostic; both 'member' and 'admin' can list\s*\/\/ the OWNER's keys\./,
    );
    expect(body).toMatch(
      /const keys = await apiKeysService\.list\(\s*ctx,\s*effective\.kind === 'team' \? \{ effectiveAccountId: effective\.accountId \} : \{\},\s*\);/,
    );
  });
  ...
  it('GET /v1/usage: team kind → load owner via authRepo.getAccount + usageService.summaryFor(owner.id, owner.tier); else currentPeriodSummary(ctx)', () => {
    expect(body).toMatch(
      /if \(effective\.kind === 'team'\) \{\s*const owner = await authRepo\.getAccount\(effective\.accountId\);[\s\S]+?const summary = await usageService\.summaryFor\(owner\.id, owner\.tier\);\s*return publicUsage\(summary\);/,
    );
    expect(body).toMatch(
      /const summary = await usageService\.currentPeriodSummary\(ctx\);\s*return publicUsage\(summary\);/,
    );
  });
  ...
  it('V-170 framing pinned: GET /v1/usage/series daily-bucketed sparkline; default 30 days / max 90; zeros come from the generate_series left-join for days with no rows. V-838 retitled this: it used to say the writers were unwired, and they are wired at bootstrap', () => {
    expect(body).toMatch(
      /\/\/ V-170 — daily-bucketed usage series for sparkline rendering\.\s*\/\/ Customer-dashboard \/usage consumes this\. Default 30 days, max 90\./,
    );
    ...
    expect(body).toMatch(
      /const series = await usageService\.dailySeries\(ctx, query\.days \?\? 30, undefined, \{\s*\.\.\.\(effective\.kind === 'team' \? \{ effectiveAccountId: effective\.accountId \} : \{\}\),\s*\}\);/,
    );
    expect(body).toMatch(
      /return \{\s*from_date: series\.fromDate,\s*to_date: series\.toDate,\s*buckets: series\.buckets,\s*\};/,
    );
  });
  ...
  it('AdminRoutesOptions: apiKeysService + usageService + authRepo (V-330e owner-tier resolution)', () => {
    ...
    expect(body).toMatch(
      /\/\*\*\s*\*\s*V-330e — needed to load the OWNER's account row \(for tier\s*\*\s*resolution\) when a team member calls \/v1\/usage with an\s*\*\s*X-Driftstack-Account header\.\s*\*\/\s*authRepo: AccountAuthRepo;/,
    );
  });
