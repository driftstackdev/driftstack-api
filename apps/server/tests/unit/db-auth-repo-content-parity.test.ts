...
describe('W448.B apps/server/src/db/auth-repo.ts content parity', () => {
  ...
  it('touchWebSessionLastUsed: 1-field set lastUsedAt where id; findTeamMemberships: select 3-field {id, ownerAccountId, role} → map to {membershipId, ownerAccountId, role}', () => {
    expect(body).toMatch(
