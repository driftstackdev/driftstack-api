...
describe('W457.B packages/recapture-automation/src/interfaces.ts content parity', () => {
  ...
  it("ListRunsOpts: 4 optional fields (archetypeId + status + limit 'Default 50, max 200' + cursor); ListRunsPage: {data + nextCursor}", () => {
    expect(body).toMatch(
      /export interface ListRunsOpts \{\s*\n?\s*\/\*\* Filter to a specific archetype\. Omit for all\. \*\/\s*\n?\s*archetypeId\?: string;[\s\S]*?status\?: RecaptureStatus;[\s\S]*?\/\*\* Page size\. Default 50, max 200\. \*\/\s*\n?\s*limit\?: number;[\s\S]*?cursor\?: string;\s*\n?\s*\}/,
    );
    expect(body).toMatch(
      /export interface ListRunsPage \{\s*\n?\s*data: readonly RecaptureRun\[\];\s*\n?\s*nextCursor: string \| null;\s*\n?\s*\}/,
    );
  });
  ...
  it("RecaptureService framing pinned: 'Top-level recapture orchestration service. Operators (founder + future automation) call triggerRecapture() when a version bump is detected; the service queues + runs the capture passes against the configured archetypes.'", () => {
    expect(body).toMatch(
      /\* Top-level recapture orchestration service\. Operators\s*\n?\s*\*\s*\(founder \+ future automation\) call `triggerRecapture\(\)` when\s*\n?\s*\*\s*a version bump is detected; the service queues \+ runs the\s*\n?\s*\*\s*capture passes against the configured archetypes\./,
    );
  });
  ...
  it("triggerRecapture framing pinned: 'Returns the run record with status queued; the worker picks it up + transitions through in_progress → completed / failed.'; signature returns Promise<RecaptureRun>", () => {
    expect(body).toMatch(
      /\* Queue a new recapture run\. Returns the run record with status\s*\n?\s*\*\s*`'queued'`; the worker picks it up \+ transitions through\s*\n?\s*\*\s*`'in_progress'` → `'completed'` \/ `'failed'`\./,
    );
  ...
  it("getRun + listRuns + recordComparison + finalizeRun signatures; recordComparison framing pinned 'Append a per-surface comparison to a run-in-progress. Called by the capture worker as it walks each surface from file 121 + the cumulative-rig snapshot. Aggregates (matchCount / diffCount / etc.) update on the run record.'", () => {
    ...
    expect(body).toMatch(
      /\* Append a per-surface comparison to a run-in-progress\. Called by\s*\n?\s*\*\s*the capture worker as it walks each surface from file 121 \+ the\s*\n?\s*\*\s*cumulative-rig snapshot\. Aggregates \(matchCount \/ diffCount \/\s*\n?\s*\*\s*etc\.\) update on the run record\./,
    );
    expect(body).toMatch(
  ...
  it("finalizeRun framing pinned: 'completed if the worker finished all surfaces; failed if it bailed mid-run. The run's aggregate counts reflect the per-surface comparisons accumulated so far.' + 3-state union ('completed'|'failed'|'cancelled')", () => {
    expect(body).toMatch(
      /\* Mark a run terminal\. `'completed'` if the worker finished all\s*\n?\s*\*\s*surfaces; `'failed'` if it bailed mid-run\. The run's aggregate\s*\n?\s*\*\s*counts reflect the per-surface comparisons accumulated so far\./,
    );
    expect(body).toMatch(
  ...
  it("IosVersionWatcher framing pinned: 'Detection layer — watches for new iOS minor versions. Out of scope for V-179 implementation; the interface is the seam so a future Apple-release-notes scraper / RSS watcher can drop in.' + today-fallback framing 'when Agent 1 notices a new iOS version, founder triggers recapture manually via RecaptureService.triggerRecapture().'", () => {
    expect(body).toMatch(
      /\* Detection layer — watches for new iOS minor versions\. Out of\s*\n?\s*\*\s*scope for V-179 implementation; the interface is the seam so a\s*\n?\s*\*\s*future Apple-release-notes scraper \/ RSS watcher can drop in\./,
    );
    expect(body).toMatch(
      /\* Today: when Agent 1 notices a new iOS version, founder triggers\s*\n?\s*\*\s*recapture manually via `RecaptureService\.triggerRecapture\(\)`\./,
    );
  });
  ...
  it("IosVersionWatcher: 3 methods (getLastSeenVersion framing 'Stored externally (filesystem JSON, key/value store) — implementations supply persistence.' + pollForTransition 'Implementations CAN call this from a cron (daily) or a manual trigger.' + recordTransitionHandled)", () => {
    expect(body).toMatch(
      /\*\s*Last seen iOS version\. Tracked across watcher invocations so\s*\n?\s*\*\s*the watcher can detect transitions\. Stored externally\s*\n?\s*\*\s*\(filesystem JSON, key\/value store\) — implementations supply\s*\n?\s*\*\s*persistence\./,
    );
    expect(body).toMatch(/getLastSeenVersion\(\): Promise<string \| null>;/);
    expect(body).toMatch(
      /\*\s*Check upstream \(Apple release notes, etc\.\) for a version\s*\n?\s*\*\s*newer than `getLastSeenVersion\(\)`\. Returns the transition if\s*\n?\s*\*\s*one is detected\. Implementations CAN call this from a cron\s*\n?\s*\*\s*\(daily\) or a manual trigger\./,
    );
