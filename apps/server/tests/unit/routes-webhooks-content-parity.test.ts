...
describe('W439.A apps/server/src/routes/webhooks.ts content parity', () => {
  ...
  it('header framing pinned: POST/GET/DELETE /v1/webhooks + GET /v1/webhooks/:id/deliveries', () => {
    expect(body).toMatch(
      /\/\/ Webhook subscription routes — POST\/GET\/DELETE \/v1\/webhooks\s*\/\/ \+ GET \/v1\/webhooks\/:id\/deliveries\./,
    );
  });
  ...
  it('imports: CreateWebhookRequest + ListDeliveriesQuery + UpdateWebhookRequest from api-types; BadRequest/Forbidden errors; WebhookDelivery/Endpoint/Service rows; resolveEffectiveAccount', () => {
    expect(body).toMatch(
      /import \{\s*CreateWebhookRequestSchema,\s*ListDeliveriesQuerySchema,\s*UpdateWebhookRequestSchema,\s*\} from '@driftstack\/api-types';/,
    );
    expect(body).toMatch(
      /import \{ BadRequestError, ForbiddenError \} from '\.\.\/lib\/errors\.js';/,
    );
    expect(body).toMatch(
      /import type \{\s*WebhookDeliveryRow,\s*WebhookEndpointRow,\s*WebhooksService,\s*\} from '\.\.\/services\/webhooks\.js';/,
    );
  });
  ...
  it('V-326e5 effectiveAccountIdForWrite framing pinned: admin-only gate for webhook write operations on team owners; throws ForbiddenError "Webhook writes on a team owner require admin role on that team."', () => {
    expect(body).toMatch(
      /\*\s*V-326e5 — admin-only gate for webhook write operations on team\s*\*\s*owners\. Returns the effective accountId \(string\) when team write\s*\*\s*should proceed, or undefined when self-scoped\. Throws ForbiddenError\s*\*\s*on member-role team requests\./,
    );
    expect(body).toMatch(
  ...
  it('V-359 publicEndpoint rotation surface framing pinned: prev_secret_prefix = first-12-chars of secretPrev (non-sensitive, same shape as current secret_prefix display); rotation_grace_expires_at lets dashboard show "rotation ends in <X>" so customers know how much longer dual-signing in effect; both null when no rotation in flight', () => {
    expect(body).toMatch(
      /\/\/ V-359 — surface the rotation grace state when active\. The previous\s*\/\/ secret's first-12-chars are non-sensitive \(same shape as the\s*\/\/ current secret_prefix display\); the grace expiry lets the\s*\/\/ dashboard show "rotation ends in <X>" so customers know how much\s*\/\/ longer dual-signing is in effect\. Both null when no rotation in\s*\/\/ flight\./,
    );
    expect(body).toMatch(
      /const rotationActive =\s*row\.secretPrev !== null &&\s*row\.secretPrevExpiresAt !== null &&\s*row\.secretPrevExpiresAt\.getTime\(\) > Date\.now\(\);/,
    );
    expect(body).toMatch(
      /prev_secret_prefix:\s*rotationActive && row\.secretPrev !== null \? row\.secretPrev\.slice\(0, 12\) : null,\s*rotation_grace_expires_at:\s*rotationActive && row\.secretPrevExpiresAt !== null\s*\? row\.secretPrevExpiresAt\.toISOString\(\)\s*: null,/,
    );
  });

  it('V-185 publicEndpoint: aggregate per-endpoint delivery_counts (delivered + failed + dlq); default {0,0,0}', () => {
    expect(body).toMatch(
      /counts: \{ delivered: number; failed: number; dlq: number \} = \{\s*delivered: 0,\s*failed: 0,\s*dlq: 0,\s*\},/,
    );
    expect(body).toMatch(
      /\/\*\* V-185 — aggregate per-endpoint delivery counts\. \*\/\s*delivery_counts: counts,/,
    );
  });
  ...
  it('publicDelivery mapper: id wdl_ + webhook_id whk_ + event_id + event_type + status + attempts + next_attempt_at + last_response_status + last_response_excerpt + last_error + delivered_at nullable + created_at', () => {
    expect(body).toMatch(
      /function publicDelivery\(row: WebhookDeliveryRow\): Record<string, unknown> \{\s*return \{\s*id: `wdl_\$\{row\.id\}`,\s*webhook_id: `whk_\$\{row\.webhookId\}`,\s*event_id: row\.eventId,\s*event_type: row\.eventType,\s*status: row\.status,\s*attempts: row\.attempts,\s*next_attempt_at: row\.nextAttemptAt\.toISOString\(\),\s*last_response_status: row\.lastResponseStatus,\s*last_response_excerpt: row\.lastResponseExcerpt,\s*last_error: row\.lastError,\s*delivered_at: row\.deliveredAt \? row\.deliveredAt\.toISOString\(\) : null,\s*created_at: row\.createdAt\.toISOString\(\),\s*\};\s*\}/,
    );
  });
  ...
  it('V-326e5 POST /v1/webhooks: admin-only when targeting team owner; CreateWebhookRequest parse; response = publicEndpoint(created.row) + secret plaintext (returned once)', () => {
    ...
    expect(body).toMatch(
      /return reply\.code\(201\)\.send\(\{\s*\.\.\.publicEndpoint\(created\.row\),\s*secret: created\.plaintextSecret,\s*\}\);/,
    );
  });
  ...
  it('V-330f GET /v1/webhooks framing pinned: read endpoints + per-endpoint counts scoped to OWNER when X-Driftstack-Account set; read-only — both roles allowed; V-804 retracted the self-only half: POST/DELETE/PATCH/rotate-secret/test/replay all resolve through effectiveAccountIdForWrite (V-326e5), so writes are admin-only on a team owner rather than self-scoped', () => {
    expect(body).toMatch(
      /\/\/ V-330f — read endpoints \+ per-endpoint counts, scoped to the\s*\/\/ OWNER when X-Driftstack-Account is set\. Read-only; both roles\s*\/\/ allowed\./,
    );
    ...
    expect(body).toMatch(
      /which is the V-326e5\s*\/\/ admin-only gate — a member-role team request gets a ForbiddenError/,
    );
    expect(body, 'the retracted self-only claim must not return').not.toMatch(
      /remain self-only until the/,
    );
    expect(body).toMatch(
      /const rowsWithCounts = await service\.listWithCounts\(\s*ctx,\s*effective\.kind === 'team' \? \{ effectiveAccountId: effective\.accountId \} : \{\},\s*\);\s*return \{ data: rowsWithCounts\.map\(\(r\) => publicEndpoint\(r\.endpoint, r\.counts\)\) \};/,
    );
  });
  ...
  it('V-351 PATCH /v1/webhooks/:id framing pinned: partial update; mirror of POST/DELETE V-326e5 admin-only-on-team gate; disabled endpoints cannot be updated (repo enforces; 409); UpdateWebhookRequest safeParse → BadRequest with first-issue message fallback "Invalid body."', () => {
    expect(body).toMatch(
      /\/\/ V-351 — partial update\. Mirror of POST \+ DELETE for the\s*\/\/ V-326e5 admin-only-on-team gate\. Disabled endpoints cannot be\s*\/\/ updated \(the repo enforces; this returns 409\)\./,
    );
    expect(body).toMatch(
      /const parsed = UpdateWebhookRequestSchema\.safeParse\(request\.body\);\s*if \(!parsed\.success\)\s*throw new BadRequestError\(parsed\.error\.issues\[0\]\?\.message \?\? 'Invalid body\.'\);/,
    );
  });
  ...
  it('GET /v1/webhooks/:id/deliveries: ListDeliveriesQuery parse; service.listDeliveries({limit, cursor?, status?, effectiveAccountId?}) → data + has_more (nextCursor !== null) + next_cursor', () => {
    expect(body).toMatch(
      /const page = await service\.listDeliveries\(ctx, id, \{\s*limit: query\.limit,\s*\.\.\.\(query\.cursor !== undefined \? \{ cursor: query\.cursor \} : \{\}\),\s*\.\.\.\(query\.status !== undefined \? \{ status: query\.status \} : \{\}\),\s*\.\.\.\(effective\.kind === 'team' \? \{ effectiveAccountId: effective\.accountId \} : \{\}\),\s*\}\);\s*return \{\s*data: page\.items\.map\(publicDelivery\),\s*has_more: page\.nextCursor !== null,\s*next_cursor: page\.nextCursor,\s*\};/,
    );
  });
  ...
  it("V-307 customer self-service replay framing pinned: account-scoped; 404s if delivery not owned by calling account; different from admin /v1/admin/webhook-deliveries/:id/replay which can replay any account's delivery; 200 with publicDelivery(updated)", () => {
    ...
    expect(body).toMatch(
      /const eff = effectiveAccountIdForWrite\(request, ctx\);\s*const updated = await service\.replayDeliveryAsCustomer\(ctx, deliveryId, \{\s*\.\.\.\(eff !== undefined \? \{ effectiveAccountId: eff \} : \{\}\),\s*\}\);\s*return reply\.code\(200\)\.send\(publicDelivery\(updated\)\);/,
    );
  });
  ...
  it('V-359 rotate-secret framing pinned: 24h grace; new plaintext returned ONCE; worker dual-signs every outbound delivery with both new+old while secret_prev_expires_at > now; admin-only on team-scoped (same gate as create/update/delete/send-test); response: id whk_ + secret plaintext + secret_prefix + prev_secret_prefix first-12-chars + grace_expires_at ISO fallback now()', () => {
    expect(body).toMatch(
      /\/\/ V-359 — rotate the signing secret with a 24h grace\. New plaintext\s*\/\/ returned ONCE; worker dual-signs every outbound delivery with both\s*\/\/ the new \+ old secret while `secret_prev_expires_at > now`\.\s*\/\/ Admin-only on team-scoped requests \(same gate as create \/ update \/\s*\/\/ delete \/ send-test\)\./,
    );
    expect(body).toMatch(
      /return reply\.code\(200\)\.send\(\{\s*id: `whk_\$\{result\.row\.id\}`,\s*secret: result\.plaintextSecret,\s*secret_prefix: result\.row\.secretPrefix,\s*prev_secret_prefix: result\.row\.secretPrev \? result\.row\.secretPrev\.slice\(0, 12\) : '',\s*grace_expires_at: result\.row\.secretPrevExpiresAt\s*\? result\.row\.secretPrevExpiresAt\.toISOString\(\)\s*: new Date\(\)\.toISOString\(\),\s*\}\);/,
    );
  });
  ...
  it('V-356 test.ping framing pinned: synthetic event bypassing subscription; lets customer verify handler before relying on it for real events; admin-only when targeting team owner (same gate as create/update/delete); 202 with delivery_id wdl_ + event_id + event_type "test.ping"', () => {
    expect(body).toMatch(
      /\/\/ V-356 — send a synthetic test\.ping event to the endpoint,\s*\/\/ bypassing subscription\. Lets the customer verify their handler\s*\/\/ before relying on it for real events\. Admin-only when targeting\s*\/\/ a team owner \(same gate as create \/ update \/ delete\)\./,
    );
    expect(body).toMatch(
      /return reply\.code\(202\)\.send\(\{\s*delivery_id: `wdl_\$\{result\.deliveryId\}`,\s*event_id: result\.eventId,\s*event_type: 'test\.ping',\s*\}\);/,
    );
  });
