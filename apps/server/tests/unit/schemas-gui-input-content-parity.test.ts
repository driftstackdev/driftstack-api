...
describe('W430.C apps/server/src/schemas/gui-input.ts content parity', () => {
  ...
  it('Server-internal posture pinned: never appears on @driftstack/api-types per L-001; human-cadence-is-behavioral-simulation rationale (bypassing automation simulation is correct, not a regression)', () => {
    expect(body).toMatch(
      /\/\/ This schema is \*\*server-internal only\*\*\. Per L-001\s*\/\/ \(docs\/locked-decisions\.md\), coordinate-level mechanics never appear\s*\/\/ on the customer-facing surface \(`@driftstack\/api-types`\)\./,
    );
    expect(body).toMatch(
      /The GUI's\s*\/\/ live viewport — where a human is literally clicking pixels in a\s*\/\/ screenshot — has a different stealth posture from automation: the\s*\/\/ human's own cadence IS the behavioral simulation, so bypassing the\s*\/\/ automation simulation layer is correct, not a regression\./,
    );
  });
  ...
  it('Endpoint + scope posture pinned: POST /v1/sessions/:id/gui-input + the gui_control scope, which no broad scope satisfies. V-788 RETRACTED "only keys minted for the self-hosted GUI workflow (enterprise tier) get it" — false, since ELEVATED_SCOPES withholds only admin + driftstack_internal_admin and any account_owner on an apiAccess tier may request the scope.', () => {
    expect(body).toMatch(
      /\/\/ Endpoint: POST \/v1\/sessions\/:id\/gui-input\.\s*\/\/ Auth: requires the `gui_control` scope\. No broad scope satisfies it, so\s*\/\/ a key only carries it when the mint request asks for it explicitly\./,
    );
  ...
  it('GUIInputActionSchema discriminated-union on kind: tap_at (x/y int min 0 viewport pixels origin top-left) + type_focused (text max 10_000 + optional delay_ms 0..500); GUIInputAction type inferred', () => {
    ...
    expect(body).toMatch(
      /\/\/ Tap at viewport pixel coordinates \(origin top-left\)\.\s*z\.object\(\{\s*kind: z\.literal\('tap_at'\),\s*x: z\.number\(\)\.int\(\)\.min\(0\),\s*y: z\.number\(\)\.int\(\)\.min\(0\),\s*\}\),/,
    );
    expect(body).toMatch(
      /\/\/ Type into the currently-focused element \(no selector\)\. Pairs with\s*\/\/ tap_at — tap focuses, type_focused enters text\./,
    );
    expect(body).toMatch(
      /z\.object\(\{\s*kind: z\.literal\('type_focused'\),\s*text: z\.string\(\)\.max\(10_000\),\s*delay_ms: z\.number\(\)\.int\(\)\.min\(0\)\.max\(500\)\.optional\(\),\s*\}\),/,
    );
  ...
  it('GUIInputRequestSchema: action + optional timeout_ms 100..60_000; GUIInputRequest type inferred', () => {
    expect(body).toMatch(
      /export const GUIInputRequestSchema = z\.object\(\{\s*action: GUIInputActionSchema,\s*timeout_ms: z\.number\(\)\.int\(\)\.min\(100\)\.max\(60_000\)\.optional\(\),\s*\}\);/,
    );
  ...
  it('GUIInputResponseSchema mirrors InteractResponse: { ok: true literal, duration_ms: int>=0 }; client-handling-identical rationale', () => {
    expect(body).toMatch(
      /\/\/ The response shape mirrors InteractResponse — we keep it identical\s*\/\/ so the client-side handling is the same\./,
    );
    expect(body).toMatch(
      /export const GUIInputResponseSchema = z\.object\(\{\s*ok: z\.literal\(true\),\s*duration_ms: z\.number\(\)\.int\(\)\.nonnegative\(\),\s*\}\);/,
    );
