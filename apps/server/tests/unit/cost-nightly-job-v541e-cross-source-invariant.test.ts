...
//
//   dedupOnAccountAndType: true — V-202d dedup primitive prevents
//   duplicate pending jobs across overlapping enqueue calls.
//
...
describe('W928 V-541.E cost-nightly-job cross-source invariant', () => {
  ...
  it('CRITICAL enqueueNextNightlyRun passes dedupOnAccountAndType: true + accountId: null + jobType: COST_NIGHTLY_JOB_TYPE + runAt: nextMidnightUtc. The 4-arg enqueue shape is what the V-202d primitive expects.', () => {
    const p = read(resolve(REPO_ROOT, 'apps/server/src/services/cost-nightly-job.ts'));
