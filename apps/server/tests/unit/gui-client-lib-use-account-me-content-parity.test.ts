...
describe('W465.A apps/gui-client/src/lib/use-account-me.ts content parity', () => {
  ...
  it("Imports: useCallback + useEffect + useState from 'react' + readApiErrorMessage from './api-errors' + useSettings from './SettingsContext'", () => {
  ...
  it("Fetcher: no-apiKey → error 'No API key configured.' early return; trailing-slash strip baseUrl; Bearer auth + accept JSON; !res.ok → readApiErrorMessage; ready spread; instance-of-Error catch narrowing", () => {
    expect(body).toMatch(
      /if \(!settings\.apiKey\) \{\s*\n?\s*setState\(\{ kind: 'error', message: 'No API key configured\.' \}\);\s*\n?\s*return;\s*\n?\s*\}\s*\n?\s*setState\(\{ kind: 'loading' \}\);/,
    );
  ...
  it('useEffect: opts.manual === true early return (no auto-fetch); else void fetcher() with [fetcher, opts.manual] deps; return {state, refetch: fetcher}', () => {
    expect(body).toMatch(
