...
describe('migration 0133 adds the fourth leg and the shadow charge rule, and nothing else', () => {
  ...
  it('the journal applies it last, after 0132, with a later `when`', () => {
    ...
    // follows 0135.
    expect(journal.entries[at + 3]?.tag).toBe('0136_credit_window_level_change_source');
    expect(journal.entries[at + 3]?.when).toBeGreaterThan(
      journal.entries[at + 2]?.when ?? Infinity,
    );
    // 0137 (the S17 re-audit fix — a lot remembers what its payment still paid
    // when it was granted) follows 0136.
    expect(journal.entries[at + 4]?.tag).toBe('0137_credit_lot_still_paid');
    ...
    // level change's payment still paid) follows 0138.
    expect(journal.entries[at + 6]?.tag).toBe('0139_credit_window_undisputed_level');
