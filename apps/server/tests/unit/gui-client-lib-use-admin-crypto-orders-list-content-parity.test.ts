...
describe('W474.B apps/gui-client/src/lib/use-admin-crypto-orders-list.ts content parity', () => {
  ...
  it("V-534.AG framing pinned: 'V-534.AG — useAdminCryptoOrdersList hook.' + admin-scope framing 'Wraps GET /v1/admin/crypto-orders (V-666.D + V-666.T). Admin-only surface — caller must have an API key with the `driftstack_internal_admin` scope; non-admin keys get a 403 which surfaces as an error state. Supports the V-666.T status + search filters via opts; refetch picks up the latest opts.' — pinned so a non-admin key gets a clear 403 surface instead of a silent blank list", () => {
    ...
    expect(body).toMatch(
      /\/\/ Wraps GET \/v1\/admin\/crypto-orders \(V-666\.D \+ V-666\.T\)\. Admin-only\s*\n?\s*\/\/ surface — caller must have an API key with the\s*\n?\s*\/\/ `driftstack_internal_admin` scope; non-admin keys get a 403 which\s*\n?\s*\/\/ surfaces as an error state\. Supports the V-666\.T status \+ search\s*\n?\s*\/\/ filters via opts; refetch picks up the latest opts\./,
    );
  });
  ...
  it("V-534.AW cursor-pagination framing pinned: 'cursor pagination (V-666.AM). When the server returns a non-null `next_cursor`, the caller can invoke `loadMore` to append the next page in place. Changing any filter resets the pagination state (the next fetch starts from the first page).'", () => {
    expect(body).toMatch(
      /\/\/ V-534\.AW — cursor pagination \(V-666\.AM\)\. When the server returns\s*\n?\s*\/\/ a non-null `next_cursor`, the caller can invoke `loadMore` to\s*\n?\s*\/\/ append the next page in place\. Changing any filter resets the\s*\n?\s*\/\/ pagination state \(the next fetch starts from the first page\)\./,
    );
  });
  ...
  it("AdminCryptoOrder extends CryptoOrderData with account_id nullable + customer_note? 'Customer-side bookkeeping note (also surfaced to the customer).' + V-666.AA internal_note? 'admin-only internal note. Not visible to the customer.' — pinned so the admin-only note column doesn't get dropped or the visibility annotation reverted", () => {
    expect(body).toMatch(
      /export interface AdminCryptoOrder extends CryptoOrderData \{\s*\n?\s*account_id: string \| null;\s*\n?\s*\/\*\* Customer-side bookkeeping note \(also surfaced to the customer\)\. \*\/\s*\n?\s*customer_note\?: string \| null;\s*\n?\s*\/\*\* V-666\.AA — admin-only internal note\. Not visible to the customer\. \*\/\s*\n?\s*internal_note\?: string \| null;\s*\n?\s*\}/,
    );
  });
  ...
  it('AdminCryptoOrdersListData {orders + nextCursor nullable V-666.AM} + ListApiResponse internal type with next_cursor? + AdminCryptoOrdersListState 5-variant with loading_more{data}', () => {
    expect(body).toMatch(
      /export interface AdminCryptoOrdersListData \{\s*\n?\s*orders: AdminCryptoOrder\[\];\s*\n?\s*\/\*\* V-666\.AM — opaque cursor for the next page, or null on the terminal page\. \*\/\s*\n?\s*nextCursor: string \| null;\s*\n?\s*\}/,
    );
    expect(body).toMatch(
      /interface ListApiResponse \{\s*\n?\s*orders: AdminCryptoOrder\[\];\s*\n?\s*next_cursor\?: string \| null;\s*\n?\s*\}/,
    );
    expect(body).toMatch(
      /export type AdminCryptoOrdersListState =\s*\n?\s*\| \{ kind: 'idle' \}\s*\n?\s*\| \{ kind: 'loading' \}\s*\n?\s*\| \{ kind: 'ready'; data: AdminCryptoOrdersListData \}\s*\n?\s*\| \{ kind: 'loading_more'; data: AdminCryptoOrdersListData \}\s*\n?\s*\| \{ kind: 'error'; message: string \};/,
    );
  });
  ...
  it("UseAdminCryptoOrdersListOpts 8-field: manual? + limit? 'Page size override; server caps at 200.' + status? V-666.T + search? V-666.T 'Free-text search across order_id / product / customer_note' + accountId? + paymentId? V-666.AS 'exact-match payment_id filter' + createdAfter?/createdBefore? V-666.BY date-range", () => {
    expect(body).toMatch(
      /\/\*\* Page size override; server caps at 200\. \*\/\s*\n?\s*limit\?: number;\s*\n?\s*\/\*\* Filter to one status \(V-666\.T\)\. \*\/\s*\n?\s*status\?: AdminCryptoOrder\['status'\] \| 'cancelled' \| null;\s*\n?\s*\/\*\* Free-text search across order_id \/ product \/ customer_note \(V-666\.T\)\. \*\/\s*\n?\s*search\?: string \| null;\s*\n?\s*\/\*\* Filter to a specific account_id\. \*\/\s*\n?\s*accountId\?: string \| null;\s*\n?\s*\/\*\* V-666\.AS — exact-match payment_id filter\. \*\/\s*\n?\s*paymentId\?: string \| null;\s*\n?\s*\/\*\* V-666\.BY — ISO 8601 lower bound on created_at \(inclusive\)\. \*\/\s*\n?\s*createdAfter\?: string \| null;\s*\n?\s*\/\*\* V-666\.BY — ISO 8601 upper bound on created_at \(exclusive\)\. \*\/\s*\n?\s*createdBefore\?: string \| null;/,
    );
  });
