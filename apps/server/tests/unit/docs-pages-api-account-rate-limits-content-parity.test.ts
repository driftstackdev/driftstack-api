...
describe('W788 docs /api/account-rate-limits content parity', () => {
  ...
  it('CRITICAL 2 bucket-key framing pinned — global (every /v1/*) + sessions:create (POST /v1/sessions only — lower cap). Matches W786 reference/rate-limits 2-bucket model.', () => {
    const p = read(PAGE);

    expect(p).toMatch(
      /Two bucket keys exist: `global` \(every authenticated\s*\n?`\/v1\/\*` call\) and `sessions:create` \(`POST \/v1\/sessions` only —\s*\n?lower cap because session creation is expensive\)\./,
    );
  });
