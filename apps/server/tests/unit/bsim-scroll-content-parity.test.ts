...
describe('W453.A packages/behavioural-simulation/src/scroll.ts content parity', () => {
  ...
  it("V-530.B framing pinned: 'V-530.B — scroll velocity profiles with exponential decay.' + 'Second module of the Phase 3 real implementation, after V-530.A (touch event distributions). Models the velocity curve a human finger flick produces on a scroll container: initial velocity from the flick, then exponential decay as friction slows the scroll.'", () => {
    ...
    expect(body).toMatch(
      /\/\/ Second module of the Phase 3 real implementation, after V-530\.A\s*\/\/ \(touch event distributions\)\. Models the velocity curve a human finger\s*\/\/ flick produces on a scroll container: initial velocity from the flick,\s*\/\/ then exponential decay as friction slows the scroll\./,
    );
  });
  ...
  it("distinct-from-mock-surface framing pinned: 'Distinct from the existing generateScrollPattern mock surface (constant per-tick deltas) — this module produces realistic decaying per-tick deltas. The existing mock surface stays unchanged for backward compatibility; new callers reach for the velocity profile.'", () => {
    expect(body).toMatch(
      /\/\/ Distinct from the existing `generateScrollPattern` mock surface\s*\/\/ \(constant per-tick deltas\) — this module produces realistic decaying\s*\/\/ per-tick deltas\. The existing mock surface stays unchanged for\s*\/\/ backward compatibility; new callers reach for the velocity profile\./,
    );
  });

  it("ScrollVelocityTick: 4-field (tMs + velocityPxPerSec + deltaPx signed + cumulativePx); 'signed: + = forward, - = reverse' framing on deltaPx", () => {
    expect(body).toMatch(
      /export interface ScrollVelocityTick \{[\s\S]*?tMs: number;[\s\S]*?velocityPxPerSec: number;[\s\S]*?\/\*\* Pixels scrolled during this tick \(signed: \+ = forward, - = reverse\)\. \*\/\s*deltaPx: number;[\s\S]*?cumulativePx: number;/,
    );
  });
  ...
  it("ScrollVelocityProfile: 7-field (direction 4-union + initialVelocityPxPerSec + decayRate '1 / seconds. Higher = faster decay' + ticks readonly + totalDistancePx absolute + durationMs + seed)", () => {
    expect(body).toMatch(
      /export interface ScrollVelocityProfile \{[\s\S]*?direction: 'up' \| 'down' \| 'left' \| 'right';[\s\S]*?initialVelocityPxPerSec: number;[\s\S]*?\/\*\* Exponential decay rate \(1 \/ seconds\)\. Higher = faster decay\. \*\/\s*decayRate: number;[\s\S]*?ticks: readonly ScrollVelocityTick\[\];[\s\S]*?\/\*\* Total distance scrolled \(absolute, in pixels\)\. \*\/\s*totalDistancePx: number;[\s\S]*?durationMs: number;[\s\S]*?seed: string;/,
    );
  });
  ...
  it("ScrollVelocityClassDefaults framing pinned: 'scroll-container has stronger flicks + lower friction (the touched surface is designed to be scrolled); generic containers have weaker flicks + higher friction (incidental scroll, e.g. background body)'", () => {
    expect(body).toMatch(
      /\* Per-element-class defaults for scroll velocity\. The `container` class\s*\*\s*the touch initiates on shapes the initial flick velocity \+ friction\.\s*\*\s*scroll-container has stronger flicks \+ lower friction \(the touched\s*\*\s*surface is designed to be scrolled\); generic containers have weaker\s*\*\s*flicks \+ higher friction \(incidental scroll, e\.g\. background body\)\./,
    );
  });
  ...
  it("SCROLL_VELOCITY_DEFAULTS: 7-entry table; Object.freeze + Readonly<Record<ElementClass, ScrollVelocityClassDefaults>>; scroll-container 2400 px/s mean + decay 2.0; input lowest 700 px/s + decay 6.0; button/link sharing 'Scrolling from a button surface is unusual but possible' framing", () => {
    expect(body).toMatch(
      /export const SCROLL_VELOCITY_DEFAULTS: Readonly<Record<ElementClass, ScrollVelocityClassDefaults>> =\s*Object\.freeze\(\{/,
    );
    expect(body).toMatch(
      /'scroll-container': \{\s*meanInitialVelocityPxPerSec: 2400,\s*initialVelocityJitter: 600,\s*meanDecayRate: 2\.0,\s*decayRateJitter: 0\.4,\s*\},/,
    );
    expect(body).toMatch(
      /\/\/ Scrolling from a button surface is unusual but possible \(touchpad\s*\/\/ \/ inertial scroll origin happens to be over a button\)\. Conservative\s*\/\/ defaults — short, weak scroll\./,
    );
    expect(body).toMatch(
  ...
  it("Module constants: DEFAULT_TICK_INTERVAL_MS=16 ('60Hz, matching touch device rates' framing), REST_VELOCITY_THRESHOLD_PX_PER_SEC=5, MAX_DURATION_MS=5000 plus bounded cadence/velocity/decay envelope", () => {
    expect(body).toMatch(
      /\/\*\* Default tick interval \(ms\)\. 16ms ≈ 60 Hz, matching touch device rates\. \*\/\s*const DEFAULT_TICK_INTERVAL_MS = 16;/,
    );
    expect(body).toMatch(
      /\/\*\* Velocity below this threshold \(px\/s\) terminates the scroll\. \*\/\s*const REST_VELOCITY_THRESHOLD_PX_PER_SEC = 5;/,
    );
    expect(body).toMatch(
      /\/\*\* Hard cap on duration to bound test runtime\. ~5 seconds is generous\. \*\/\s*const MAX_DURATION_MS = 5000;/,
    );
  ...
  it('generateScrollVelocityProfile framing pinned: decay sampled until rest; overrides unable to settle in MAX_DURATION_MS reject without a synthetic final tick; direction and total-distance conventions pinned', () => {
    expect(body).toMatch(
      /\*\s*v\(t\) = v0 \* exp\(-decayRate \* t\)\s*\*\s*sampled at `tickIntervalMs` intervals until velocity drops below the\s*\*\s*rest threshold \(5 px\/s\)\. Explicit overrides that cannot settle inside\s*\*\s*`MAX_DURATION_MS` are rejected instead of compressing unseen motion into a\s*\*\s*synthetic final tick\./,
    );
    expect(body).toMatch(
      /\* Direction sign convention:\s*\*\s*- 'down' \/ 'right' → positive `deltaPx`\s*\*\s*- 'up' \/ 'left'\s*→ negative `deltaPx`\s*\*\s*\* `totalDistancePx` is always positive \(absolute distance scrolled\)\./,
    );
    expect(body).toMatch(
  ...
  it("Per-tick deltaPx integration framing pinned: '∫ v(τ)dτ from t to t+tickSec = (v0 / decayRate) * (exp(-decay * t) - exp(-decay * (t+tickSec)))' with positive decay guaranteed at the boundary", () => {
    expect(body).toMatch(
      /\/\/ Pixels scrolled this tick: ∫ v\(τ\)dτ from t to t\+tickSec\s*\/\/\s*= \(v0 \/ decayRate\) \* \(exp\(-decay \* t\) - exp\(-decay \* \(t\+tickSec\)\)\)/,
    );
    expect(body).toMatch(
      /const deltaPxAbs =\s*\(v0 \/ decayRate\) \*\s*\(Math\.exp\(-decayRate \* tSec\) - Math\.exp\(-decayRate \* \(tSec \+ tickSec\)\)\);/,
    );
    expect(body).toMatch(
  ...
  it("Rest-velocity termination guard: 'if velocityPxPerSec < REST_VELOCITY_THRESHOLD_PX_PER_SEC && tMs > 0 break' — bounds profile length when velocity dies below 5 px/s; tickIntervalMs > 0 guard with throw on <= 0", () => {
    expect(body).toMatch(
      /if \(tickIntervalMs <= 0\) \{\s*throw new Error\(\s*`generateScrollVelocityProfile: tickIntervalMs must be > 0 \(got \$\{tickIntervalMs\}\)`,\s*\);/,
    );
    expect(body).toMatch(
      /if \(velocityPxPerSec < REST_VELOCITY_THRESHOLD_PX_PER_SEC && tMs > 0\) \{\s*break;\s*\}/,
    );
  });
  ...
  it('v0/decayRate jitter with bounded minimums: v0 = max(1, mean + uniformSigned*jitter); decayRate = max(0.1, mean + uniformSigned*jitter); defaultSeed = `scroll-v:${direction}:${elementClass}`', () => {
    expect(body).toMatch(
      /Math\.max\(\s*1,\s*defaults\.meanInitialVelocityPxPerSec \+ uniformSigned\(\) \* defaults\.initialVelocityJitter,\s*\)/,
    );
    expect(body).toMatch(
      /Math\.max\(0\.1, defaults\.meanDecayRate \+ uniformSigned\(\) \* defaults\.decayRateJitter\)/,
    );
    expect(body).toMatch(
      /function defaultSeed\(opts: GenerateScrollVelocityProfileOpts\): string \{\s*return `scroll-v:\$\{opts\.direction\}:\$\{opts\.elementClass\}`;\s*\}/,
    );
  });
