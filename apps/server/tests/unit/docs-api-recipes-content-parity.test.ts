...
describe('docs/api/recipes content parity', () => {
  ...
  it('current create/list/read/delete surface and explicit no-execute boundary are pinned', () => {
    expect(body).toMatch(
      /The current surface covers create, list, read, and delete:\s*`POST \/v1\/recipes`, `GET \/v1\/recipes`, `GET \/v1\/recipes\/\{id\}`, and\s*`DELETE \/v1\/recipes\/\{id\}`\. There is no recipe-execution endpoint/,
    );
  ...
  it('Resource shape and public-detail redaction pinned: list metadata stays compact; detail retains sensitive selectors/order/marker but never returns saved type values to read scope', () => {
    ...
    expect(body).toMatch(
      /to get its public `intent_log`\. Sensitive `type` steps retain their\s*selector, order, and `sensitive: true` marker but omit `value`\./,
    );
  ...
  it('Get-one endpoint documents the public intent log, sensitive-value omission, and cross-account 404 anti-enumeration', () => {
    ...
    expect(body).toMatch(
      /including its public `intent_log` array \(the\s*list endpoint omits it\)\./,
    );
    expect(body).toMatch(
      /Sensitive `type` intents omit `value`, even\s*when sensitivity is inferred from a password, OTP, PIN, card, or API\s*key selector; these steps still carry `sensitive: true`/,
    );
    expect(body).toMatch(
      /doesn't distinguish missing from forbidden, to avoid\s*leaking existence\./,
    );
  });
  ...
  it('Delete endpoint documented as shipped: DELETE /v1/recipes/{id} returns 204 + NOT idempotent (already-deleted returns 404) + cross-account 404 — pinned so the non-idempotent contract (matching the SDKs, which propagate the 404 rather than swallowing it) stays explicit', () => {
    ...
    expect(body).toMatch(
      /Delete is \*\*not\*\* idempotent: deleting an already-deleted recipe\s*returns 404, not 204\./,
    );
  });
});
