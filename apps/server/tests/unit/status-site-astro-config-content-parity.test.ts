...
// runtime-fetch target (would create status.driftstack.dev↔/v1/status
...
describe('W537.A apps/status-site/astro.config.mjs content parity', () => {
  ...
  it("V-295c + status.driftstack.dev + hermetic-build framing pinned: '@ts-check' + 'V-295c — public service status page.' + 'Static Astro output served from Cloudflare Pages at status.driftstack.dev. At runtime the page fetches `https://api.driftstack.dev/v1/status/incidents` and renders. No build-time fetch — keeps the build hermetic and decoupled from the API's availability.' — pinned so the V-295c anchor + status.driftstack.dev-canonical-subdomain + Cloudflare-Pages-static + runtime-fetch-from-api.driftstack.dev/v1/status/incidents + hermetic-build (decoupled from API availability) commitment survives (drift to build-time fetch would block deploy when /v1/status is down, exactly when the status page is needed most)", () => {
