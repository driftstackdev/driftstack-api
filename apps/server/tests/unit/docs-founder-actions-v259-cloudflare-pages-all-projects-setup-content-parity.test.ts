...
describe('W545.A /docs/founder-actions/v259-cloudflare-pages-all-projects-setup.md content parity', () => {
  ...
  it("Header + V-259-paired-with-V-258 + 2-wired-2-unwired framing pinned: '# V-259 — Cloudflare Pages: full project setup (founder ops action)' + 'Per V-259 / paired with V-258: consolidates all four Cloudflare Pages projects the Driftstack stack needs, in one runbook so the founder can do this in a single Cloudflare-dashboard session.' + 'Two of the four currently have GitHub Actions deploy workflows wired (marketing + docs). The other two (customer-dashboard + admin-panel) have astro configs but their deploy workflows haven't shipped yet — the projects can still be pre-created so the workflow rollouts are unblocked when they land.' — pinned so the V-259-paired-V-258 + 4-project-consolidation + 2-workflows-wired (marketing+docs) + 2-pre-staged-projects (customer-dashboard+admin-panel) + workflow-rollout-unblock commitment survives", () => {
    ...
    expect(body).toMatch(
      /Two of the four currently have GitHub Actions deploy workflows wired \(marketing \+ docs\)\./,
    );
    expect(body).toMatch(
      /The other two \(customer-dashboard \+ admin-panel\) have astro configs but their deploy workflows haven't shipped yet — the projects can still be pre-created so the workflow rollouts are unblocked when they land\./,
    );
  });

  it("4-project inventory + slug + custom-domain + workflow + status framing pinned: '`driftstack-marketing` + `driftstack.dev` + `www.…` + `.github/workflows/deploy-marketing.yml` + Wired (V-091 era)' + '`driftstack-docs` + `docs.driftstack.dev` + `.github/workflows/deploy-docs.yml` (V-258) + Wired (V-258)' + '`driftstack-customer-dashboard` + `app.driftstack.dev` + Not yet wired — Astro SSR via `@astrojs/cloudflare`; future V-NNN' + '`driftstack-admin-panel` + `admin.driftstack.dev` + (planned, V-135) + Not yet wired — gated on Cloudflare Access SSO config' — pinned so the 4-project (marketing + docs + customer-dashboard + admin-panel) inventory + 4-custom-domain-mapping + V-091-V-258-V-135 status-anchor + Cloudflare-Access-SSO gating commitment survives", () => {
    ...
    expect(body).toMatch(/Wired \(V-091 era\)/);
    expect(body).toMatch(/`driftstack-docs`/);
    expect(body).toMatch(/`docs\.driftstack\.dev`/);
    expect(body).toMatch(/`\.github\/workflows\/deploy-docs\.yml` \(V-258\)/);
    expect(body).toMatch(/Wired \(V-258\)/);
    expect(body).toMatch(/`driftstack-customer-dashboard`/);
    expect(body).toMatch(/`app\.driftstack\.dev`/);
    expect(body).toMatch(/Not yet wired — Astro SSR via `@astrojs\/cloudflare`; future V-NNN/);
    expect(body).toMatch(/`driftstack-admin-panel`/);
    expect(body).toMatch(/`admin\.driftstack\.dev`/);
    expect(body).toMatch(/\(planned, V-135\)/);
    expect(body).toMatch(/Not yet wired — gated on Cloudflare Access SSO config/);
  });
  ...
  it("Per-project A/B/C/D shape + V-219-oxblood-D-badge + V-135 admin-pre-staging framing pinned: '### A. `driftstack-marketing`' + 'Trigger first deploy: GitHub repo → **Actions** → **Deploy marketing site** → **Run workflow** → from `main`.' + 'Wire custom domains: CF Pages project → **Custom domains** → add `driftstack.dev` AND `www.driftstack.dev`. CF auto-creates DNS records if the zone is in this account.' + 'Verify <https://driftstack.dev>: `index.astro` should render with the V-219 brand identity (oxblood D-badge + Geist Sans + slate palette).' + '### B. `driftstack-docs`' + '### C. `driftstack-customer-dashboard`' + 'Pre-create the project so the future deploy workflow lands cleanly:' + 'The repo variable name will be `CLOUDFLARE_DASHBOARD_PROJECT_NAME` when the workflow ships; create the variable with that name + value `driftstack-customer-dashboard` to pre-stage.' + '### D. `driftstack-admin-panel`' + 'V-135 lands the admin-panel deploy + Cloudflare Access SSO gate.' + 'The custom domain (`admin.driftstack.dev`) wiring waits for V-135 because the Access policy attaches at the origin level and needs to be configured against the live Pages project.' + 'Repo variable name will be `CLOUDFLARE_ADMIN_PROJECT_NAME` when the workflow ships.' — pinned so the 4-section-A-B-C-D per-project shape + V-219 oxblood-D-badge + Geist-Sans + slate-palette + V-135 admin-panel-deploy + Cloudflare-Access-origin-level-policy + 3-future-repo-var-name (CLOUDFLARE_DASHBOARD_PROJECT_NAME + CLOUDFLARE_ADMIN_PROJECT_NAME) commitment survives", () => {
    ...
    expect(body).toMatch(/### C\. `driftstack-customer-dashboard` \(workflow: planned\)/);
    expect(body).toMatch(/Pre-create the project so the future deploy workflow lands cleanly:/);
    expect(body).toMatch(
      /The repo variable name will be `CLOUDFLARE_DASHBOARD_PROJECT_NAME` when the workflow ships; create the variable with that name \+ value `driftstack-customer-dashboard` to pre-stage\./,
    );
    expect(body).toMatch(/### D\. `driftstack-admin-panel` \(workflow: planned, V-135\)/);
    expect(body).toMatch(/V-135 lands the admin-panel deploy \+ Cloudflare Access SSO gate\./);
    expect(body).toMatch(
      /The custom domain \(`admin\.driftstack\.dev`\) wiring waits for V-135 because the Access policy attaches at the origin level and needs to be configured against the live Pages project\./,
    );
    expect(body).toMatch(
      /Repo variable name will be `CLOUDFLARE_ADMIN_PROJECT_NAME` when the workflow ships\./,
    );
  });
  ...
  it("Path-filter verification + free-tier-cost + rollback framing pinned: '## Verifying the workflows are wired correctly' + 'After completing A and B, trigger one push under `apps/marketing-site/**` AND one push under `apps/docs/**`. Each should trigger ONLY the matching workflow' + 'A trivial change under apps/marketing-site/ → only Deploy marketing site runs.' + 'A trivial change under apps/docs/ → only Deploy doc site runs.' + 'A change under apps/server/ → neither runs.' + 'This is the path-filter design preventing cross-deployment storms when only one app changes.' + '## Cost (informational)' + '500 builds per month per account.' + 'Unlimited bandwidth on static assets.' + '1 build at a time per project (the `concurrency` group in each workflow respects this).' + 'Post-launch, if the rate ever approaches 500/month, CF Pages Pro is $20/mo for 5,000 builds.' — pinned so the path-filter-cross-deployment-storm-prevention + 500-builds-month-free + Pages-Pro-$20mo-5000 commitment survives", () => {
    ...
    expect(body).toMatch(
      /After completing A and B, trigger one push under `apps\/marketing-site\/\*\*` AND one push under `apps\/docs\/\*\*`\./,
    );
    expect(body).toMatch(
    ...
    expect(body).toMatch(/# A change under apps\/server\/ → neither runs\./);
    expect(body).toMatch(
    ...
    expect(body).toMatch(/- 500 builds per month per account\./);
    ...
    expect(body).toMatch(
      /Post-launch, if the rate ever approaches 500\/month, CF Pages Pro is \$20\/mo for 5,000 builds\./,
    );
  });
