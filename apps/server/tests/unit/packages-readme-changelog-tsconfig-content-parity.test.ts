...
describe('W618 packages READMEs + CHANGELOGs + tsconfigs content parity', () => {
  ...
  it('sdk-go/README.md: alpha + zero-non-stdlib runtime deps + context-aware throughout + go get url + Go 1.21+ + driftstack.New(apiKey)+Close+ctx-first Quickstart + 11-row Resources accessor table (Sessions/Profiles/ProfileSnapshots V-312/APIKeys V-296/Usage etc) pinned', () => {
    ...
    expect(body).toMatch(
      /`client\.Sessions`\s+\| `Create`, `List`, `Get`, `Navigate`, `Interact`, `Wait`, `GetState`, `Capture`, `Extract`, `Search`, `Destroy`/,
    );
    expect(body).toMatch(
  ...
  it('sdk-python/README.md: alpha not-on-PyPI yet + driftstack-sdk dist name / driftstack import name + Python 3.10+ + sync Driftstack + async AsyncDriftstack + 10-row Resources accessor table (sessions/profiles/profile_snapshots V-312/api_keys V-296/usage/webhooks V-307/team V-298/billing/auth/account V-385) pinned', () => {
    ...
    expect(body).toMatch(/^## Resources$/m);
    expect(body).toMatch(
      /`client\.sessions`\s+\| `create`, `list`, `get`, `navigate`, `interact`, `wait`, `get_state`, `capture`, `extract`, `search`, `destroy`/,
    );
    expect(body).toMatch(
