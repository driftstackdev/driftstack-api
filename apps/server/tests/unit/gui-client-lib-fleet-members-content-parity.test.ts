...
describe('W469.B apps/gui-client/src/lib/fleet-members.ts content parity', () => {
  ...
  it("V-346 framing pinned: 'V-346 — Fleet member registry. CRUD over tauri-plugin-store.' + 'A \"fleet member\" is a Driftstack control-plane API server the founder runs locally on a Mac mini in a fleet (i.e. multiple Mac minis, each running an api-server instance + driver, fronted by the GUI). Each entry is just a label + base URL; the fleet view pings each member's /version on demand to surface status.'", () => {
    ...
    expect(body).toMatch(
      /\/\/ A "fleet member" is a Driftstack control-plane API server the\s*\/\/ founder runs locally on a Mac mini in a fleet \(i\.e\. multiple\s*\/\/ Mac minis, each running an api-server instance \+ driver, fronted\s*\/\/ by the GUI\)\. Each entry is just a label \+ base URL; the fleet\s*\/\/ view pings each member's \/version on demand to surface status\./,
    );
  });
  ...
  it("Mirrors-V-244 framing pinned: 'Stored under settings.json keyed separately from settings + proxies (mirrors the V-244 proxies.ts pattern). Local-only — no server round-trip; the founder owns the fleet topology.'", () => {
    expect(body).toMatch(
      /\/\/ Stored under settings\.json keyed separately from settings \+ proxies\s*\/\/ \(mirrors the V-244 proxies\.ts pattern\)\. Local-only — no server\s*\/\/ round-trip; the founder owns the fleet topology\./,
    );
  });
  ...
  it("FleetMember 5-field: id + label + baseUrl 'same shape as the GUI's primary base URL' + notes nullable + createdAt 'ISO8601 created timestamp. Hand-set by the GUI; not server-authoritative.'", () => {
    expect(body).toMatch(
      /export interface FleetMember \{\s*id: string;\s*label: string;\s*\/\*\* e\.g\. http:\/\/10\.0\.0\.5:3000 — same shape as the GUI's primary base URL\. \*\/\s*baseUrl: string;\s*notes: string \| null;\s*\/\*\* ISO8601 created timestamp\. Hand-set by the GUI; not server-authoritative\. \*\/\s*createdAt: string;\s*\}/,
    );
  });

  it("FleetMemberDraft 3-field (label + baseUrl + notes — no id/createdAt) + STORE_FILE 'settings.json' + FLEET_KEY 'fleetMembers'", () => {
    expect(body).toMatch(
      /export interface FleetMemberDraft \{\s*label: string;\s*baseUrl: string;\s*notes: string \| null;\s*\}/,
    );
  ...
  it("addFleetMember + updateFleetMember: baseUrl.replace(/\\/+$/, '') trailing-slash strip on BOTH paths (downstream `${baseUrl}/version` would double-slash without it)", () => {
    expect(body).toMatch(
      /export async function addFleetMember\(draft: FleetMemberDraft\): Promise<FleetMember> \{\s*return writeLock\(async \(\) => \{\s*const all = await listFleetMembersUnlocked\(\);\s*const next: FleetMember = \{\s*id: mintId\(\),\s*label: draft\.label,\s*baseUrl: draft\.baseUrl\.replace\(\/\\\/\+\$\/, ''\),/,
    );
    expect(body).toMatch(
      /const updated: FleetMember = \{\s*\.\.\.\(all\[idx\] as FleetMember\),\s*label: patch\.label,\s*baseUrl: patch\.baseUrl\.replace\(\/\\\/\+\$\/, ''\),\s*notes: patch\.notes,\s*\};/,
    );
  });
  ...
  it("validateDraft: label.trim().length === 0 → 'Required.' + baseUrl.trim().length === 0 → 'Required.' + new URL() try/catch → 'Invalid URL.' + protocol must be http:/https: → 'Must be http:// or https:// URL.'", () => {
    expect(body).toMatch(
      /export function validateDraft\(d: FleetMemberDraft\): DraftValidation \{\s*const errors: Partial<Record<keyof FleetMemberDraft, string>> = \{\};\s*if \(d\.label\.trim\(\)\.length === 0\) errors\.label = 'Required\.';\s*if \(d\.baseUrl\.trim\(\)\.length === 0\) \{\s*errors\.baseUrl = 'Required\.';\s*\} else \{\s*try \{\s*const u = new URL\(d\.baseUrl\);\s*if \(u\.protocol !== 'http:' && u\.protocol !== 'https:'\) \{\s*errors\.baseUrl = 'Must be http:\/\/ or https:\/\/ URL\.';\s*\}\s*\} catch \{\s*errors\.baseUrl = 'Invalid URL\.';\s*\}\s*\}/,
    );
  });
  ...
  it("FleetMemberPing 6-field: ok + durationMs + version optional 'From /version on the target. Undefined when ping failed.' + driver 3-union ('mock'|'webkit'|'playwright') + playwrightBrowser 3-union ('webkit'|'chromium'|'firefox') + error optional 'When ping failed: short human-readable error.'", () => {
    expect(body).toMatch(
      /export interface FleetMemberPing \{\s*ok: boolean;\s*durationMs: number;\s*\/\*\* From \/version on the target\. Undefined when ping failed\. \*\/\s*version\?: string;\s*driver\?: 'mock' \| 'webkit' \| 'playwright';\s*playwrightBrowser\?: 'webkit' \| 'chromium' \| 'firefox';\s*\/\*\* When ping failed: short human-readable error\. \*\/\s*error\?: string;\s*\}/,
    );
  });
  ...
  it("pingFleetMember: V-346 framing 'fetch /version on the member; resolves with shape suitable for the FleetView. Uses fetch with a 5s timeout via AbortController.' + performance.now() duration + 5000ms AbortController.abort + fetch with signal + !res.ok → `HTTP ${res.status.toString()}` error fallback", () => {
    expect(body).toMatch(
      /\*\s*V-346 — fetch \/version on the member; resolves with shape suitable\s*\*\s*for the FleetView\. Uses fetch with a 5s timeout via AbortController\./,
    );
    expect(body).toMatch(
