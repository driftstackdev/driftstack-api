...
describe('W437.A apps/server/src/routes/sessions.ts content parity', () => {
  ...
  it('imports: 6 Zod schemas from @driftstack/api-types + SessionRecord/SessionsService + GUIInputRequestSchema + BadRequest/Forbidden errors + AccountAuthRepo + resolveEffectiveAccount', () => {
    expect(body).toMatch(
  ...
  it("V-326e1 POST /v1/sessions framing pinned: X-Driftstack-Account → new session on OWNER's account; caller role MUST be admin on that team (Q1 — member is read-only on writes); member role 403; tier-derived concurrent cap uses OWNER tier", () => {
    ...
    expect(body).toMatch(
      /app\.post\(\s*\n?\s*'\/v1\/sessions',\s*\n?\s*\{\s*\n?\s*preHandler: \[app\.requireAuth, app\.rateLimit\('sessions:create'\)\],\s*\n?\s*\},/,
    );
    expect(body).toMatch(
