...
describe('W463.A apps/docs/src/data/nav.ts content parity (S22.2 all-50-routes tree + S22.4 endpoint children)', () => {
  ...
  it('total tree size: 61 top-level routes + 143 child anchors, including the archetype catalog and its list endpoint. V-847 raised it from 141 — V-843 documented the crypto quote and receipt endpoints and V-846 gave them nav children, but this census lives in a DIFFERENT file from the one V-846 raised, so it kept the old number', () => {
    ...
    expect(anchors).toHaveLength(143);
    ...
    expect(new Set(anchors).size).toBe(143);
  });
