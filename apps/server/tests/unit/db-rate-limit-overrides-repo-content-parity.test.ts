...
describe('W444.B apps/server/src/db/rate-limit-overrides-repo.ts content parity', () => {
  ...
  it('Upsert-by-(account_id,bucket_key) framing pinned: "Drizzle-backed RateLimitOverridesRepo. Upsert by (account_id, bucket_key) — re-setting the same bucket replaces the prior override."', () => {
    expect(body).toMatch(
      /\/\/ Drizzle-backed RateLimitOverridesRepo\. Upsert by \(account_id,\s*\/\/ bucket_key\) — re-setting the same bucket replaces the prior override\./,
    );
  });
  ...
  it('imports: SQL type + and/desc/eq/gt/lt; RateLimitOverrideRecord/Repo + SetOverrideInput; Database; rateLimitOverrides schema', () => {
    ...
    expect(body).toMatch(
      /import type \{\s*RateLimitOverrideRecord,\s*RateLimitOverridesRepo,\s*SetOverrideInput,\s*\} from '\.\.\/services\/rate-limit-overrides\.js';/,
    );
  ...
  it('clear: account+bucket-scoped delete returning {id}; returns result.length > 0', () => {
    expect(body).toMatch(
      /async clear\(accountId: string, bucketKey: string\): Promise<boolean> \{\s*const result = await this\.database\.db\s*\.delete\(rateLimitOverrides\)\s*\.where\(\s*and\(\s*eq\(rateLimitOverrides\.accountId, accountId\),\s*eq\(rateLimitOverrides\.bucketKey, bucketKey\),\s*\),\s*\)\s*\.returning\(\{ id: rateLimitOverrides\.id \}\);\s*return result\.length > 0;\s*\}/,
    );
  });
