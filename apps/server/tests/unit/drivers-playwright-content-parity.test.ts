...
describe('W454.C apps/server/src/drivers/playwright.ts content parity', () => {
  ...
  it("V-333b framing pinned: 'V-333b — Playwright-backed driver. Foundation for self-hosted local dev + E2E testing on the Mac BEFORE the WebKit fork (Agent 1) lands the production driver. Selecting via DRIVER=playwright + PLAYWRIGHT_BROWSER=webkit|chromium|firefox.'", () => {
    expect(body).toMatch(
      /\/\/ V-333b — Playwright-backed driver\. Foundation for self-hosted local\s*\n?\s*\/\/ dev \+ E2E testing on the Mac BEFORE the WebKit fork \(Agent 1\) lands\s*\n?\s*\/\/ the production driver\. Selecting via DRIVER=playwright \+\s*\n?\s*\/\/ PLAYWRIGHT_BROWSER=webkit\|chromium\|firefox\./,
    );
  });
  ...
  it("Trade-offs framing pinned: 'Playwright's WebKit channel is the same WebKit upstream; it does NOT include the Driftstack source modifications.' + no iPhone-archetype mapping (viewport + UA only) + single-context-per-session", () => {
    expect(body).toMatch(
      /\/\/\s*- Playwright's WebKit channel is the same WebKit upstream; it\s*\n?\s*\/\/\s*does NOT include the Driftstack source modifications\. Sites\s*\n?\s*\/\/\s*that fingerprint at the WebKit C\+\+ layer will see "vanilla"\s*\n?\s*\/\/\s*WebKit\. Adequate for non-stealth E2E tests; insufficient for\s*\n?\s*\/\/\s*evaluating fingerprint quality against production targets\./,
    );
    expect(body).toMatch(
      /\/\/\s*- No iPhone-archetype mapping \(no audio\/screen\/HW concurrency\s*\n?\s*\/\/\s*overrides\)\. Each archetype just maps to a viewport \+ UA string\s*\n?\s*\/\/\s*that approximates iPhone-Safari\. The real driver does much\s*\n?\s*\/\/\s*more\./,
    );
    expect(body).toMatch(
      /\/\/\s*- Single-context-per-session keeps memory bounded; no profile\s*\n?\s*\/\/\s*persistence yet \(V-333c follow-on\)\./,
    );
  });

  it('What-this-driver-IS-for framing pinned: founder local dev + CI E2E without spinning up WebKit fork', () => {
    expect(body).toMatch(
      /\/\/ What this DRIVER IS for:\s*\n?\s*\/\/\s*- Founder running the API \+ GUI on their Mac and verifying the\s*\n?\s*\/\/\s*full session create\/navigate\/destroy flow against a real\s*\n?\s*\/\/\s*browser without waiting on Agent 1\./,
    );
    expect(body).toMatch(
      /\/\/\s*- CI E2E tests of the route layer \+ service layer against a\s*\n?\s*\/\/\s*real browser, without spinning up the WebKit fork\./,
    );
  });
  ...
  it("Production-rejection framing pinned: 'What this DRIVER is NOT for: Production customer traffic. The WebKit fork is the only production-eligible driver. PlaywrightDriver is dev/test only.'", () => {
    expect(body).toMatch(
      /\/\/ What this DRIVER is NOT for:\s*\n?\s*\/\/\s*- Production customer traffic\. The WebKit fork is the only\s*\n?\s*\/\/\s*production-eligible driver\. PlaywrightDriver is dev\/test only\./,
    );
  });

  it('imports: chromium/firefox/webkit + Browser/BrowserContext/Page types from @playwright/test; BadRequestError + DriverNotIntegratedError from ../lib/errors; 12 driver types', () => {
  ...
  it("PlaywrightBrowserKind 3-value union; PlaywrightDriverConfig: browserKind required + optional headed framing 'true to launch a visible browser window (great for local Mac dev / debugging). false to run headless (CI). Default false.'", () => {
    ...
    expect(body).toMatch(
      /\*\s*`true` to launch a visible browser window \(great for local Mac\s*\n?\s*\*\s*dev \/ debugging\)\. `false` to run headless \(CI\)\. Default false\./,
    );
  });
  ...
  it('getBrowser: 3-branch ternary (chromium|firefox|webkit default); browserPromise lazy-init; HEADED-INVERSION GUARD: headless: this.config.headed !== true (dropping !== inverts behavior)', () => {
    expect(body).toMatch(
      /const launcher =\s*\n?\s*this\.config\.browserKind === 'chromium'\s*\n?\s*\? chromium\s*\n?\s*: this\.config\.browserKind === 'firefox'\s*\n?\s*\? firefox\s*\n?\s*: webkit;/,
    );
    expect(body).toMatch(
  ...
  it("createSession framing pinned: 'best-effort iPhone-archetype mapping. Real archetypes are richer (HW concurrency, audio API tells, etc.); this just sets the viewport + UA so basic fingerprinting works.' + 5-field newContext (viewport + userAgent + deviceScaleFactor:3 + isMobile:true + hasTouch:true)", () => {
    expect(body).toMatch(
      /\/\/ V-333b — best-effort iPhone-archetype mapping\. Real archetypes\s*\n?\s*\/\/ are richer \(HW concurrency, audio API tells, etc\.\); this just\s*\n?\s*\/\/ sets the viewport \+ UA so basic fingerprinting works\./,
    );
    expect(body).toMatch(
      /const context = await browser\.newContext\(\{\s*\n?\s*viewport,\s*\n?\s*userAgent,\s*\n?\s*deviceScaleFactor: 3,\s*\n?\s*isMobile: true,\s*\n?\s*hasTouch: true,\s*\n?\s*\}\);/,
    );
  });
  ...
  it("Session id framing pinned: 'Synthetic session id — playwright doesn't expose a stable public guid on BrowserContext, so we generate our own. Format: pw_<random-hex>_<unix-ms-base36>.'", () => {
    expect(body).toMatch(
      /\/\/ Synthetic session id — playwright doesn't expose a stable\s*\n?\s*\/\/ public guid on BrowserContext, so we generate our own\.\s*\n?\s*\/\/ Format: pw_<random-hex>_<unix-ms-base36>\./,
    );
    expect(body).toMatch(
      /const sessionId = `pw_\$\{Math\.floor\(Math\.random\(\) \* 0xffffffff\)\s*\n?\s*\.toString\(16\)\s*\n?\s*\.padStart\(8, '0'\)\}_\$\{Date\.now\(\)\.toString\(36\)\}`;/,
    );
  });
  ...
  it("navigate: 3-branch ternary on input.waitUntil ('networkidle'|'load'|else 'domcontentloaded' default); response status fallback 0", () => {
    expect(body).toMatch(
      /waitUntil:\s*\n?\s*input\.waitUntil === 'networkidle'\s*\n?\s*\? 'networkidle'\s*\n?\s*: input\.waitUntil === 'load'\s*\n?\s*\? 'load'\s*\n?\s*: 'domcontentloaded',/,
    );
  ...
  it("interact / guiInput / wait → throw DriverNotIntegratedError framing pinned 'PlaywrightDriver returns a NotIntegrated shape for surfaces that haven't been minimally wired so the gap is loud + visible.'", () => {
    expect(body).toMatch(
      /\/\/ V-333b — interact \/ wait \/ capture \/ getState have minimal\s*\n?\s*\/\/ Playwright mappings sufficient for E2E smoke testing\. The full\s*\n?\s*\/\/ semantic surface \(recipe library, behavioral simulation,\s*\n?\s*\/\/ recapture-automation\) is NOT modeled here; that's the production\s*\n?\s*\/\/ WebKitDriver's job\. PlaywrightDriver returns a NotIntegrated\s*\n?\s*\/\/ shape for surfaces that haven't been minimally wired so the gap\s*\n?\s*\/\/ is loud \+ visible\./,
    );
    expect(body).toMatch(
      /async interact\([\s\S]*?\): Promise<InteractResult> \{\s*\n?\s*await Promise\.resolve\(\);\s*\n?\s*throw new DriverNotIntegratedError\(\);\s*\n?\s*\}/,
    );
    expect(body).toMatch(
      /async guiInput\([\s\S]*?\): Promise<GUIInputResult> \{\s*\n?\s*await Promise\.resolve\(\);\s*\n?\s*throw new DriverNotIntegratedError\(\);\s*\n?\s*\}/,
    );
    expect(body).toMatch(
      /async wait\([\s\S]*?\): Promise<WaitResult> \{\s*\n?\s*await Promise\.resolve\(\);\s*\n?\s*throw new DriverNotIntegratedError\(\);\s*\n?\s*\}/,
    );
  });
  ...
  it("capture: 'screenshot' branch page.screenshot({fullPage}) + base64 encoding; non-screenshot ('state'/'pdf') → throw DriverNotIntegratedError; framing pinned 'state / pdf fall through; not yet wired.'", () => {
    expect(body).toMatch(
      /if \(input\.kind === 'screenshot'\) \{\s*\n?\s*const buf = await entry\.page\.screenshot\(\{ fullPage: input\.fullPage \}\);\s*\n?\s*return \{\s*\n?\s*kind: 'screenshot',\s*\n?\s*data: buf\.toString\('base64'\),\s*\n?\s*encoding: 'base64',\s*\n?\s*byteSize: buf\.byteLength,\s*\n?\s*durationMs: Date\.now\(\) - start,\s*\n?\s*\};\s*\n?\s*\}/,
    );
    expect(body).toMatch(
      /\/\/ 'state' \/ 'pdf' fall through; not yet wired\.\s*\n?\s*throw new DriverNotIntegratedError\(\);/,
    );
  });

  it('destroy: context.close().catch(swallow); requireSession: throws BadRequestError on missing session', () => {
    expect(body).toMatch(
      /await entry\.context\.close\(\)\.catch\(\(\) => \{\s*\n?\s*\/\* swallow — best-effort \*\/\s*\n?\s*\}\);/,
    );
    expect(body).toMatch(
      /private requireSession\(sessionId: DriverSessionId\): SessionEntry \{\s*\n?\s*const entry = this\.sessions\.get\(sessionId\);\s*\n?\s*if \(!entry\) \{\s*\n?\s*throw new BadRequestError\(`Driver session "\$\{sessionId\}" not found\.`\);\s*\n?\s*\}\s*\n?\s*return entry;\s*\n?\s*\}/,
    );
  });
  ...
  it("approximateViewport: 3-case (iphone-16-pro 402x874 + iphone-15 393x852 + iphone-14 390x844) + default iPhone 16 Pro framing 'Defaults to iPhone 16 Pro dimensions when the archetype is unfamiliar.'", () => {
    expect(body).toMatch(
      /\* Best-effort archetype → viewport mapping\. Defaults to iPhone 16\s*\n?\s*\*\s*Pro dimensions when the archetype is unfamiliar\./,
    );
    expect(body).toMatch(
  ...
  it('approximateUserAgent: ios-26 special case (WebKit 619.x family Safari 17.4) + default (iOS 17.4 fallback) framing pinned \'Real driver picks a UA per the exact iOS / Safari combo; this is "good enough" for fingerprint-permissive sites.\'', () => {
    expect(body).toMatch(
      /\* Best-effort archetype → UA mapping\. Real driver picks a UA per the\s*\n?\s*\*\s*exact iOS \/ Safari combo; this is "good enough" for fingerprint-\s*\n?\s*\*\s*permissive sites\./,
    );
