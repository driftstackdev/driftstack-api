...
describe('W453.C packages/behavioural-simulation/src/dwell.ts content parity', () => {
  ...
  it("V-530.C framing pinned: 'V-530.C — dwell time models + element-region-aware click-position bias.' + 'Third sub-slice of V-530 (per the anti-substitution clause).' + two-refinement framing (dwell distribution shape + element-region-aware click bias)", () => {
    ...
    expect(body).toMatch(
      /\/\/ Third sub-slice of V-530 \(per the anti-substitution clause\)\. Extends\s*\n?\s*\/\/ the touch event distributions from V-530\.A with two refinements:/,
    );
    expect(body).toMatch(
      /\/\/\s*1\. Dwell time models — beyond a flat per-class mean, model the dwell\s*\n?\s*\/\/\s*distribution shape \(lognormal-flavoured for long-tailed dwell, vs\s*\n?\s*\/\/\s*tight gaussian for quick taps\)\./,
    );
    expect(body).toMatch(
      /\/\/\s*2\. Element-region-aware click-position bias — within an element,\s*\n?\s*\/\/\s*humans don't tap uniformly: buttons get tapped near the visual\s*\n?\s*\/\/\s*affordance edge, links near the underlined text region, images\s*\n?\s*\/\/\s*near features \(face \/ focal point\)\./,
    );
  });
  ...
  it("DwellShape 3-value union ('tight'|'normal'|'long-tailed'); per-shape framing pinned 'tight — narrow gaussian around the mean. Snappy taps (button-like).' + 'long-tailed — log-skewed; mean unchanged, right tail extended.'", () => {
    expect(body).toMatch(
      /\*\s*'tight'\s*— narrow gaussian around the mean\. Snappy taps \(button-like\)\.\s*\n?\s*\*\s*'normal' — moderate gaussian\. Typical mixed interaction\.\s*\n?\s*\*\s*'long-tailed' — log-skewed; mean unchanged, right tail extended\./,
    );
  ...
  it("ClickRegion: 3-field (center fraction + radius fraction 'max 0.5' + weight relative 'generator normalises across regions')", () => {
    expect(body).toMatch(
      /export interface ClickRegion \{[\s\S]*?\/\*\* Region centre as fraction of element bounds \(0\.\.1\)\. \*\/\s*\n?\s*center: \{ x: number; y: number \};[\s\S]*?\/\*\* Region radius as fraction of element bounds \(0\.\.1, max 0\.5\)\. \*\/\s*\n?\s*radius: \{ x: number; y: number \};[\s\S]*?\/\*\* Probability weight \(relative\)\. The generator normalises across regions\. \*\/\s*\n?\s*weight: number;/,
    );
  });
  ...
  it('CLICK_REGIONS framing pinned: \'Real-world taps on a button are tightly clustered near the visual affordance edge (where finger pressure feels "right"), not perfectly centred — but close enough that one centre-biased region captures the essence. Images get 2 regions (centre for focal-point content + upper-half for caption-tap on annotated images).\'', () => {
    expect(body).toMatch(
      /\*\s*Real-world taps on a button are tightly clustered near the visual\s*\n?\s*\*\s*affordance edge \(where finger pressure feels "right"\), not perfectly\s*\n?\s*\*\s*centred — but close enough that one centre-biased region captures the\s*\n?\s*\*\s*essence\. Images get 2 regions \(centre for focal-point content \+\s*\n?\s*\*\s*upper-half for caption-tap on annotated images\)\./,
    );
  });
  ...
  it('CLICK_REGIONS table: button 1-region (0.5,0.5, radius 0.18×0.22, weight 1.0); link 1-region with 0.45 y-offset (text baseline above geometric centre rationale); input 1-region at 0.4 x (placeholder text area rationale); image 2-region (0.7 focal-point + 0.3 upper); video 2-region (0.8 play lower + 0.2 close upper-right); scroll-container 1-region; generic 1-region', () => {
    expect(body).toMatch(
      /button: \[\s*\n?\s*\{\s*\n?\s*center: \{ x: 0\.5, y: 0\.5 \},\s*\n?\s*radius: \{ x: 0\.18, y: 0\.22 \},\s*\n?\s*weight: 1\.0,\s*\n?\s*\},\s*\n?\s*\],/,
    );
    expect(body).toMatch(
      /\/\/ Underlined text region tends to be slightly above element centre\s*\n?\s*\/\/ \(text baseline is above geometric centre for typical link styling\)\./,
    );
    expect(body).toMatch(
  ...
  it("sampleDwellMultiplier framing pinned: 'Mean is 1.0 across all shapes; only the distribution shape differs.' + tight σ≈0.08 + normal σ≈0.18 + long-tailed via lognormal exp(N(0, σ)) with E[X]=1 mean-correction (μ=-σ²/2)", () => {
    expect(body).toMatch(
      /\*\s*Sample a dwell-time multiplier from the requested shape\. Mean is 1\.0\s*\n?\s*\*\s*across all shapes; only the distribution shape differs\./,
    );
    expect(body).toMatch(
      /\*\s*- tight: gaussian-ish \(sum of 3 uniforms\), σ ≈ 0\.08\.\s*\n?\s*\*\s*- normal: gaussian-ish, σ ≈ 0\.18\.\s*\n?\s*\*\s*- long-tailed: log-skewed via exp\(N\(0, σ\)\)\. Right tail extends to ~3x\s*\n?\s*\*\s*while preserving mean ≈ 1\.0 by the lognormal mean-correction\s*\n?\s*\*\s*exp\(σ²\/2\)\./,
    );
    expect(body).toMatch(
      /\/\/ Lognormal with E\[X\] = exp\(μ \+ σ²\/2\)\. To get mean = 1, set μ = -σ²\/2\.\s*\n?\s*return Math\.exp\(-\(\(sigma \* sigma\) \/ 2\) \+ sigma \* z\);/,
    );
  });

  it('Box-Muller long-tailed sampling: u1=max(rng(), 1e-9) (avoid log(0)); z=sqrt(-2*log(u1))*cos(2*PI*u2); sigma=0.4', () => {
    expect(body).toMatch(
      /\/\/ Box-Muller → normal\(0, 1\); shape via lognormal\.\s*\n?\s*const u1 = Math\.max\(rng\(\), 1e-9\);\s*\n?\s*const u2 = rng\(\);\s*\n?\s*const z = Math\.sqrt\(-2 \* Math\.log\(u1\)\) \* Math\.cos\(2 \* Math\.PI \* u2\);\s*\n?\s*const sigma = 0\.4;/,
    );
  });
  ...
  it("RegionAwareTouchEvent extends TouchEvent + 3 extra fields (selectedRegionIndex 0-indexed 'Lets callers report or test the region-weighting empirically' + dwellShape + dwellMultiplier 1.0=at-class-mean framing)", () => {
    expect(body).toMatch(
      /export interface RegionAwareTouchEvent extends TouchEvent \{[\s\S]*?\/\*\*\s*\n?\s*\*\s*Which region \(0-indexed into the class's region map\) the click landed\s*\n?\s*\*\s*in\. Lets callers report or test the region-weighting empirically\.\s*\n?\s*\*\/\s*\n?\s*selectedRegionIndex: number;[\s\S]*?dwellShape: DwellShape;[\s\S]*?\/\*\* The dwell multiplier sampled \(1\.0 = at-class-mean\)\. \*\/\s*\n?\s*dwellMultiplier: number;/,
    );
  });
  ...
  it('generateRegionAwareTouchEvent: throws on width/height <= 0 with bounds context; throws on empty regions; seed = `region-touch:${elementClass}:${JSON.stringify(bounds)}`', () => {
    expect(body).toMatch(
      /if \(opts\.bounds\.width <= 0 \|\| opts\.bounds\.height <= 0\) \{\s*\n?\s*throw new Error\(\s*\n?\s*`generateRegionAwareTouchEvent: bounds must have positive width \+ height ` \+\s*\n?\s*`\(got width=\$\{opts\.bounds\.width\}, height=\$\{opts\.bounds\.height\}\)`,\s*\n?\s*\);/,
    );
    expect(body).toMatch(
      /if \(regions\.length === 0\) \{\s*\n?\s*throw new Error\(\s*\n?\s*`generateRegionAwareTouchEvent: at least one region required for \$\{opts\.elementClass\}`,/,
    );
    expect(body).toMatch(
  ...
  it('Region-local bounds shrink: regionLeft/Top via centre - radius; regionWidth/Height via radius × 2; max(1, ...) floor; passes shrunken bounds to generateTouchEvent with region-suffixed seed', () => {
    expect(body).toMatch(
      /const regionLeft = opts\.bounds\.x \+ opts\.bounds\.width \* \(region\.center\.x - region\.radius\.x\);\s*\n?\s*const regionTop = opts\.bounds\.y \+ opts\.bounds\.height \* \(region\.center\.y - region\.radius\.y\);\s*\n?\s*const regionWidth = opts\.bounds\.width \* region\.radius\.x \* 2;\s*\n?\s*const regionHeight = opts\.bounds\.height \* region\.radius\.y \* 2;/,
    );
    expect(body).toMatch(
      /const regionBounds: ElementBounds = \{\s*\n?\s*x: regionLeft,\s*\n?\s*y: regionTop,\s*\n?\s*width: Math\.max\(1, regionWidth\),\s*\n?\s*height: Math\.max\(1, regionHeight\),\s*\n?\s*\};/,
    );
