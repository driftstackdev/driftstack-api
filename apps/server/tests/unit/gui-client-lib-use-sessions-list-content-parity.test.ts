...
describe('W472.B apps/gui-client/src/lib/use-sessions-list.ts content parity', () => {
  ...
  it("Same V-534 polling state-machine pattern: SessionsListState 4-variant union + manual?-aware initial state + no-apiKey 'No API key configured.' + readApiErrorMessage delegation + instance-of-Error catch + useEffect manual gate; useCallback deps [settings.apiKey, settings.baseUrl, limit]", () => {
    expect(body).toMatch(
    ...
    expect(body).toMatch(/\}, \[settings\.apiKey, settings\.baseUrl, limit\]\);/);
    expect(body).toMatch(
