...
describe('W435.B packages/api-types/src/admin.ts content parity', () => {
  ...
  it('Admin API contracts framing pinned: /v1/admin/* routes require admin scope (D-012 + D-025); schemas describe public request/response shape', () => {
    expect(body).toMatch(
      /\/\/ Admin API contracts\. Routes under \/v1\/admin\/\* require the admin\s*\n?\s*\/\/ scope \(see D-012 \+ D-025\)\. These schemas describe the public shape\s*\n?\s*\/\/ of admin requests and responses\./,
    );
  });
  ...
  it('V-218 ValidationSchedule: id uuid + archetype_id + cadence_seconds positive int + enabled + last_run_at nullable + next_run_at + last_run_id nullable + reason nullable + created/updated_at', () => {
    ...
    expect(body).toMatch(
      /export const ValidationScheduleSchema = z\.object\(\{\s*\n?\s*id: z\.string\(\)\.uuid\(\),\s*\n?\s*archetype_id: z\.string\(\),\s*\n?\s*cadence_seconds: z\.number\(\)\.int\(\)\.positive\(\),\s*\n?\s*enabled: z\.boolean\(\),\s*\n?\s*last_run_at: z\.string\(\)\.nullable\(\),\s*\n?\s*next_run_at: z\.string\(\),\s*\n?\s*last_run_id: z\.string\(\)\.nullable\(\),\s*\n?\s*reason: z\.string\(\)\.nullable\(\),\s*\n?\s*created_at: z\.string\(\),\s*\n?\s*updated_at: z\.string\(\),\s*\n?\s*\}\);/,
    );
  });
  ...
  it('UpsertValidationSchedule: archetype_id min 1 + cadence_seconds int 60..1y + enabled optional default true + reason max 500 optional; ListValidationSchedulesResponse: data array', () => {
    expect(body).toMatch(
      /export const UpsertValidationScheduleRequestSchema = z\.object\(\{\s*\n?\s*archetype_id: z\.string\(\)\.min\(1\),\s*\n?\s*cadence_seconds: z\s*\n?\s*\.number\(\)\s*\n?\s*\.int\(\)\s*\n?\s*\.min\(60\)\s*\n?\s*\.max\(60 \* 60 \* 24 \* 365\),\s*\n?\s*enabled: z\.boolean\(\)\.optional\(\)\.default\(true\),\s*\n?\s*reason: z\.string\(\)\.max\(500\)\.optional\(\),\s*\n?\s*\}\);/,
    );
    expect(body).toMatch(
      /export const ListValidationSchedulesResponseSchema = z\.object\(\{\s*\n?\s*data: z\.array\(ValidationScheduleSchema\),\s*\n?\s*\}\);/,
    );
  });

  it('ChangeTier / SuspendAccount / UnsuspendAccount: tier + audit-row reason max 500 optional', () => {
    expect(body).toMatch(
      /export const ChangeTierRequestSchema = z\.object\(\{\s*\n?\s*tier: AccountTierSchema,\s*\n?\s*\/\*\* Optional human-readable reason recorded in the audit row\. \*\/\s*\n?\s*reason: z\.string\(\)\.max\(500\)\.optional\(\),\s*\n?\s*\}\);/,
    );
    expect(body).toMatch(
      /export const SuspendAccountRequestSchema = z\.object\(\{\s*\n?\s*\/\*\* Optional reason recorded in the audit row\. \*\/\s*\n?\s*reason: z\.string\(\)\.max\(500\)\.optional\(\),\s*\n?\s*\}\);/,
    );
    expect(body).toMatch(
      /export const UnsuspendAccountRequestSchema = z\.object\(\{\s*\n?\s*reason: z\.string\(\)\.max\(500\)\.optional\(\),\s*\n?\s*\}\);/,
    );
  });
  ...
  it('V-281 AddSupportNote framing pinned: free-form admin note on audit log; operator context the log does not auto-capture (post-incident summary / support call notes / out-of-band action receipts); audit-only, never touches billing/sessions/keys', () => {
    expect(body).toMatch(
      /\*\s*Records a free-form admin note on the customer's audit log\. Operator\s*\n?\s*\*\s*uses this to attach context the audit log doesn't capture\s*\n?\s*\*\s*automatically — post-incident summary, customer-support call notes,\s*\n?\s*\*\s*out-of-band action receipts\./,
    );
    expect(body).toMatch(
      /\*\s*Audit-only: never touches billing \/ sessions \/ keys\. Recording does\s*\n?\s*\*\s*not produce a side effect on the account state\./,
    );
    expect(body).toMatch(
      /export const AddSupportNoteRequestSchema = z\.object\(\{\s*\n?\s*note: z\.string\(\)\.min\(1\)\.max\(2000\),\s*\n?\s*\}\);/,
    );
  });
  ...
  it('V-281 RecordRefund framing pinned: operator records manual Stripe-dashboard refund; endpoint does NOT call Stripe (money movement out-of-band); audit row = post-action receipt for compliance + support follow-up; V-280 launch runbook + founder tier-3 boundary on direct financial actions', () => {
    expect(body).toMatch(
      /\*\s*Records that the operator manually issued a refund via the Stripe\s*\n?\s*\*\s*dashboard\. The endpoint does NOT call Stripe\. Money movement happens\s*\n?\s*\*\s*out-of-band; the audit row is the post-action receipt for compliance\s*\n?\s*\*\s*and customer support follow-up\./,
    );
    expect(body).toMatch(
      /\*\s*Per V-280 launch-day-runbook \+ the founder's tier-3 boundary on\s*\n?\s*\*\s*direct financial actions\./,
    );
    expect(body).toMatch(
      /export const RecordRefundRequestSchema = z\.object\(\{\s*\n?\s*\/\*\* The Stripe charge \/ payment_intent \/ invoice id refunded\. \*\/\s*\n?\s*external_reference: z\.string\(\)\.min\(3\)\.max\(120\),\s*\n?\s*\/\*\* Refund amount in cents\. May be partial\. \*\/\s*\n?\s*amount_cents: z\.number\(\)\.int\(\)\.positive\(\),\s*\n?\s*\/\*\* Currency ISO 4217; defaults to USD if omitted\. \*\/\s*\n?\s*currency: z\.string\(\)\.length\(3\)\.optional\(\),\s*\n?\s*\/\*\* Reason recorded on the audit row \+ the customer-visible audit slice\. \*\/\s*\n?\s*reason: z\.string\(\)\.min\(1\)\.max\(500\),\s*\n?\s*\}\);/,
    );
  });
  ...
  it('AdminAccountResponse = AccountSchema mirror; returned by mutation endpoints so callers see post-update state without extra GET', () => {
    expect(body).toMatch(
      /\/\/ Admin account view \(mirrors AccountSchema; returned by mutation endpoints\s*\n?\s*\/\/ so callers see the post-update state without an extra GET\)\./,
    );
  ...
  it('V-521 ListAuditLogQuery target_resource_id framing pinned: admin-side parity with V-484 customer audit-log filter set; drill into single resource (one webhook delivery) across every admin action that touched it', () => {
    expect(body).toMatch(
      /\/\/ V-521 — admin-side parity with the V-484 customer audit-log\s*\n?\s*\/\/ filter set\. Drill into a single resource \(e\.g\. one webhook\s*\n?\s*\/\/ delivery\) across every admin action that touched it\./,
    );
    expect(body).toMatch(
      /export const ListAuditLogQuerySchema = z\.object\(\{\s*\n?\s*admin_id: z\.string\(\)\.optional\(\),\s*\n?\s*target_id: z\.string\(\)\.optional\(\),\s*\n?\s*action: AdminAuditActionSchema\.optional\(\),\s*\n?\s*from: Iso8601Schema\.optional\(\),\s*\n?\s*to: Iso8601Schema\.optional\(\),/,
    );
  ...
  it('AdminAuditLogEntry: id uuid + admin_account_id + admin_key_id + action + target_account_id nullable + target_resource_id nullable + input_payload record nullable + result + ip_address nullable + timestamp', () => {
    expect(body).toMatch(
      /export const AdminAuditLogEntrySchema = z\.object\(\{\s*\n?\s*id: z\.string\(\)\.uuid\(\),\s*\n?\s*admin_account_id: z\.string\(\),\s*\n?\s*admin_key_id: z\.string\(\),\s*\n?\s*action: AdminAuditActionSchema,\s*\n?\s*target_account_id: z\.string\(\)\.nullable\(\),\s*\n?\s*target_resource_id: z\.string\(\)\.nullable\(\),\s*\n?\s*input_payload: z\.record\(z\.unknown\(\)\)\.nullable\(\),\s*\n?\s*result: z\.string\(\),\s*\n?\s*ip_address: z\.string\(\)\.nullable\(\),\s*\n?\s*timestamp: Iso8601Schema,\s*\n?\s*\}\);/,
    );
  });
