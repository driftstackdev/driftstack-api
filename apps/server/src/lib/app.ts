...
export async function buildApp(deps: AppDeps): Promise<FastifyInstance> {
  ...
  if (deps.aiCredits !== undefined) {
    // S15's admin routes write to the SEPARATE dark audit trail
    // (`db/ai-credits-admin-audit-repo.ts`), never `adminAuditService`, and
    // write it inside each change's own transaction (S15/S16 audit #2), so
    // there is no pool-level audit writer to pass.
