...
const TEMPLATES = {
  ...
  'signup-welcome': {
    ...
    text: (v) =>
      `Your Driftstack account is ready.\n\nNext steps:\n  1. Start free or pick a paid tier at ${v.dashboardUrl}\n  2. Create your first API key from the dashboard (paid plans)\n  3. Run your first session — in the desktop app, or from your code with the SDK\n\nAny questions: reply to this email.\n\n— Driftstack`,
    html: (v) =>
      `<p>Your Driftstack account is ready.</p><p>Next steps:</p><ol><li>Start free or pick a paid tier at <a href="${v.dashboardUrl}">${v.dashboardUrl}</a></li><li>Create your first API key from the dashboard (paid plans)</li><li>Run your first session — in the desktop app, or from your code with the SDK</li></ol><p>Any questions: reply to this email.</p><p>— Driftstack</p>`,
  },
  'session-failed-first': {
    subject: 'Driftstack — your first session failure',
    text: (v) =>
      `One of your Driftstack sessions failed: ${v.sessionId}\n\nError: ${v.errorMessage}\n\nThis is a one-time notice — we won't email you about later failures; the dashboard and your webhooks track those. Common causes and fixes are documented at ${v.docsUrl}.\n\n— Driftstack`,
    html: (v) =>
      `<p>One of your Driftstack sessions failed: <code>${v.sessionId}</code></p><p><strong>Error:</strong> ${v.errorMessage}</p><p>This is a one-time notice — we won't email you about later failures; the dashboard and your webhooks track those. Common causes and fixes are documented at <a href="${v.docsUrl}">${v.docsUrl}</a>.</p><p>— Driftstack</p>`,
  },
  ...
  'session-success-first': {
    ...
    text: (v) =>
      `Your first Driftstack session ran successfully (${v.sessionId}). Your account is set up and webhook deliveries are flowing.\n\nNext steps:\n  1. Watch live activity in the dashboard: ${v.dashboardUrl}\n  2. Read the quickstart to learn about recordings, profiles and webhook events: ${v.docsUrl}\n  3. Create more API keys for staging, CI or other environments from the dashboard.\n\nThis is a one-time email — we won't email you about later sessions. The dashboard and your webhooks take over from here.\n\n— Driftstack`,
    html: (v) =>
      `<p>Your first Driftstack session ran successfully (<code>${v.sessionId}</code>). Your account is set up and webhook deliveries are flowing.</p><p><strong>Next steps:</strong></p><ol><li>Watch live activity in the <a href="${v.dashboardUrl}">dashboard</a>.</li><li>Read the <a href="${v.docsUrl}">quickstart</a> to learn about recordings, profiles and webhook events.</li><li>Create more API keys for staging, CI or other environments from the dashboard.</li></ol><p>This is a one-time email — we won't email you about later sessions. The dashboard and your webhooks take over from here.</p><p>— Driftstack</p>`,
  },
  'tier-changed': {
  ...
  'byok-anthropic-key-rotation-reminder': {
    ...
    text: (v) =>
      `Your stored Anthropic API key on Driftstack is ${v.ageDays} days old. We recommend replacing it every 90 days.\n\nReplace by: ${v.rotateBy} (UTC)\n\nGenerate a fresh key in your Anthropic console (https://console.anthropic.com/), then update it on Driftstack through the API — the steps are documented at:\nhttps://docs.driftstack.io/api/byok-anthropic\n\nCheck your key's age and status anytime on the dashboard: ${v.dashboardUrl}/settings\n\n— Driftstack`,
    html: (v) =>
      `<p>Your stored Anthropic API key on Driftstack is <strong>${v.ageDays} days old</strong>. We recommend replacing it every 90 days.</p><table cellpadding="4" style="border-collapse:collapse"><tr><td><strong>Replace by:</strong></td><td>${v.rotateBy} (UTC)</td></tr></table><p>Generate a fresh key in your <a href="https://console.anthropic.com/">Anthropic console</a>, then update it on Driftstack through the API — <a href="https://docs.driftstack.io/api/byok-anthropic">the steps are documented here</a>.</p><p>Check your key's age and status anytime on the <a href="${v.dashboardUrl}/settings">dashboard</a>.</p><p>— Driftstack</p>`,
  },
  ...
  'webhook-secret-rotation-reminder': {
    ...
    text: (v) =>
      `Your Driftstack webhook signing secret is ${v.ageDays} days old. We recommend replacing it every 90 days.\n\nEndpoint: ${v.endpointUrl}\nSecret starts with: ${v.secretPrefix}\nReplace by: ${v.rotateBy} (UTC)\n\nThere's no downtime: after you rotate, the old secret keeps working for 24 hours so you can update your server at your own pace.\n\nRotate it here: ${v.dashboardUrl}/webhooks\n\n— Driftstack`,
    html: (v) =>
      `<p>Your Driftstack webhook signing secret is <strong>${v.ageDays} days old</strong>. We recommend replacing it every 90 days.</p><table cellpadding="4" style="border-collapse:collapse"><tr><td><strong>Endpoint:</strong></td><td><code>${v.endpointUrl}</code></td></tr><tr><td><strong>Secret starts with:</strong></td><td><code>${v.secretPrefix}</code></td></tr><tr><td><strong>Replace by:</strong></td><td>${v.rotateBy} (UTC)</td></tr></table><p>There's no downtime: after you rotate, the old secret keeps working for 24 hours so you can update your server at your own pace.</p><p><a href="${v.dashboardUrl}/webhooks">Rotate it here: ${v.dashboardUrl}/webhooks</a></p><p>— Driftstack</p>`,
  },
  // Arc 3 sub-slice 28.4 (v2-#28) — server-initiated force-rotation
  ...
  'webhook-secret-grace-expiring': {
    ...
    text: (v) =>
      `Heads up — the old secret for one of your webhook endpoints is about to stop working.\n\nEndpoint: ${v.endpointUrl}\nNew secret starts with: ${v.secretPrefix}\nOld secret stops working: ${v.graceWindowEndsAt} (UTC)\n\nIf your server still checks deliveries with the OLD secret, update it to the new one before the deadline. After that, deliveries are signed only with the new secret, so a server still using the old one will reject them.\n\nGet the current secret at:\n${v.dashboardUrl}/webhooks\n\n— Driftstack`,
    html: (v) =>
      `<p>Heads up — the old secret for one of your webhook endpoints is about to stop working.</p><table cellpadding="4" style="border-collapse:collapse"><tr><td><strong>Endpoint:</strong></td><td><code>${v.endpointUrl}</code></td></tr><tr><td><strong>New secret starts with:</strong></td><td><code>${v.secretPrefix}</code></td></tr><tr><td><strong>Old secret stops working:</strong></td><td>${v.graceWindowEndsAt} (UTC)</td></tr></table><p>If your server still checks deliveries with the OLD secret, update it to the new one before the deadline. After that, deliveries are signed only with the new secret, so a server still using the old one will reject them.</p><p>Get the current secret at <a href="${v.dashboardUrl}/webhooks">${v.dashboardUrl}/webhooks</a>.</p><p>— Driftstack</p>`,
  },
  'webhook-secret-force-rotated': {
    subject: 'Driftstack — your webhook secret was auto-rotated for security',
    text: (v) =>
      `Your Driftstack webhook signing secret was more than 91 days old, so we rotated it for you as a security measure.\n\nEndpoint: ${v.endpointUrl}\nNew secret starts with: ${v.newSecretPrefix}\nOld secret stops working: ${v.graceWindowEndsAt} (UTC)\n\nThe old secret keeps working until the date above, so your server has time to switch to the new one. After that, only the new secret will work.\n\nGet the new secret and manage your endpoints at:\n${v.dashboardUrl}/webhooks\n\n— Driftstack`,
    html: (v) =>
      `<p>Your Driftstack webhook signing secret was more than 91 days old, so we rotated it for you as a security measure.</p><table cellpadding="4" style="border-collapse:collapse"><tr><td><strong>Endpoint:</strong></td><td><code>${v.endpointUrl}</code></td></tr><tr><td><strong>New secret starts with:</strong></td><td><code>${v.newSecretPrefix}</code></td></tr><tr><td><strong>Old secret stops working:</strong></td><td>${v.graceWindowEndsAt} (UTC)</td></tr></table><p>The old secret keeps working until the date above, so your server has time to switch to the new one. After that, only the new secret will work.</p><p>Get the new secret and manage your endpoints at <a href="${v.dashboardUrl}/webhooks">${v.dashboardUrl}/webhooks</a>.</p><p>— Driftstack</p>`,
  },
