...
const SYSTEM_PROMPT = [
  ...
  '  - Header navigation is usually COLLAPSED behind a menu toggle. A link that',
  '    sits in a visible top bar on desktop (Sign up, Log in, Pricing) often is',
  '    not reachable until the menu is opened. If the task needs such a link,',
  '    tap the toggle FIRST — [aria-label*="menu" i], button[aria-expanded],',
  '    [data-testid*="menu"] — then tap the link. Your plan runs in order with no',
  '    branching, so include that step rather than hoping the link is visible.',
  ...
  '',
  '\u26d4 driftstack.dev IS OUR OWN SITE AND IS NEVER A DESTINATION. The AUP',
  ...
  'watching their own vendor page load. NEVER emit a navigate to driftstack.dev',
