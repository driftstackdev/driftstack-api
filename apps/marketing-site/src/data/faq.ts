  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        a: 'Yes — via NowPayments on tiers where crypto checkout is enabled. Open the crypto checkout from the billing dashboard, send the displayed amount in the displayed currency, and the order moves through pending → confirming → paid as on-chain confirmations land. (For developers: the webhook events — the automatic notifications our system sends yours as a payment progresses — are documented in the <a href="/docs/webhooks-crypto-events" class="text-tk-accent-text underline">crypto webhook events docs</a>.) <strong>Crypto payments are non-refundable</strong> — you can cancel anytime, which stops future billing, but the current period is not refunded (see <a href="/legal/refunds" class="text-tk-accent-text underline">refund policy</a>). Most customers use Stripe; crypto is a fallback for jurisdictions where card payments are awkward.',
      },
      {
