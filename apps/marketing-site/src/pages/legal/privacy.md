...
see §7 / DPA Annex 3), optional Customer-stated infrastructure
region preference (one of "us" / "eu" / "apac"; informational and
non-binding because it does not change current data residency — see
§6 / §7 for the authoritative routing).
...
on Customer's behalf** under the [DPA](/legal/dpa/), not as
Controller. Desktop-local recording files remain on Customer's
device and are not collected or retained by Driftstack. Live-session
media is ephemeral: it streams through LiveKit and is dropped on
session end. Live-session media is encrypted in transit on each
...
or marketing cookies remain disabled unless the required consent
mechanism and disclosure are active.
...
**Why:** infrastructure operation and abuse detection, and to let
Customers and prospects verify Service availability.

**Legal basis:** Article 6(1)(f) — legitimate interest in operating
the status page.

**Status-page content:** the status page surfaces Service-level
health metadata only (incident title, severity, affected components,
chronological updates, and resolution time). It does **not** expose
Customer Data, Capture or live-session content, Session payloads,
account identifiers, or PII. Driftstack operators or automated
health probes generate the data shown.

**Probe history:** Driftstack records each automated health probe
against its own public endpoints (timestamp, success flag, latency,
HTTP status, and error message). This internal operational data does
not reference a Customer or visitor and is retained for 30 days for
diagnostic purposes.

**Cookies:** none. The status page is served from Cloudflare Pages
and reads incident data from
`https://api.driftstack.dev/v1/status/incidents`.

### 3.10 Status-page email subscriptions

**What:** the email address submitted through the status-page
“Subscribe to incident updates” form, opaque opt-in and unsubscribe
tokens generated by Driftstack, and opt-in / unsubscribe timestamps.

**Why:** to notify Customers and prospects when a public Driftstack
Service incident is posted or resolved.

**Legal basis (GDPR Art 6):** Article 6(1)(a) — explicit,
freely-given consent through the double-opt-in flow. The address joins
the notification list only after the subscriber opens the
confirmation link. Every notification includes one-click
unsubscribe.
...
without a fresh double opt-in; the address is purged from that row 90
days after unsubscribe.

**Recipients:** Driftstack's EU-hosted Postgres stores the record;
Postmark dispatches notification emails as the Sub-processor listed
in Section 7.
...
**What:** real-time WebRTC media streams (rendered browser screen and
optional audio) generated only when Customer or authorised
Driftstack support explicitly starts a temporary live session. The
pipeline can carry rendered frames, screen-coordinate metadata,
optional audio (off by default), a LiveKit room id, and connection
metadata such as timestamps, ICE candidates, peer ids, and the region
selected by LiveKit Cloud.

**Why:** to enable real-time observation of an in-progress browser
session for Customer debugging or authorised support.

**Legal basis (GDPR Art 6):** Article 6(1)(b) — performance of the
contract for Customer-initiated use. A support viewer joins only
after Customer authorisation.

**Source:** Customer starts the live session through the GUI Client
or dashboard. Support cannot silently start one.

**Retention:** live-session media is **not stored**. Frames pass
through LiveKit's SFU in real time and are dropped on session end. No
durable copy lands in Driftstack's control plane. Driftstack has no
cloud recording endpoint or recording-retention window. Customer may
instead create a desktop-local recording on Customer's device or
request an inline API Capture artifact that the Capture endpoint does
not retain.

**Recipients:** LiveKit, listed in Section 7, for the SFU; Driftstack
support only while an authorised support live session is active.

**Cookies:** none.

**Cryptography:** media is encrypted in transit on each WebRTC
connection using DTLS-SRTP. LiveKit receives, processes, and forwards
...
published at [`/trust/sub-processors`](/trust/sub-processors/).
...
- **Right to data portability** (Article 20) — Personal Data the
  ...
  machine-readable format. Driftstack provides a self-service audit
  export at `/v1/account/audit-log/export?format=csv|json` (Customer
  Dashboard → Audit log → Export), capped at 10,000 rows per export;
  older entries remain available through the paginated read API.
