// W599.A — drift guard for the S14 new content surfaces (2026-07-03,
// D8): /use-cases (hub + 3 persona pages), /how-it-works, /glossary.
// Mirror pair: apps/server/tests/unit/
// marketing-site-pages-new-surfaces-content-parity.test.ts.
//
// What this pins and why:
//   • Route files exist + each page's unique h1 (PageHero title prop).
//   • Plan pointers: multi-account → /pricing#manual, qa-testing +
//     web-scraping → /pricing#api (the persona→ladder mapping is the
//     funnel's core claim: a person clicking → Manual, code calling →
//     API — drift would send a persona at the wrong ladder).
//   • CtaBand targets per page (all primary CTAs land on
//     /pricing#free, the honest no-card entry point).
//   • qa-testing cites the EXACT registered archetype slug
//     'iphone17_ios18_7_safari26_4' (registry-enforced by W280.A; a
//     drifted slug here would advertise a device we don't ship).
//   • /how-it-works stays ZERO-code (the page's whole reason to
//     exist); the homepage hero keeps its in-page "#how-it-works"
//     anchor (pinned — the new route is the nav entry, NOT a hero
//     retarget).
//   • /glossary anchor ids are a public contract (Band-B small-print
//     jargon links target them) — all 18 pinned; the three site-wide
//     metaphors (profile = saved iPhone identity, concurrent =
//     browser tabs, fingerprint = what a website can measure about a
//     visitor) must read the same here as on /pricing + faq.ts.
...
describe('W599.A S14 new-surface routes exist', () => {
...
describe('W599.A /use-cases/multi-account (operators + account teams)', () => {
...
describe('W599.A /use-cases/qa-testing (QA + engineering teams)', () => {
  const body = read(QA);
...
describe('W599.A /use-cases/web-scraping (data teams)', () => {
  ...
  it('population-matched framing reuses homepage §6 phrases: one of millions of real iPhones + stable across sessions vs 100% unique (2026-09-15 plain words: "population-stable" / "stealth Chromium mints" gone, same facts)', () => {
...
describe('W599.A /how-it-works (zero-code explainer)', () => {
  const body = read(HOW);
...
describe('W599.A /glossary (quiet reference page)', () => {
...
describe('W599.A honesty negatives across all six new surfaces', () => {
